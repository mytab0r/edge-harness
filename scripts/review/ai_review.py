...
Контракт ответа (вдохновлён живым решением владельца в Harness — см.
ADR 0007; сам паритет отсюда не проверить, поэтому фиксируем только наш
контракт): единственный сигнал вердикта — машиночитаемая строка
«ВЕРДИКТ: approve|rework», встречающаяся РОВНО ОДИН раз; маркера нет или их
два — error, неоднозначность никогда не одобряет. Позиция строки не
проверяется (#1332, дельта ADR 0007 п.4): требование «последняя» при
действующей единственности ничего не снимало, зато отбрасывало корректные
вердикты — см. докстринг parse_verdict.
...
# Контракт ответа модели. Строка ВЕРДИКТ обязана быть ЕДИНСТВЕННОЙ —
# двусмысленность это error, а не одобрение. ПОЗИЦИЯ строки не проверяется
# (#1332, дельта ADR 0007 п.4): при действующей единственности требование
# «последняя» ничего не снимало, зато отбрасывало корректные вердикты —
# см. докстринг parse_verdict. Модель периодически
...
# test_ai_review.py::test_every_pre_model_reason_carries_prefix держит связь:
# новая ветка «ревью не состоялось», написанная мимо константы, красит CI.
REVIEW_NEVER_HAPPENED_PREFIX = "ревью не состоялось"

# #1332, третий класс причины (находка ai-ревью PR #1333, head 7804149):
# гигантский дифф без обязательной строки РАЗМЕР (#939) — модель ОТВЕТИЛА,
# контракт ВЕРДИКТ не нарушала, и шапка «ответ не соответствует контракту
# вердикта» лгала бы. Признак класса — префикс-константа причины (тот же
# механизм, что REVIEW_NEVER_HAPPENED_PREFIX выше); причину собирает
# size_missing_reason — единственное место, где живёт этот текст.
SIZE_JUDGMENT_MISSING_PREFIX = "суждение о размере отсутствует"
...
def parse_verdict(answer: str) -> str:
    """approve | rework | error. Единственный сигнал — машиночитаемая строка
    ВЕРДИКТ, встречающаяся РОВНО ОДИН раз в ответе; маркера нет или их два —
    error. ПОЗИЦИЯ не требуется (#1332).

    Почему позиция снята. Требование «последняя строка» при уже действующем
    требовании «ровно одна» не покупает НИЧЕГО: неоднозначность снимает
    единственность, а не место. Цена же требования измерена на живых прогонах —
    три из одиннадцати упавших ai-review за сутки 2026-09-16 отдали
    contract_violation при НАЛИЧИИ корректного маркера, просто не в конце.
    Дословный случай (PR #1328, прогон 35155260819): модель выдала
    `ВЕРДИКТ: approve` первой строкой и следом содержательный разбор — гейт
    выбросил одобрение целиком, PR встал с меткой ai:failed, дорогой вызов
    модели сожжён. Модели, обслуживающие цепочку, склонны дописывать
    мета-комментарий после решения; контракт, воюющий с этой склонностью,
    проиграл ей в трёх из одиннадцати разобранных отказов за сутки —
    измеренная цена, не риторика.

    Что НЕ изменилось: два маркера по-прежнему error (модель противоречит
    себе), ноль маркеров по-прежнему error (решения нет — угадывать его по
    прозе запрещено, «алерт не гадает»). Промпт просит писать строку сразу,
    до пояснений — против забывания, не ради позиции: машина принимает её
    в любом месте и больше не отвергает ответ за её нарушение.
...
def parse_size_verdict(answer: str) -> tuple[str, str | None]:
    ...
    ищется по ВСЕМУ ответу, и матчей обязано быть РОВНО один (прежняя оговорка
    «не только последней строке — ВЕРДИКТ уже занимает эту позицию» снята
...
def size_missing_reason(added: int) -> str:
    """Причина verdict=error «гигантский дифф без строки РАЗМЕР» (#939) —
    чистая функция, не проза внутри cmd_verdict: шапка этого класса
    (error_headline) и её тест обязаны разбирать РЕАЛЬНУЮ строку причины,
    как это сделано для цепочки через error_reason. Текст начинается
    с SIZE_JUDGMENT_MISSING_PREFIX — единственного признака класса."""
    return (f"{SIZE_JUDGMENT_MISSING_PREFIX}: дифф +{added} строк "
            f"(порог {check_pr.LARGE_DIFF_HUGE_LINES}) требует явного суждения "
            "о размере (строка «РАЗМЕР: оправдан» либо «РАЗМЕР: раздут: …», "
            "см. ai_prompt.md), но модель её не дала — строки нет, их "
            "несколько, или формулировка не по контракту (тот же класс, что "
            "пустой rework без находок, #210)")
...
def size_question_section(added: int) -> str:
    ...
    return (
        ...
        "(в любом месте ответа) РОВНО одну из двух строк:\n"
        "РАЗМЕР: оправдан\n"
...
def error_reason(answer: str, dsh_rc: str, failure_reason: str = "",
    ...
    if failure_reason == "empty_diff":
        return (f"{REVIEW_NEVER_HAPPENED_PREFIX} — дифф PR пуст (0 изменённых файлов), "
    ...
    if failure_reason == "diff_source_mismatch":
        return (f"{REVIEW_NEVER_HAPPENED_PREFIX} — источники диффа PR разошлись: gh "
    ...
    if tag == review_labels.FAILURE_REASON_QUOTA_EXHAUSTED:
        return (f"{REVIEW_NEVER_HAPPENED_PREFIX} — квота провайдера исчерпана надолго "
    ...
    if tag == review_labels.FAILURE_REASON_RATE_LIMIT_BUDGET:
        return (f"{REVIEW_NEVER_HAPPENED_PREFIX} — временный RATE_LIMIT провайдера не "
                f"снялся за отведённый бюджет ожидания внутри прогона (код "
                f"возврата {dsh_rc})")
    if tag == review_labels.FAILURE_REASON_TRANSPORT:
        return f"{REVIEW_NEVER_HAPPENED_PREFIX} — ошибка провайдера/транспорта DSH (код возврата {dsh_rc})"
...
def size_judgment_missing(reason: str) -> bool:
    """Причина — «гигантский дифф без обязательной строки РАЗМЕР» (#939),
    а не нарушение контракта строки ВЕРДИКТ: модель ответила и вердикт-строку
    дала ровно одну. Признак — SIZE_JUDGMENT_MISSING_PREFIX (тот же механизм,
    что review_never_happened выше)."""
    return (reason or "").startswith(SIZE_JUDGMENT_MISSING_PREFIX)


def review_never_happened(reason: str) -> bool:
    """Отказ произошёл ДО ответа модели (цепочка не дала ответа, дифф пуст,
    транспорт), а не при разборе ответа. Признак — общий префикс-константа
    REVIEW_NEVER_HAPPENED_PREFIX, которым помечены ВСЕ такие ветки составителя
    причины выше: формулировка каждой меняется свободно, класс — нет. Связь
    держит гвардия (test_every_pre_model_reason_carries_prefix), иначе новая
    ветка, написанная мимо константы, молча вернула бы шапку про контракт."""
    return (reason or "").startswith(REVIEW_NEVER_HAPPENED_PREFIX)


def error_headline(reason: str) -> str:
    """Шапка строки ::error:: — ПО КЛАССУ отказа, не одна на все (#1332,
    дефект 2; тот же класс, что #1322 в scripts/worker/task.sh). Отдельная
    функция, а не выражение внутри cmd_verdict: внутри неё выбор не
    проверяется тестом — мутация «вернуть общую шапку» проходила зелёной.

    Три класса, не одна шапка (находка ai-ревью PR #1333, head 7804149 —
    size-ветка #939 была третьим живым источником причины и печаталась под
    шапкой контракта ВЕРДИКТ, которого она не нарушала):
      отказ до модели            -> «до модели не дошло» (причина рядом несёт
                                    REVIEW_NEVER_HAPPENED_PREFIX в начале);
      гигантский дифф без РАЗМЕР -> «ответ не соответствует контракту
                                    строки РАЗМЕР» (контракт ВЕРДИКТ выполнен);
      всё остальное              -> «ответ не соответствует контракту
                                    вердикта».

    Ветка отказа-до-модели НЕ повторяет начало причины (та же находка):
    cmd_verdict печатает «<шапка> (<причина>)» одной строкой, а каждая
    причина этого класса УЖЕ начинается с REVIEW_NEVER_HAPPENED_PREFIX —
    шапка с тем же началом дублировала его в каждой такой строке. Класс
    назван остатком шапки и полным текстом причины рядом. Формулировки
    здесь подобраны так, чтобы НЕ повторять литерал префикса: его в
    исходнике модуля ровно два вхождения, и это инвариант, который держит
    test_every_pre_model_reason_carries_prefix."""
    if review_never_happened(reason):
        return "до модели не дошло"
    if size_judgment_missing(reason):
        return "ответ не соответствует контракту строки РАЗМЕР"
    return "ответ не соответствует контракту вердикта"
...
def cmd_verdict(args: argparse.Namespace) -> int:
    ...
    if size_status == "bloated" and verdict != "error":
        ...
        verdict = "error"
        reason = size_missing_reason(added)
    ...
    if verdict == "error":
        ...
        # #1332, дефект 2 (тот же класс, что #1322 в scripts/worker/task.sh):
        # шапка была ОДНА на все причины, и исчерпание цепочки печаталось как
        # «ответ не соответствует контракту вердикта» — при том что модель
        # вообще ничего не отвечала. Три прогона из одиннадцати за сутки
        # 2026-09-16. Лечатся эти два случая противоположным (ждать сброса
        # квоты против починить промпт/парсер), поэтому шапка обязана их
        # различать — AGENTS.md, «алерт не гадает».
        print(f"::error::{error_headline(reason)} ({reason}) "
