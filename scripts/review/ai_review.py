...
единственный сигнал вердикта — машиночитаемая строка
...
def parse_verdict(answer: str) -> str:
    ...
    проигрывает ей на каждом прогоне.
...
def error_headline(reason: str) -> str:
    ...
    Ветка отказа-до-модели НЕ повторяет начало причины (находка ревью
    PR #1333): cmd_verdict печатает «<шапка> (<причина>)» одной строкой, а
    каждая причина этого класса УЖЕ начинается с REVIEW_NEVER_HAPPENED_
    PREFIX — шапка с тем же началом дублировала его в каждой такой строке.
    Класс назван остатком шапки и полным текстом причины рядом. Формулировки
    здесь подобраны так, чтобы НЕ повторять литерал префикса: его в исходнике
    модуля ровно два вхождения, и это инвариант, который держит
