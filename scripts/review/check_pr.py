...
Проверки:
  ...
  3. Крупный дифф (> LARGE_DIFF_LINES) помечается меткой review:large — авто-слияние
     ...
     не подтверждаются фактом approve — модель ОТДЕЛЬНО и явно судит объём
     (оправдан/раздут, scripts/review/ai_review.py::huge_diff_size_gate) —
     мандат владельца 2026-09-11 (#939) убрал эскалацию владельцу по причине
     размера: не креденшел и не деньги, решать не человеку (отменяет #204
     п.«escalate»/#901).
...
# Второй, более высокий порог: дифф длиннее него не подтверждается фактом
# approve сам по себе — AI-ревью обязано ОТДЕЛЬНО и явно судить объём
# (huge_diff_size_gate в ai_review.py, #939) — «раздут» становится rework,
# «нет суждения» становится error, «оправдан» ставит large-ok как обычно.
# Эскалация владельцу по причине размера убрана (мандат 2026-09-11): не
# креденшел, не деньги — решает автоматика, не человек. Значение —
...
def main() -> int:
    ...
    # см. LARGE_DIFF_HUGE_LINES там же: диффы сверх неё требуют ОТДЕЛЬНОГО
    # суждения модели о размере (huge_diff_size_gate, #939), не эскалации
    # владельцу. Остальные проверки метка не отключает.
