...
Живой расход читается тем же секретом CLOUDFLARE_API_TOKEN через GraphQL:
Workers requests/сутки — python scripts/measure/quotas.py (workflow
quotas.yml, #324, workersInvocationsAdaptive); DO rows_read/rows_written —
python scripts/measure/do_rows_read.py --days N (задача #320; право Account
Analytics Read у токена есть — доказано живыми прогонами 2026-09-05, раздел
«Замер факта: rows_read в проде» там же).
Не подтверждено: датасет GB-s duration — инструмента под него ещё нет
(таблица «Что реально доступно» в docs/agents/INFRA-CF.md).
EOF
