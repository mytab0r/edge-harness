...
if resp=$(cf_get "/accounts/${CLOUDFLARE_ACCOUNT_ID}/logpush/jobs"); then
  ...
  echo
  # Некритичное замечание ревью PR #328: прежний текст гадал о причине
  # («Logs Read или план») — факт один: cf_get не выполнился, и точный код
  # он уже напечатал сам. Интерпретация — в доке, где замер зафиксирован.
  echo "Факт: GET /accounts/{id}/logpush/jobs не выполнен — точный код и причина в строке «НЕТ ДОСТУПА»/«ОШИБКА» выше, их печатает cf_get."
  echo "docs/agents/INFRA-CF.md: замер 2026-09-05 зафиксировал на этом эндпоинте HTTP 403 (право Logs Read токену не выдано); выдаёт право владелец."
  echo "«Нет доступа к списку» ≠ «Logpush не настроен»: о наличии или отсутствии джобов здесь утверждать нельзя."
fi
