...
# ── случай 16: обратное поле «Что блокирует» с номерами — валидно ─────────────
# Задача #710: поле «Что блокирует: B» переносится auto_wire'ом с разворотом,
# тело с ним уже несёт разбираемую связь; гейт, знающий только прямое поле,
# отказывал бы телу, которое граф потом разберёт (находка ревью PR #804).
rm -f "$MARKER"
body16="### Что блокирует
#123
"
if ! bash "$SCRIPT_SRC" --title t --body "$body16" --label task "${NOT_PROCESS_ACK[@]}" >"$WORK/out16" 2>"$WORK/err16"; then
  note "FAIL случай 16: обратное поле «Что блокирует» отклонено"; cat "$WORK/err16"; fail=1
elif [ ! -f "$MARKER" ]; then
  note "FAIL случай 16: gh не вызван для обратного поля"; fail=1
else
  note "OK случай 16: обратное поле «Что блокирует» принято"
fi

# ── случай 17: обратное поле «Что блокирует» с «ничем» — валидно ─────────────
rm -f "$MARKER"
body17="## Что блокирует
ничем
"
if ! bash "$SCRIPT_SRC" --title t --body "$body17" --label task "${NOT_PROCESS_ACK[@]}" >"$WORK/out17" 2>"$WORK/err17"; then
  note "FAIL случай 17: обратное поле «ничем» отклонено"; cat "$WORK/err17"; fail=1
elif [ ! -f "$MARKER" ]; then
  note "FAIL случай 17: gh не вызван для обратного поля «ничем»"; fail=1
else
  note "OK случай 17: обратное поле «Что блокирует: ничем» принято"
fi
