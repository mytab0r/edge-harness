...
# переменную (#422) переживает unset: если цепочка провайдеров откажет
# (квота/лимит/исчерпание),
...
if [ "$rc" -eq 0 ]; then
  ...
  # Цепочка провайдеров отказала (#422) — не сбой агента: событие журнала различает это
  ...
  flush_events
  # Квота надолго, бюджет RATE_LIMIT ИЛИ цепочка исчерпана целиком (#727/#805;
  # с #1084 исчерпание — универсальный исход отказа, конкретную причину
  # называют ::warning:: каждой попытки)
  ...
  if [ -n "$ISSUE_NUMBER" ] && { [ "$HANDS_TASK_FAILURE_REASON" = "quota_exhausted" ] || \
    ...
    if [ "$release_rc" -eq 0 ]; then
      echo "Цепочка провайдеров отказала — задача #$ISSUE_NUMBER возвращена в пул немедленно: $release_out"
    else
