...
# независимо): инцидент #1100 (полный путь 6.1%→100% занял 83 минуты,
# документированный тест test_trend_horizon_catches_1100_incident_before_
# exhaustion в test_quota_alert.py) показывает, что горизонт 45 минут ловит
# реальный тренд с запасом ~53 минуты до исчерпания и ~46 минут до первого
# отказа оркестратора — независимое подтверждение достаточности, а не
# копия чужого числа. Единственное определение (не дублируется в другом
# модуле): прежняя гвардия дрейфа сверяла это число с ЧУЖОЙ формулой
# (CHECK_INTERVAL_MINUTES × 3 в quota_watch.py), а не с копией самого себя —
# ревизия #1184 сняла и гвардию, и формулу как проверяющую не то (см.
# git-историю test_quota_watch.py).
