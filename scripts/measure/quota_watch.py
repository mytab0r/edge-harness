...
30 прогонов и не 7-дневным лукбеком; чтение маркеров #120 ограничено
MARKER_SCAN_PAGES. Худший случай тика — 1 вызов списка прогонов + прогоны
внутри окна простоя (при каденции 15 мин — 3–4) + до 2 страниц комментариев:
десятки запросов/час, не сотни.
...
`scan_measurement_history` сканирует ровно одну страницу — 30
...
Правка: элемент возврата (`scan`, константы `SCAN_*`) различает исходы, и
...
- `SCAN_ALL_WINDOW` — просмотрены ВСЕ завершённые прогоны внутри окна,
  успешного замера нет ни в одном: success_age — ДОСТОВЕРНАЯ нижняя
  граница простоя.
- `SCAN_CEILING` — страница полна и целиком внутри окна, за её пределами
  завершённые прогоны есть: success_age — та же нижняя граница, точный
...
- `SCAN_STALE_PROVEN` — досрочная остановка скана (found: ревью PR #607,
  ...
  эскалация доказана, глубже скан не идёт.
...
# SCAN_ALL_WINDOW — просмотрены ВСЕ завершённые прогоны внутри окна,
# успешного замера нет ни в одном; success_age — нижняя граница простоя;
# SCAN_CEILING — страница полна и целиком внутри окна, за её пределами
# завершённые прогоны есть (осмотрены не все); success_age — та же нижняя
# граница простоя, точный возраст последнего успеха не установлен;
# SCAN_STALE_PROVEN — досрочная остановка скана (found: ревью PR #607,
...
# доказана, его шаги (и всё, что старше) даже не запрашиваются (стоимость
# тика ограничена окном простоя, не лукбеком/страницей).
SCAN_EXACT = "exact"
...
# (SCAN_STALE_PROVEN, main-путь) и по MEASUREMENT_SCAN_PER_PAGE; 7 дней
# лишь гарантируют, что цикл не остановится РАНЬШЕ по возрасту прогона, не
# найдя достаточно старый прогон в пределах страницы. Исход «страница полна,
# история продолжается, успешного замера нет» — НЕ холодный старт, а
...
def scan_measurement_history(
    ...
    """Один скан истории прогонов от новых к старым — два возраста
    ...
    скан не идёт вглубь лукбека/страницы.
    Иначе (found: ревью PR #607, «цена гейта не сведена против бюджета
    github.token») затяжной отказ замера гонял бы по 30 jobs-вызовов за тик
    из общего на репозиторий лимита 1000 запросов/час.
    ...
    Сбой самой проверки истории (сеть, права) или шаги более свежих, чем
    нижняя граница, прогонов, осмотреть не удалось — api_ok=False:
    решение (и троттлинг, и простой) по таким данным принимать нельзя."""
    try:
        data = pulse_guard.gh(
        ...
        return MeasurementScan(None, None, False, SCAN_COLD_START, 0)
    ...
    if uninspected:
        # Успеха среди осмотренных нет, но часть прогонов НОВЕЕ нижней
        # границы не осмотрена — пропущенный мог оказаться самым свежим
        # замерившим, нижняя граница простоя недоказуема.
...
def gate_main() -> int:
    ...
    if scan_result.scan == SCAN_CEILING:
        reason = (f"{reason}; кроме того, успешного замера нет среди последних "
                  f"{MEASUREMENT_SCAN_PER_PAGE} завершённых прогонов (самый старый из просмотренных "
