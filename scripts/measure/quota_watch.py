...
(непредсказуемо — от нуля до нескольких синхронизаций в минуту): проверка
истории прогонов этого же workflow — один дешёвый GitHub REST вызов (не
CF), не требующий отдельного маркера-«тика» в #120 (не плодим комментарии
на каждый холостой пуш). Худший случай — 96 дешёвых проверок/сутки (~9
GraphQL-запросов каждая, ~864/сутки) плюс ДО 1 полного среза/сутки внутри
первой четверти часа (см. `full_sweep`) — на два порядка меньше
документированного лимита Cloudflare API (1200 запросов/5 минут на токен).
