...
# ── Паттерн «Класс закрыт» — это ERE, а не PCRE (#1436) ──────────────────────
#
# `run_class_closed_check` зовёт `git grep -nE`, то есть POSIX ERE с
# GNU-расширениями. Автор, привыкший к Python `re`, пишет PCRE — и попадает в
# одну из двух ловушек. Что в какую попадает, ИЗМЕРЕНО живым `git grep`
# 2026-09-22 на временном репозитории, а не взято из документации и не
# перенесено из формулировки задачи (её список оказался неверен наполовину,
# см. ниже):
#
#   ГРОМКАЯ — git падает rc=128, но текстом, который не называет ни причину,
#   ни лечение:
#       (?:a|b)  →  fatal: … 'Invalid preceding regular expression'
#       (?i)abc  →  то же
#       (?=a)    →  то же
#       (?!a)    →  то же
#
#   ТИХАЯ — git НЕ падает, считает совпадения и отдаёт правдоподобное число,
#   которое ничего не значит. Это опаснее громкой: гейт «подтверждает»
#   заявление, которого автор не делал.
#       \d  матчит БУКВУ «d», не цифру (стенд: файл «5» не совпал, файл «d»
#            совпал). \D — букву «D». Замена: [0-9] и [^0-9].
#       a+?  матчит «xyz» по паттерну q+?xyz — `?` сделал `q+` необязательным,
#            а не ленивым. <.*?> на «<a><b>» вернул «<a><b>» целиком, то есть
#            жадно. Лениво в ERE не выражается вовсе — паттерн надо
#            переписать через отрицающий класс, например <[^>]*>.
#
#   РАБОТАЕТ КАК ОЖИДАЕТСЯ (GNU-расширения) — НЕ отвергаем, иначе это тормоз
#   без причины: \w, \W, \s, \S, \b, {n,m}, [[:digit:]].
#
# Поправка к формулировке задачи #1436, названная вслух: она просила
# отвергать «как минимум (?:, (?i), (?=, (?!, \d, \w, \s, \b». Замер
# показал, что \w/\s/\b работают правильно (отвергать их нельзя), а
# главная опасность — не названные в задаче ленивые квантификаторы, потому
# что они молчат.
_ERE_SILENT_DIGIT_RE = re.compile(r"(?<!\\)\\(?:\\\\)*([dD])")


def _unescaped_positions(pattern: str) -> list[int]:
    """Индексы символов паттерна, НЕ экранированных обратным слэшем.

    Нужен именно обход, а не регулярка по паттерну: `\\*?` — это литеральная
    звёздочка, сделанная необязательной (законно), а `a*?` — ленивый
    квантификатор (молча жадный). Различает их только чётность слэшей
    слева."""
    free: list[int] = []
    index = 0
...
def ere_pattern_problems(pattern: str) -> list[str]:
    """Ловушки PCRE-в-ERE в паттерне «Класс закрыт» — список адресных
    сообщений (пусто = паттерн честный ERE).

    Проверяется ДО `git grep`: громкие конструкты иначе доходят до автора
    ретрансляцией текста git («Invalid preceding regular expression»), а
    тихие не доходят вовсе."""
    problems: list[str] = []
    free = set(_unescaped_positions(pattern))

    for index in sorted(free):
        char = pattern[index]
        nxt = pattern[index + 1] if index + 1 < len(pattern) else ""
        if char == "(" and nxt == "?":
            construct = pattern[index:index + 3]
            problems.append(
                f"«{construct}» — расширение группы из PCRE; `git grep -nE` это ERE и "
                f"падает на нём с «Invalid preceding regular expression». "
                f"Незахватывающая группа `(?:…)` в ERE пишется обычной `(…)`; "
                f"`(?i)`/`(?=`/`(?!` аналога не имеют — перепиши паттерн"
            )
        elif char in "*+?}" and nxt == "?" and (index + 1) in free:
            problems.append(
                f"«{char}?» — ленивый квантификатор из PCRE. `git grep -nE` НЕ падает: "
                f"он читает `?` как «предыдущее необязательно» и молча считает не то "
                f"(замер: `q+?xyz` совпал со строкой «xyz», где ни одного `q` нет). "
                f"Лениво в ERE не выражается — перепиши через отрицающий класс, "
                f"например `<[^>]*>` вместо `<.*?>`"
            )

    for match in _ERE_SILENT_DIGIT_RE.finditer(pattern):
        letter = match.group(1)
        problems.append(
            f"«\\{letter}» — класс цифр из PCRE. `git grep -nE` НЕ падает: он матчит "
            f"БУКВУ «{letter}» (замер: файл со строкой «5» не совпал, файл со строкой "
            f"«{letter}» совпал), то есть число совпадений будет правдоподобным и "
            f"бессмысленным. Замена: "
            f"{'[0-9]' if letter == 'd' else '[^0-9]'}"
        )
    return problems
...
class ClassClosedClaim:
    """Заявление «класс закрыт»: ERE-паттерн + ожидаемое число совпадений.

    Паттерн проверяется в `__post_init__`, а не в `run_class_closed_check`
    (#1436): так объект с ловушкой PCRE физически не существует, и ни один
    вызывающий — ни гейт, ни будущий — не может её обойти, собрав claim
    руками. Тот же приём «нарушить невозможно, а не написано», который
    AGENTS.md требует от носителя решения."""

    pattern: str
    expected_count: int

    def __post_init__(self) -> None:
        problems = ere_pattern_problems(self.pattern)
        if problems:
            raise MutationClaimFormatError(
                f"{CLASS_CLOSED_HEADING}: паттерн «{self.pattern}» написан на PCRE, а "
                f"проверяется `git grep -nE` — это POSIX ERE. "
                + " | ".join(problems)
            )
