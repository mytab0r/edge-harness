#!/usr/bin/env python3
"""Патчит распакованный плагин dsh-anthropic-oauth-pool: четыре точечных правки.
...
файла»):
  - PATCH_POOL_CLASSIFY_EXPORT (lib/pool.js): добавляет ЧИСТУЮ функцию
    ...
    `unknown` (аккаунт ни разу не пробовался в этом процессе). Секреты/токены
    сюда не попадают — только числовой код ответа.
