...
Живой инцидент 2026-09-22: `@deepseek-ai/cordis` 4.0.4 (опубликован
15:36:40Z) попал в диапазон `^4.0.1` пакета `dsh@0.1.1-rc.2`, и `dsh` начал
падать с «user patch-layer watching requires the Cordis HMR service» ДО
обращения к провайдеру. Конвейер слияний встал: ни один PR не мог получить
`ai:ok`, восемь провайдеров цепочки «отказывали» одинаково, а итог советовал
повтор, который не мог помочь никогда.
...
def test_drifted_version_is_a_loud_refusal(tmp_path):
    ...
    решать."""
    result = _run_check(_npm_root(tmp_path, "4.0.4"))

    assert result.returncode != 0, "уехавшая зависимость прошла молча — это и есть #1467"
    assert "4.0.4" in result.stderr
    assert _pinned("DSH_EXPECTED_CORDIS") in result.stderr
