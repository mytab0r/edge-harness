...
задачей #224 — «граф блокировок вместо выдуманных дат»; дополнен задачей
...
порядке:
  ...
  (2) Метка `IMPACT_LABEL` (`impact:system`, задача #1178) — задача СНИМАЕТ
      измеренный системный барьер (потолок пропускной способности, класс
      отказа, затрагивающий несколько механизмов), если задача не попала в
      уровень 1. Самооценка заводящего/разбирающего, а не факт (в отличие
      от уровня 1) — честно названо как таковая (docstring `IMPACT_LABEL`).
      Причина появления уровня: замер #1178 на живом пуле (93 свободные
      задачи, 2026-09-14T00:30Z) — тай-брейк по номеру ВНУТРИ уровня
      `area:process` систематически топил свежезаведённые барьерные задачи
      (#1161/#1172/#1149/#1141/#1160/#1109, номер > 1100) под старыми
      «хвостами чеклиста ревью» той же меты (#1007..#1019, номер < 1020) —
      обеим не хватало отличающего признака ВНУТРИ уровня. Уровень 2 решает
      это не переворотом тай-брейка (риск: «старые долги» той же меты без
      явного веса ушли бы в хвост под свежими), а отдельным, явно
      объявляемым тиром выше.
  (3) Метка `area:process` (задача про сам процесс работы — протокол,
      гвардии, контракт, ревью-гейты, CI-ворота), если задача не попала в
      уровни 1/2. Уровень остаётся большим (114 из 303 открытых, замер
      ...
      транзитивным весом (см. (4)) убирает эту вырожденность там, где граф
      её видит, честно оставляя тай-брейк по номеру там, где сигнала
      физически нет (не выдуманная дата — явно объявленное «неизвестно»,
      см. `priority_reason`).
  (4) Транзитивный вес графа блокировок (`task_deps.transitive_blocking_counts`,
      ...
      параллельный расчёт. Решает ВНУТРИ каждого из уровней 1/2/3.
  (5) Номер issue как proxy даты создания (тайбрейк, без изменений с #361,
      СОЗНАТЕЛЬНО не тронут задачей #1178 — см. (2): «старые долги не
      забываем» внутри уровня остаётся в силе, барьерная задача получает
      свой более приоритетный УРОВЕНЬ, а не переворот сравнения по номеру).
...
меты/импакта/поломки (все прежние тесты «выбрать по номеру» остаются
зелёными без изменений: без `labels`/`blocked_by_open` уровни 1/2/3/4 не
отличают кандидатов → тайбрейк по номеру, тот же результат).
...
ни `IMPACT_LABEL` — не молчаливое вырождение: `main()` печатает
предупреждение в stderr при выборе (видимый сигнал, не тихий факт), выбор
при этом не останавливается (вырождение в уровень 5 для всех — легитимно).
Связка с #720 (объявление связи не спрашивают при заведении — граф долго
останется разреженным): приоритет здесь СОЗНАТЕЛЬНО спроектирован
работающим и на разреженном графе — уровень 1 (сейчас красный) не зависит
от графа вовсе, уровень 4 (транзитивный вес) деградирует к 0 для
большинства задач, но не ломает сортировку и не искажает уровни 1/2/3.
Автоматическое обратное заполнение
...
# Метка веса (задача #1178, вариант 1 из четырёх предложенных): задача СНИМАЕТ
# измеренный системный барьер (потолок пропускной способности, класс отказа,
# затрагивающий несколько механизмов сразу) — самооценка заводящего, а не
# факт (в отличие от `BROKEN_LABELS` ниже — «конвейер красный сейчас»), но
# признак, которого до этой задачи не существовало вовсе. Живой замер #1178
# (93 свободные задачи, 2026-09-14): тай-брейк по номеру issue внутри тира
# `META_LABEL` систематически топит свежезаведённые барьерные задачи под
# старыми «хвостами чеклиста ревью» той же меты — обеим не хватало отличающего
# признака ВНУТРИ тира 1. Ставится вручную при заведении/разборе задачи, той
# же конвенцией, что и `META_LABEL` (`docs/agents/PROTOCOL.md`) — газа
# (условия снятия) нет, метка живёт до закрытия issue, см. `docs/agents/
# LABELS.md`.
...
def _is_impact(issue: dict[str, Any], impact_label: str = IMPACT_LABEL) -> bool:
    """Метка веса `IMPACT_LABEL` (#1178) — независима от `meta_label`: задача
    может нести обе (барьерная задача про сам процесс работы) или только
    `impact_label` (барьерная задача вне `area:process`, например миграция
    патч-серии `dsh-edge` — живой пример #1149 замера #1178)."""
    return any(label.get("name") == impact_label for label in (issue.get("labels") or []))
...
def issue_priority_key(
    ...
    """Кортеж из трёх позиций сравнения, ПЯТЬ уровней по смыслу (задача #224,
    обобщает #361; тир `impact_label` добавлен задачей #1178):
    (1) `tier` — 0, если задача в `urgent` (несёт `BROKEN_LABELS` или
        транзитивно блокирует такую — см. `_urgent_numbers`, ФАКТ, не
        самооценка); иначе 1, если помечена `impact_label` (`IMPACT_LABEL`,
        #1178 — задача СНИМАЕТ измеренный системный барьер, самооценка
        заводящего, но явно объявленная, не выведенная угадыванием по
        содержимому тела); иначе 2, если помечена `meta_label`; иначе 3 —
        единственная позиция сравнения, различающая уровни (1)/(2)/(3)
        исходного описания задачи #224/#1178;
    (2) минус транзитивный вес (`transitive`, задача #224: обобщение
        `blocking_open` на всю цепочку, не только прямых соседей — см.
        `task_deps.transitive_blocking_counts`) — решает ВНУТРИ `tier`, тем
        же способом на всех пяти уровнях, включая новый;
    (3) номер issue — тайбрейк, proxy даты создания (меньше — раньше);
        НЕ тронут задачей #1178 намеренно — «старые долги не забываем»
        внутри тира остаётся в силе, барьерная задача просто получает СВОЙ
        более приоритетный тир, а не переворачивает сравнение по номеру.

    `impact_label` — независим от `meta_label` (см. `_is_impact`): задача с
    `IMPACT_LABEL` обгоняет ЛЮБУЮ `meta_label`-задачу без этой метки,
    независимо от того, несёт ли она саму `meta_label` тоже.
    ...
    tier = (
        0 if is_urgent
        else 1 if _is_impact(issue, impact_label)
        else 2 if _is_meta(issue, meta_label)
        else 3
    )
...
def graph_is_empty(
    issues: list[dict[str, Any]], meta_label: str = META_LABEL,
    impact_label: str = IMPACT_LABEL,
) -> bool:
    """Ни у кого нет транзитивного веса > 0, ни у кого нет `meta_label`,
    `impact_label` (#1178) и никто не «сейчас красный»
    (`BROKEN_LABELS`/`_urgent_numbers`) — уровни 1/2/3/4 не отличают НИ ОДНОГО
    кандидата, приоритет целиком вырождается в уровень 5 (номер). Не поломка
    (легитимное состояние на старте внедрения графа/до заведения ci-failure
    задач), но обязана быть видимым сигналом (AGENTS.md: fail loud, не
    ...
    return all(
        ...
        and not _is_impact(issue, impact_label)
...
def priority_reason(
    ...
    broken_labels: frozenset[str] = BROKEN_LABELS, impact_label: str = IMPACT_LABEL,
) -> str:
    ...
    elif _is_impact(issue, impact_label):
        tier_text = f"тир 1 — метка «{impact_label}» (снимает измеренный системный барьер, #1178)"
    elif _is_meta(issue, meta_label):
        tier_text = f"тир 2 — метка «{meta_label}» (про сам процесс работы)"
    else:
        tier_text = "тир 3 — не чинит текущий красный CI, не снимает системный барьер и не про процесс"
