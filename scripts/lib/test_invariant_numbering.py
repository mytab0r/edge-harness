...
def test_parse_registry_entries_pins_the_live_repo_invariants_registry():
    ...
    # Не range(1, N): #1253 занял 22, #1277 занял 23 (арбитр
    # `invariant_numbering.py next` — 19/20/21 заняты сторонними открытыми
    # PR на 2026-09-14/15), поэтому реестр main+этой ветки — 1..18 подряд
    # плюс 22/23, с разрывом до слияния тех PR. Разрыв — ожидаемое следствие
    # арбитража по открытым PR, не дрейф формата.
    assert set(parsed) == {str(n) for n in range(1, 19)} | {"22", "23"}, (
