...
_dsh_chain_report_exhausted() { # count
  ...
  [ "$unclear" -gt 0 ] && action="${action:+$action; }установить причину у: $names_unclear — провайдер ответил 404, а 404 не различает «такой модели нет» и «такого маршрута нет» (#1494). Дешёвая проверка: scripts/measure/provider_latency.py зовёт ТУ ЖЕ запись цепочки (тот же base_url, id и секрет) прямым `<base_url>/chat/completions` — HTTP 200 там означает, что запись цепочки исправна и чинить надо путь вызова, а не id модели (живой случай: docs/research/28-provider-chain-truth-table.md)"
