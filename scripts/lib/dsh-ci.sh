...
# следующего провайдера» — эта последняя категория (стоп-класс) резервируется
# только за причинами, которые ДЕЙСТВИТЕЛЬНО не зависят от того, какого
# провайдера/аккаунта/base_url/секрета пробуют дальше: искажённый вердикт-
# контракт решается выше по стеку (ai_review.py::parse_verdict, эта функция
# его не видит), сама CLI dsh сломана целиком (не текст ответа провайдера, а
# ошибка ДО сетевого вызова) или явно названный отказ credentials текущей
# записи (см. INVALID_API_KEY ниже — единственный оставшийся именованный
# стоп-класс).
#
...
# факт «наш таймаут», факт «класс не распознан» (переключаемся по #1084) или
# первая строка именованного стоп-класса.
#
# Ветка «стоп-класс» и ветка «класс не распознан» ниже — единственные, что
# кладут в переменную СЫРОЙ фрагмент stderr клиента модели, а не заранее
# известную безопасную строку (имя причины/литеральный маркер/факт пустоты) —
# обе обязаны идти через redact() (#743): начало ответа 401 у провайдера
# типично содержит эхо заголовка Authorization, GitHub маскирует только
# точное совпадение секрета, производное (подстрока/префикс) — нет.
# Маскируем здесь, у источника, ОДИН раз — оба места печати (::warning:: и
# ::error:: ниже) читают уже замаскированную переменную, второй копии redact
# на каждую точку вывода не нужно (то же место правды, что redact() выше в
# этом файле).
dsh_chain_should_advance() { # err_file failure_reason rc
  ...
  # #1084: единственный оставшийся ИМЕНОВАННЫЙ стоп-класс — явный отказ
  # credentials ЭТОЙ записи (прод-форма классификатора dsh — «dsh:
  # INVALID_API_KEY: ...», тот же префиксный стиль, что HTTP_404:/
  # RATE_LIMIT:/INVALID_REQUEST:/STREAM_CLOSED: выше). Смена ключа/аккаунта
  # следующей записи цепочки НЕ гарантирует, что там всё исправно (секрет
  # мог быть отозван у всех разом при ротации) — но признак говорит о
  # проблеме НАСТРОЙКИ/секрета, которую агенту/человеку стоит увидеть явно
  # (::error:: с честным текстом), а не растворять в перечисленных 8
  # попытках переключения. Единственный случай в списке владельца #1084,
  # который остаётся генуинно «стоп»: «нет ключа» и «квота аккаунта»
  # обрабатываются раньше — как пропуск ДО вызова dsh (секрет не передан
  # workflow'ом) и как quota_exhausted (advance) соответственно, оба НЕ
  # доходят до этой функции с сырым текстом.
  if grep -qE 'INVALID_API_KEY:' "$err_file"; then
    DSH_CHAIN_CLASS_NOTE="$(tr '\n' ' ' <"$err_file" | cut -c1-200 | redact)"
    return 1
  fi
  # #1084: КЛАСС не распознан (не входит ни в один явный признак выше) —
  # умолчание перевёрнуто с «стоп» на «пробуем следующего»: см. развёрнутый
  # довод в комментарии над функцией. Раньше именно эта ветка возвращала 1
  # (стоп) для ЛЮБОГО непойманного текста — новый провайдер узнавался только
  # из уже упавшего прогона (#737/#877/#1062/STREAM_CLOSED выше — четыре
  # инцидента ради четырёх строк). Теперь новый нераспознанный текст не
  # останавливает цепочку целиком сам по себе — останавливают только явно
  # названные причины (INVALID_API_KEY выше, quota/rate_limit в начале
  # функции — там осознанно решено иначе).
