...
#   3. в РАБОЧЕМ ДЕРЕВЕ появился хотя бы один новый коммит за время этого
#      прогона (branch_start_sha != branch_end_sha) — иначе PR, найденный
#      pr_outcome.py, может оказаться артефактом ПРЕДЫДУЩЕГО прогона, ровно
#      как в живом случае.
#
...
#
# Не подтверждено (находка ai-review PR #880, второй раунд): признак 3
# (start_sha != end_sha) не отличает НОВЫЙ коммит от переписанной истории —
# `git rebase`/`commit --amend` тоже двигают HEAD без единой строки новой
# работы. Маршрут воркера предписывает DSH ребейзить при дрейфе main (шаг 3
# `route_pr_step`), и такой ребейз внутри прогона теоретически мог бы дать
# тот же зелёный вердикт без работы этого прогона — узкая форма того же
# класса #876, не закрытая здесь. Усиление признака (датировка коммитов
# относительно начала прогона, сверка с головой PR) — не сделано, честно
# названо, а не тихо допущено.
#
...
# диагностики — при эволюции гейта список гэпов молча протухал бы).
# Пустая строка на успехе; иначе — предложения через ", ", каждое называет
# КОНКРЕТНО несработавшее условие, не общий текст.
dsh_worker_run_is_success() { # rc chain_provider branch_start_sha branch_end_sha
  local rc=$1 chain_provider=$2 start_sha=$3 end_sha=$4
  local gaps=""
  [ "$rc" -eq 0 ] \
  ...
  [ "$start_sha" != "$end_sha" ] \
    || gaps="${gaps:+$gaps, }новых коммитов в ветке за этот прогон нет"
