...
def label_search_value(label: str) -> str:
    ...
    * `search/issues?q=label:<метка>` — метка это часть ВЫРАЖЕНИЯ ПОИСКА,
      ...
      отправить `%253A` вместо двоеточия. Ломает же запрос другое: разбор
      выражения у GitHub режет квалификатор по ПЕРВОМУ двоеточию, и
      `label:session:archive-pending` читается как метка `session` — фильтр
      молча отбирает не то, пустым списком, а не ошибкой. Ровно тот же
      молчаливый промах, что класс #938, только на другой поверхности.

    Лечение — кавычки выражения поиска: `label:"session:archive-pending"`.
    Ставятся, только когда значение их требует (двоеточие или пробел), —
    лишние кавычки на простом `task` читались бы как часть имени.
    """
