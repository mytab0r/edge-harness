...
def label_search_value(label: str) -> str:
    """Значение метки для КВАЛИФИКАТОРА поиска (`search/issues?q=label:…`).

    Второй способ подстановки метки в запрос, и он не взаимозаменяем с
    `label_query_value` выше (#1432). Разница не стилистическая:

    * `issues?labels=<метка>` — метка это значение query-ПАРАМЕТРА, и её
      обязано кодировать процентами: `label_query_value`;
    * `search/issues?q=label:<метка>` — метка это часть ВЫРАЖЕНИЯ ПОИСКА,
      которое целиком кодируется процентами уже на сборке URL
      (`urllib.parse.quote(query)`). Кодировать её здесь вторично значило бы
      отправить `%253A` вместо двоеточия. Замер 2026-09-22 (#1432, живой
      репозиторий, метка `area:process`): движок декодирует ТОЛЬКО
      транспортный слой — `label:area:process` без кавычек и
      `label:"area:process"` в кавычках дают по 343 совпадений, а
      `label:area%3Aprocess` (проценты ВНУТРИ значения, как после
      label_query_value) — 0, молча пустой список: тот же класс #938, на
      другой поверхности. Таблица замера —
      docs/research/21-github-actions.md.

    Кавычки (`label:"session:archive-pending"`) — выбранная форма значения с
    двоеточием/пробелом: обе формы дают одинаковый результат (замер: 343 = 343),
    кавычки однозначны для парсера и ставятся одним местом. На простом значении
    (`task`) они не нужны, но и не вредят — «лишние кавычки становятся частью
    имени» не подтверждено и замером опровергнуто, поэтому условность формы —
    вопрос краткости живого запроса, не семантики.

    Отрицание той же формой (`-label:"…"`) — прод-форма запроса свипа закрытых
    задач (#1432), померена тем же живым прогоном 2026-09-22:
    `-label:"area:process"` → 606 и `label:"area:process"` → 344 в сумме дают
    все 950 issues репозитория, то есть отрицание с кавычками работает без
    потерь; форма с сырым двоеточием (`-label:session:archived`) дала тот же
    счёт, что и кавычки. Таблица — docs/research/21-github-actions.md.
    """
...
#
# Находки реестра (#1262), отмеченные исправленными (НАХОДКА-ЗАКРЫТА),
# НЕ идут через этот факт шапки: маркер живёт в ТЕЛЕ PR (review_findings.
# merge_resolved_marker), тем же приёмом, что чеклист #462 — after_merge уже
# читает pull["body"] без отдельного сетевого запроса; сканировать историю
# комментариев PR ради этого поля было бы лишним GET на каждое слияние.
FACT_RE = re.compile(
