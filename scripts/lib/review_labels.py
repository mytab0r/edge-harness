...
def label_search_value(label: str) -> str:
    ...
    * `search/issues?q=label:<метка>` — метка это часть ВЫРАЖЕНИЯ ПОИСКА,
    ...
    двоеточием/пробелом: обе рабочие формы дают одинаковый результат, кавычки
    однозначны для парсера и ставятся одним местом. Ставятся, только когда
    значение их требует, — лишние кавычки на простом `task` читались бы как
    часть имени.
    """
