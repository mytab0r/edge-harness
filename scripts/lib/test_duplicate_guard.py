...
def test_evidence_common_path_filtered_on_large_pool():
    ...
    `new_body` дословно цитирует номер кандидата #3 (правило (c) требует
    self-cite + общий путь/ссылку) — без этого условие (c) не срабатывает
    вовсе независимо от фильтра, и мутация «`_common_items` -> всегда
    пусто» красит тест ложно-зелёным (находка ревью PR #579: «тест не
    краснеет от снятия фильтра»). С self-cite на #3 общий путь — ЕДИНСТВЕННАЯ
    вторая улика: фильтр активен -> путь common -> пусто; фильтр снят ->
    путь считается уликой -> матч на #3."""
    common_path = "docs/research/30-rejected-alternatives.md"
    boilerplate = f"- [x] Я прочитал {common_path} и задача не из отвергнутых"
    new_body = f"# Совсем другая задача, см. #3\n{boilerplate}"
    candidates = [
