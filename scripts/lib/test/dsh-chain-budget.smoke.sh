...
# таймаутом job'а worker.yml (timeout-minutes: 340)). Проверяет DSH_CHAIN_TOTAL_BUDGET_SECS
...
#
#   Сценарии 4-9 — СИМУЛИРОВАННЫЕ часы (симуляция нужна, потому что
...
#   min(аппетит, DSH_ATTEMPT_KNIFE_SECS) — ножа ИМЕННО этой попытки, как
#   его выставляет dsh_run_with_retry (в проде то же делает настоящий
...
#   сценарий 9 — ретраебельный RATE_LIMIT с НЕнулевой ценой попытки:
#   потолок доказан суммарными симулированными часами (≤ бюджет+1с), а не
#   подсчётом переменных (находка ai-review PR #1247: ретраи одного
#   провайдера получали полный срез таймаута). Исполненная мутация (нож
#   ретраев снят): 301 ретрай × 60с = 18360с симулированных часов при
#   бюджете 100с — 183× бюджета, потому что бюджет ожидания RATE_LIMIT
#   считал только ПАУЗЫ, не цену попыток.
#
...
# либо одно число на все вызовы; обрезается ножом ИМЕННО этой попытки
# (DSH_ATTEMPT_KNIFE_SECS, ставит dsh_run_with_retry перед каждым запуском
# с учётом дедлайна цепочки #1160; fallback на DSH_TIMEOUT_SECS — для
# сценариев без цепочного дедлайна) — так же, как реальный процесс
# обрезается настоящим `timeout`.
...
allowed="${DSH_ATTEMPT_KNIFE_SECS:-${DSH_TIMEOUT_SECS:-999999999}}"
...
# 03:13:39Z → 08:11:23Z, окно 17864с ≈ 298 мин, отменён внешне по таймауту
# job'а (timeout-minutes: 340)). Фактические длительности попыток из его лога: GLM 7200с (наш
...
# ── 9) Ретраибельный RATE_LIMIT + НЕнулева цена попытки (находка ai-review
# PR #1247: бюджет резал только границу провайдера, а каждый РЕТРАЙ внутри
# dsh_run_with_retry получал полный срез таймаута — «висящая» попытка с
# RATE_LIMIT сжигала срезы подряд, и класс #1141 оставался достижимым
# внутри формального бюджета). Симуляция: бюджет 100с,
# DSH_TIMEOUT_SECS=1000 (больше бюджета — связывать обязан дедлайн цепочки,
# не таймаут попытки), GLM каждая попытка «висит» 60с и отвечает
# ретраебельным RATE_LIMIT, бюджет ожидания RATE_LIMIT большой (1800с —
# НЕ связывающий). Потолок доказан СУММАРНЫМИ часами: часы симуляции
# обязаны остановиться на бюджете (≤101с); ИСПОЛНЕННАЯ мутация (нож
# ретраев снят) уводит их на 18360с — 301 ретрай × 60с + паузы, 183×
# бюджета (бюджет ожидания RATE_LIMIT считает только паузы, не цену
# попыток).
reset_sim_scenario
export SIM_MODE_glm_5_3_flash=rate_limit
export SIM_ADVANCE_glm_5_3_flash=60
export DSH_CHAIN_TOTAL_BUDGET_SECS=100
export DSH_TIMEOUT_SECS=1000
export DSH_RATE_LIMIT_INITIAL_DELAY_SECS=1
export DSH_RATE_LIMIT_MAX_DELAY_SECS=1
export DSH_RATE_LIMIT_MAX_WAIT_SECS=1800
LOG="$WORK/log9.txt"
run_sim "$LOG"
OUT="$(cat "$LOG")"
[ -f "$SIM_CALLED_GLM" ] || fail "9) GLM обязан быть вызван: $OUT"
[ ! -f "$SIM_CALLED_OR2" ] || fail "9) OpenRouter-2 НЕ обязан быть вызван — бюджет (100с) исчерпан ретраями GLM: $OUT"
[ "$DSH_RUN_FAILURE_REASON" = "chain_budget_exhausted" ] \
  || fail "9) ожидался chain_budget_exhausted изнутри ретраев, получено '$DSH_RUN_FAILURE_REASON'"
[[ "$OUT" == *"нож попытки 2 урезан остатком бюджета цепочки (#1160): 39с"* ]] \
  || fail "9) второй РЕТРАЙ обязан получить нож, урезанный остатком бюджета (39с из 100), а не полный срез: $OUT"
[[ "$OUT" == *"попытка 3 НЕ стартует"* ]] \
  || fail "9) третья попытка обязана НЕ стартовать — fail loud на исчерпании дедлайна: $OUT"
[[ "$OUT" == *"суммарный бюджет (100с/#1160) исчерпан после 100с и 1 из 4 провайдеров (опробованы: GLM)"* ]] \
  || fail "9) каноническое сообщение цепочки обязано назвать 100с и 1 из 4 опробованных: $OUT"
SIM_CLOCK_END="$(cat "$SIM_CLOCK")"
[ "$SIM_CLOCK_END" -le 101 ] \
  || fail "9) суммарные симулированные часы ${SIM_CLOCK_END}с при бюджете 100с — ретраи одного провайдера вышли за общий бюджет (потолок доказывается суммарным временем, не числом переменных)"
[ "$SIM_CLOCK_END" -ge 99 ] \
  || fail "9) суммарные часы ${SIM_CLOCK_END}с — меньше цены даже одной попытки (60с): сценарий перестал видеть ненулевую цену попытки, почините фикстуру"
echo "SMOKE(chain-budget): 9) ретраебельный RATE_LIMIT с ценой попытки 60с остановлен дедлайном на 100с суммарных часов (исполненная мутация без среза ретраев даёт 18360с) — ок"
