...
# ── 21) Бюджет ожидания retryAt — СУММАРНЫЙ, не per-попытка: два подряд
#      close-retryAt (6с каждый) при бюджете 10с — первый укладывается
#      (10>6), второй уже НЕ укладывается в остаток (10-6=4 < 6) -> третьей
...
(
  ...
  [[ "$OUT" == *"жду и повторяю тем же провайдером (#1288)"* ]] || { echo "::error::21) первый retryAt (3с) обязан был уложиться в бюджет и вызвать повтор: $OUT" >&2; exit 1; }
  [[ "$OUT" == *"это дольше бюджета ожидания 30с (уже ждал "*"с, остаток "*"с)"* ]] || { echo "::error::21) второй retryAt (50с) обязан честно назвать превышение ОСТАТКА суммарного бюджета (не полного бюджета заново): $OUT" >&2; exit 1; }
...
(
  ...
  [[ "$OUT" == *"это дольше бюджета ожидания 300с"* ]] || { echo "::error::22) сообщение обязано честно назвать превышение бюджета (не молчать, не гадать): $OUT" >&2; exit 1; }
