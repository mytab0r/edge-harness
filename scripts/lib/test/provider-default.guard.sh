...
# директорию `test/` или `testdata/`). Это ТОТ ЖЕ признак, которым до находки
# #1111 вручную, по одному разу на файл, размечались dsh-clients.smoke.sh/
...
# не требует отдельной правки этого файла. `testdata/` — то же соглашение
# для ФАЙЛОВ ДАННЫХ теста, а не кода: живой случай — корпус реальных цитат
# логов scripts/orchestra/testdata/reachability/real_error_log_corpus.txt
# (issue #1172), чьё НАЗНАЧЕНИЕ — нести чужой литерал (`nemotron-3-ultra`)
# байт-в-байт; без него каждый корпус реальных цитат снова требовал бы
# ручной allowlist-записи, ровно тот ручной режим, который #1111 снял.
#
