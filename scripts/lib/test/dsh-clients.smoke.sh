...
curl() { # заглушка-диспетчер по URL; поддерживает -o FILE и -w %{http_code}
  ...
  case "$url" in
    ...
    *api.telegram.org*)
      ...
      # категорию (#1461). Многострочный текст отчёта журнал не рвёт: перенос
      # экранирует сам `log_call`, один раз на все девять точек записи — см.
      # его определение выше. Здесь копии этой обработки НЕТ намеренно (две
      # копии одного правила — отложенный рецидив, AGENTS.md).
      log_call "TG-SEND $*"
