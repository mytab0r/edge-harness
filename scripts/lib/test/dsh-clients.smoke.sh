...
# Один вызов — одна строка журнала, и это инвариант, на который опираются ВСЕ
# проверки ниже (`grep -F … | head -1` берёт запись целиком). Держался он до
# сих пор случайно: ни один аргумент не содержал перевода строки. Как только
# содержит — запись рвётся на две, проверка видит первый обрывок и краснеет на
# ИСПРАВНОМ коде (живой случай #1461: категория сигнала стоит отдельной первой
# строкой текста, смок выдал «заголовок ушёл неэкранированным», хотя
# экранирование работало). Перевод строки экранируется здесь, в стенде, а не
# обходится подгонкой проверок — иначе тот же отказ вернётся на следующем
# многострочном аргументе.
log_call() { printf '%s\n' "${*//$'\n'/\\n}" >>"$CALLLOG"; }
...
curl() { # заглушка-диспетчер по URL; поддерживает -o FILE и -w %{http_code}
  ...
  case "$url" in
    */api/auth/login)
      # #572: SMOKE_MORDA_LOGIN_FAIL воспроизводит морду недоступную ЦЕЛИКОМ
      # (квота/500/сеть) — ни один из 4 внутренних ретраев dsh_edge_login не
      # проходит, login возвращает 1 честно, не зависает.
      if [ -n "${SMOKE_MORDA_LOGIN_FAIL:-}" ]; then
        code=500
        body=""
      else
        code=303
    ...
    */ingest)
      # #572: SMOKE_MORDA_INGEST_FAIL воспроизводит «морда есть (логин/сессия
      # прошли), но приём транскрипта сломан» — прод-форма ответа скопирована
      # из живого прогона 34719189444 (500 Internal runtime error, ровно тот
      # `appendHarnessEvents`/openAgentForTurn класс, что и завёл #572).
      if [ -n "${SMOKE_MORDA_INGEST_FAIL:-}" ]; then
        code=500
        body='{"ok":false,"error":"Internal runtime error."}'
      else
        local count
    ...
    *api.telegram.org*)
      ...
      # ассерты держат parse_mode=HTML, экранирование заголовка (tg_html) и
      # категорию (#1461). Журнал ПОСТРОЧНЫЙ, а текст отчёта с #1461
      # многострочный (категория — первая строка, `\n`, текст): перенос внутри
      # аргумента рвал запись журнала — хвост текста становился «бездомной»
      # строкой без префикса TG-SEND, и построчные ассерты видели только
      # «⚙️ Конвейер» (красный прогон repo-ci на исправном проде). Экранируем
      # переносы ТОЛЬКО В ЛОГЕ — сам вызов идёт как шёл (#1373: заглушка не
      # упрощает форму вызова, под которую писалась).
      local _tg_args=${*//$'\n'/\\n}
      log_call "TG-SEND $_tg_args"
...
dsh() { # прогон пишет спул+ответ; dump-config доказывает монтаж плагина
  case "${1:-}" in
    ...
    --profile)
      ...
      # (сценарий worker-chain-refusal-green ниже).
...
# только «слито в main» (scheduler.after_merge). Отчёт об открытом PR — ход
# конвейера: категория видна префиксом первой строки (#1461).
...
grep -qF -- "⚙️ Конвейер" <<<"$tg_line" \
  || { echo "::error::SMOKE: worker: отчёт без категории конвейера: $tg_line" >&2; exit 1; }
...
# предсуществующем PR» ОБЯЗАН быть красным, а не «справился» по факту одного
# существования PR (находка ai-review PR #880:
...
# pr_outcome_rc»). После #1084/#1286 путь разбивается на два: (а) dsh отказал
# на провайдерном классе — задача возвращается в пул (release-full), сигнал
# уходит в комментарий и Telegram, а job остаётся ЗЕЛЁНЫМ: красный прогон
# воркера по чужой вине кормил бы предохранитель диспатча (#226) и
# задерживал восстановление после сброса квоты (#205) — #1286, живой прогон
# 34893177035; (б) dsh отчитался успехом, но ни одного нового коммита не
# оставил — единственная форма, в которой текст гейта «не доказывает работу
# этого прогона» достижим.
scenario_start
WORKER_LOGIN="mytab0r" \
WORKER_TASK="123" \
...
SMOKE_DSH_STOP_ERROR="INVALID_API_KEY: unauthorized" \
  run_client "worker-chain-refusal-green" "$REPO/scripts/worker/task.sh"
...
# 2) worker: RATE_LIMIT Weekly/Monthly (квота надолго) — задача возвращается
# в пул СРАЗУ (снят и замок, и назначение), сообщение различает «цепочка
# провайдеров отказала» от «воркер не справился» (правило AGENTS.md), а job
# с #1286 остаётся ЗЕЛЁНЫМ: квота — вина провайдера, не сбой воркера.
scenario_start
WORKER_LOGIN="mytab0r" \
WORKER_TASK="123" \
...
GH_PR_LIST_URL_JSON='[]' \
SMOKE_RATE_LIMIT_MODE="quota-exhausted" \
  run_client "worker-rate-limit-quota" "$REPO/scripts/worker/task.sh"
assert_log "GH-API-LOCK-DELETE refs/locks/task-123" "worker-rate-limit-quota: замок снят при квоте — задача возвращена в пул, не ждёт TTL-сборщика (#422)"
assert_log "GH-API-UNASSIGN issue-123" "worker-rate-limit-quota: назначение снято при квоте — задача свободна другим каналам (#422)"
assert_log "цепочка провайдеров отказала" "worker-rate-limit-quota: сообщение различает провайдера от собственной ошибки"
...
echo "SMOKE: worker-rate-limit-quota — ок (#422/#1286: job зелёный, задача в пуле)"
...
# ── Морда недоступна: воркер обязан продолжить работу (#572) ────────────────────
# Живой инцидент 2026-09-12: квота DO-чтений морды превышена (5 161 474 /
# 5 000 000, 103.2%) — прогон 34719189444 упал на «Сессия harness-1051 не
# создана в морде — ход работы останется невидимым (#119)», хотя до этой
# строки задача воркера ещё даже не начиналась. Ниже — оба класса отказа
# морды (полная недоступность на логине; логин/сессия есть, но приём
# транскрипта сломан) и доказательство, что в ОБОИХ job зелёный, работа
# доведена до отчёта, а факт «транскрипт не доехал» всё равно виден в выводе
# job'а — не тихо.

# 1) Морда недоступна ЦЕЛИКОМ (логин отказывает на каждой из 4 попыток —
...
WORKER_LOGIN="mytab0r" \
WORKER_TASK="123" \
RUNNER_TEMP="$TMP/rt-w-morda-down" \
GH_TOKEN="smoke-pat-token" \
TELEGRAM_BOT_TOKEN="smoke-tg-token" \
TELEGRAM_CHAT_ID="42" \
...
grep -qF -- "не создана в морде — ход работы останется невидимым" "$WORKER_MORDA_DOWN_OUT" \
  && { echo "::error::SMOKE: worker-morda-down: старая фатальная формулировка (#119) всё ещё звучит" >&2
       cat "$WORKER_MORDA_DOWN_OUT" >&2; exit 1; }
echo "SMOKE: worker-morda-down — ок (#572)"

# 2) Морда ЕСТЬ (логин и сессия проходят — session.create/session.rename
# видны в журнале), но приём транскрипта сломан (500 «Internal runtime
# error.» — прод-форма живого класса #572, тот же 500, что видит
# scheduler.py::append_session_notes). Отличие от сценария 1 выше — сообщение
# называет «приём сломан», а не «морды нет», и «Ноль событий … при успешном
# прогоне» здесь НЕ должно появляться (тот гейт различает «морды нет вовсе» от
# «морда есть, приём сломан» — при сломанном приёме он бессмыслен и намеренно
# пропущен, см. комментарий в task.sh у DSH_EDGE_MORDA_AVAILABLE).
scenario_start
WORKER_MORDA_INGEST_FAIL_OUT="$TMP/worker-morda-ingest-fail.out"
WORKER_LOGIN="mytab0r" \
WORKER_TASK="124" \
RUNNER_TEMP="$TMP/rt-w-morda-ingest-fail" \
GH_TOKEN="smoke-pat-token" \
TELEGRAM_BOT_TOKEN="smoke-tg-token" \
TELEGRAM_CHAT_ID="42" \
...
assert_log "GH-COMMENT" "worker-morda-ingest-fail: нет отчёта в задачу — сломанный приём транскрипта не должен топить работу (#572)"
grep -qF -- "::warning::Транскрипт не принят мордой" "$WORKER_MORDA_INGEST_FAIL_OUT" \
  || { echo "::error::SMOKE: worker-morda-ingest-fail: нет ::warning:: о непринятом транскрипте — сломанный приём стал тихим" >&2
       cat "$WORKER_MORDA_INGEST_FAIL_OUT" >&2; exit 1; }
grep -qF -- "Ноль событий в сессии морды при успешном прогоне" "$WORKER_MORDA_INGEST_FAIL_OUT" \
  && { echo "::error::SMOKE: worker-morda-ingest-fail: гейт «ноль событий» сработал поверх уже известного сломанного приёма — «морды нет» и «приём сломан» перепутаны" >&2
       cat "$WORKER_MORDA_INGEST_FAIL_OUT" >&2; exit 1; }
grep -qF -- "Транскрипт не принят мордой — ход работы останется невидимым" "$WORKER_MORDA_INGEST_FAIL_OUT" \
  && { echo "::error::SMOKE: worker-morda-ingest-fail: старая фатальная формулировка (#119) всё ещё звучит" >&2
       cat "$WORKER_MORDA_INGEST_FAIL_OUT" >&2; exit 1; }
echo "SMOKE: worker-morda-ingest-fail — ок (#572)"
