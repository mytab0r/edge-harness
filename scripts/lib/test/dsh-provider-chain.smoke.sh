    ...
    --profile)
      ...
      case "$mode" in
        ...
        unknown-error)
          ...
          return 1 ;;
        http410)
          # #1289, живой инцидент — прогон worker.yml 34893177035 (тот же
          # прогон, что #1288): NVIDIA-NIM-1/2 отдали эту строку дважды
          # подряд — дословная прод-форма, не пересказ.
...
# ── 20) #1289, живой инцидент — прогон worker.yml 34893177035: NVIDIA-NIM-1/2
# отдали HTTP_410 Gone дважды подряд, и это падало в общий «класс не
# распознан» (тот же текст, что сценарий 18) — не различая транзиентный сбой
# от «эндпоинт мёртв навсегда». Решение по-прежнему advance (цепочка ИДЁТ
# дальше в ЭТОМ прогоне, поведение сценариев 1/18 — RATE_LIMIT(quota)/
# неизвестный класс — не меняется), но сообщение обязано:
#   (а) назвать СВОЙ текст причины («HTTP_410»), не «класс не распознан»;
#   (б) назвать это терминальным, не транзиентным, отказом;
#   (в) назвать конкретное действие (discovery/рунбук), не «пробую снова»;
#   (г) если модель известна — назвать её в сообщении.
reset_scenario
SMOKE_MODE_primary_model="http410"
SMOKE_MODE_secondary_model="ok"
LOG="$WORK/log20.txt"
dsh_run_with_provider_chain "$ANSWER" "$ERR" "промпт smoke" >"$LOG" 2>&1
OUT="$(cat "$LOG")"
[ "$DSH_RUN_RC" = "0" ] || fail "20) ожидался успех после HTTP_410 у первого, получено $DSH_RUN_RC"
[ "$DSH_CHAIN_PROVIDER" = "SECONDARY" ] || fail "20) ожидался переход на SECONDARY при HTTP_410 PRIMARY, получено '$DSH_CHAIN_PROVIDER'"
[ "$DSH_CHAIN_TRIED" = "PRIMARY, SECONDARY" ] || fail "20) DSH_CHAIN_TRIED='$DSH_CHAIN_TRIED' — оба провайдера обязаны быть опробованы"
[[ "$OUT" == *"класс отказа: HTTP_410"* ]] \
  || fail "20) явная ветка HTTP_410 обязана дать СВОЙ текст причины, не «класс не распознан»: $OUT"
[[ "$OUT" != *"класс не распознан"* ]] \
  || fail "20) HTTP_410 не обязан падать в общий недиагностируемый класс — он именован явно: $OUT"
[[ "$OUT" == *"терминальный отказ"* ]] \
  || fail "20) сообщение обязано назвать HTTP_410 терминальным, не транзиентным отказом: $OUT"
[[ "$OUT" == *"provider_model_discovery.py"* ]] \
  || fail "20) сообщение обязано назвать конкретное действие (discovery), не «пробую снова»: $OUT"
[[ "$OUT" == *"(модель 'primary-model')"* ]] \
  || fail "20) сообщение обязано назвать модель этой записи: $OUT"
[[ "$OUT" != *"класс НЕ переключаемый"* ]] \
  || fail "20) HTTP_410 у ОДНОГО провайдера не обязан останавливать всю цепочку (следующая запись — другая модель): $OUT"
echo "SMOKE(chain): 20) HTTP_410 Gone -> именованный терминальный класс, автопереход с честным адресным сообщением (#1289) — ок"

echo "SMOKE(chain): все сценарии цепочки провайдеров целы — гвардия класса #727/#737/#743/#857/#877/#880/#1062/#1084/#1121/#1289 зелёная"
