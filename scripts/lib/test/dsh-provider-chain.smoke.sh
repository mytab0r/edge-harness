...
( export DSH_PROVIDER_CHAIN="$UNCLEAR_CHAIN"
  ...
  # Дословно, вместе с угловыми скобками: обратные кавычки вокруг пути в
  # ДВОЙНЫХ кавычках bash — это подстановка команды, а не разметка. Живой
  # случай этого же PR: строка действия несла `<base_url>/chat/completions`,
  # bash попытался выполнить `<base_url>` и написал в stderr «No such file or
  # directory»; сообщение при этом ПЕЧАТАЛОСЬ почти целиком, поэтому проверка
  # по подстроке «provider_latency.py» выше осталась зелёной, а красным стал
  # чужой смоук клиентов через три шага. Эта проверка ловит класс на месте.
  [[ "$OUT" == *"<base_url>/chat/completions"* ]] \
    || { echo "::error::34) путь из действия потерялся — вероятно, разметка обратными кавычками стала подстановкой команды: $OUT" >&2; exit 1; }
