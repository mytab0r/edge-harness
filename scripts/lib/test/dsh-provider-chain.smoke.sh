...
# ключом 20 минут спустя — вердикт «dead_model» отправлял чинить исправное.
...
( export DSH_PROVIDER_CHAIN="$UNCLEAR_CHAIN"
  ...
  [[ "$DSH_CHAIN_OUTCOME_SUMMARY" == *"причина не установлена"* ]] \
    || { echo "::error::34) сводка обязана назвать класс «причина не установлена»: '$DSH_CHAIN_OUTCOME_SUMMARY'" >&2; exit 1; }
