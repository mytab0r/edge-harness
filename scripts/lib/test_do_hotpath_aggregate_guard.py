...
определению (спека 14.4). Полный `GROUP BY status` по таблице без фильтра
...
`messages` (`messages` вдобавок не имела ретеншена вовсе, растёт вечно).
...
# Строковый литерал SQL, не упоминание в прозе/комментарии: `GROUP BY <col>"`
# — закрывающая кавычка сразу после имени колонки маркирует конец запроса.
SQL_GROUP_BY_RE = re.compile(r'GROUP BY \w+"')
