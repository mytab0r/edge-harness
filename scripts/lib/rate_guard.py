...
Решение принимается по МЕНЬШЕМУ из двух остатков. Изначально это была
страховка на случай, если заголовки окажутся не тем счётчиком; страховка
больше не нужна как страховка, но остаётся как поведение. Живой раннер
ответил на первом же прогоне нового гейта (35724770013, job `test`,
12:01:55 UTC, issue #1457):

    rate_guard: rate_limit говорит 5000/5000, заголовки (core) — 4821/5000 (use 179)

`rate_limit` — полный бак (`used 0`), заголовки — `used 179` на ПЕРВОМ шаге
job'а, которых сам job не делал: счётчик общий на репозиторий и отражает
реальный расход. То есть читать надо заголовки, и минимум из двух с ними
совпадает. Оба числа продолжают печататься рядом: это дешёвый непрерывный
замер, а не остаток неизвестности.
...
def is_rate_limit_refusal(text: object) -> bool:
    """True — текст отказа означает «счётчик GitHub ответил: пусто».

    Одно место правды на классификацию (#1004): её спрашивают и сам гейт, и
    гвардии каталога `scripts/ci/guards`, которые ходят в API своим кодом.
    Две копии регулярки разошлись бы на первом же новом варианте текста."""
    return bool(RATE_LIMIT_REFUSAL_RE.search(str(text or "")))


def run_guard_main(main_callable, *, guard: str) -> int:
    """Обёртка `main()` гвардии, которая читает GitHub API (#1004).

    Зачем. Обязательная проверка `test` гоняет каталог `scripts/ci/guards`
    одним шагом, и гейт квоты его НЕ гасит — гасить целиком нельзя, там
    подавляющее большинство гвардий в API не ходит. Но ЧЕТЫРЕ ходят (замер
    2026-09-22: ci_guard_registration_guard, decision_numbering,
    pr_mutation_claim_check, plugin_manager_roster_guard), и при исчерпанном
    бюджете они падали трейсбеком — то есть красили чужой PR причиной, к
    его диффу отношения не имеющей. Живой случай: прогон 35727716021,
    PR #1458, шаг «Каталог гвардий» — `decision-doc-numbering-guard`
    свалился на `HTTP 403: API rate limit exceeded for installation` ПОСЛЕ
    того, как гейт квоты корректно пропустил шаг инвариантов.

    Граница та же, что у самого гейта («Граница» в докстринге модуля):
    исчерпанный счётчик — это ОТВЕТ, а не сбой измерения, поэтому
    `::warning::` и код 0. Любая другая ошибка проходит наверх нетронутой:
    гвардия обязана краснеть на настоящей поломке.

    Честный потолок, названный вслух: проверка при этом не выполняется. Это
    осознанный размен — красная обязательная проверка по чужой квоте стоит
    дороже, чем один пропуск, а следующий прогон (после сброса бюджета)
    проверит то же самое. Пропуск виден в логе `::warning::`, а не молчит."""
    try:
        return main_callable()
    except Exception as error:  # noqa: BLE001 — классифицируем и пробрасываем чужое
        if not is_rate_limit_refusal(error):
            raise
        print(
            f"::warning::{guard}: бюджет GitHub API исчерпан — проверка пропущена, "
            f"job остаётся зелёным (#1004). Ответ GitHub: {error}"
        )
        return 0
