...
def test_extract_task_refs_does_not_strip_numeric_ratio_before_hash():
    # Живой контрпример (PR #1247, 2026-09-14): «при дефолтных 300/180
    # ретраи... #877» — оба сегмента до `#` чисто числовые (300, 180), это
    # НЕ форма owner/repo (репозиторий GitHub не бывает голым числом без
    # буквы) — #877 обязана остаться нашей задачей, иначе защита от чужого
    # репозитория сама стала бы новым классом ложноотрицательных срабатываний.
    text = "при дефолтных 300/180 ретраи RATE_LIMIT бюджетированы (#877)."
