...
ALLOWED_SINGLE_PAGE_CALLS = {
    ...
    ("scripts/orchestra/pulse_guard.py", "failure_watch"):
        "явный `per_page` — запрошены последние N ЗАВЕРШЁННЫХ прогонов ОДНОГО "
        "workflow (#477; вывод фильтруется клиентом — "
        ...
        "список. Размер — FAILURE_WATCH_PER_PAGE (20) с двойным запасом для "
        "orchestra.yml (100, FAILURE_WATCH_PER_PAGE_BY_WORKFLOW): он "
        "единственный отслеживаемый с PR-триггером, страница считается "
        "безотносительно окна, и при merge-шторме красных contract-прогонов "
        "свежий schedule-провал вытеснялся бы за страницу молча (замер ревью "
        "PR #488, раунд 4: 11 за 42 минуты, 2026-09-06).",
