...
ALLOWED_SINGLE_PAGE_CALLS = {
    ...
    ("scripts/orchestra/pulse_guard.py", "failure_watch"):
        "явный `per_page=20` — запрошены последние N провалов ОДНОГО workflow "
        "(#477), тот же контракт «дай N последних», что у recent_runs выше: "
        "функция смотрит только на самый свежий в окне свежести (runs[0] "
        "после клиентских фильтров), не листает список. 20, а не 3 — запас, "
        "чтобы клиентский фильтр event!=pull_request (класс красных PR-чеков "
        "ведёт автодетектор простоя #201, находка ревью PR #488) не вытеснил "
        "свежий провал основного события за страницу: PR-триггер среди "
        "отслеживаемых есть только у orchestra.yml (job contract).",
