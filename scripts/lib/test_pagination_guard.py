...
ALLOWED_SINGLE_PAGE_CALLS = {
    ...
    ("scripts/orchestra/repo_invariants.py", "fetch_actions_workflows"):
        "реестр workflow-записей Actions API этого репозитория (#940, "
        "инвариант 17) — растущий, но НЕ без верхней границы (уточнено после "
        "находки ревью PR #944): GitHub запрещает удалять запись, у которой "
        "был хоть один прогон, даже после удаления файла с диска, поэтому "
        "число растёт монотонно и навсегда (27 на замер 2026-09-11, включая "
