...
ALLOWED_SINGLE_PAGE_CALLS = {
    ...
    ("scripts/orchestra/pulse_guard.py", "failure_watch"):
        "явный `per_page=20` — запрошены последние N ЗАВЕРШЁННЫХ прогонов "
        ...
        "список. 20, а не 3 — запас, чтобы клиентские фильтры "
        "(event!=pull_request — класс красных PR-чеков ведёт автодетектор "
        "простоя #201, находка ревью PR #488; вывод не из "
        "FAILURE_WATCH_RUN_CONCLUSIONS) не вытеснили свежий провал основного "
        "события за страницу: PR-триггер среди отслеживаемых есть только у "
        "orchestra.yml (job contract).",
