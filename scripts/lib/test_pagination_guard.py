...
ALLOWED_SINGLE_PAGE_CALLS = {
    ...
    ("scripts/orchestra/scheduler.py", "in_progress_worker_runs"):
        "до WORKER_MAX_CONCURRENCY воркеров параллельно (#827): "
        ...
        "параллельности, не растёт). Единственное место запроса "
        "in_progress-прогонов воркера: stalled_worker_runs, "
    ...
    ("scripts/orchestra/scheduler.py", "closed_tasks_needing_archive"):
        "`per_page=limit` — потолок `CLOSED_TASK_SWEEP_LIMIT` (20, #1432) и "
        "есть смысл вызова: столько закрытых задач свип ставит в очередь за "
        "один пульс, хвост режется `[:limit]` и НЕ теряется — задача без "
        "обеих меток будет найдена тем же запросом на следующем проходе "
        "(носитель состояния — метка на задаче, не результат одного поиска; "
        "форма вызова — конкатенация строк, поэтому CALL_RE её не видит, "
        "признание осознанным делает эта запись и комментарий у вызова).",
}
