...
Contents API уже используется этим же процессом для похожей проверки
(scheduler.py::docs_missing читает repos/{repo}/contents/{file}?ref=main тем
же способом).

Обоснование «файл маленький (JSON на несколько сотен записей)» из прежней
редакции СНЯТО как неверное: файл растёт монотонно и к 2026-09-21 весил
97 771 байт, а его base64 (≈130 364 символа) упёрся в потолок ОДНОГО
аргумента командной строки (MAX_ARG_STRLEN, 131 072 байта) и уронил проход
оркестратора — issue #1406, прогон 35585768697. Размер здесь больше не
аргумент ни за, ни против Contents API: тело уходит файлом (`gh api
--input`, см. write_registry), и от длины оно не зависит вовсе. Потолок,
который остаётся реальным, — 1 МБ самого Contents API на файл, и он назван
отдельно в fetch_registry ниже.

Перебор класса «тело неограниченного размера в argv» по репозиторию сделан
2026-09-21 и дал НОЛЬ других мест (вывод здесь, а не только в теле PR
#1407 — комментарий читает участник переписки, модуль читает каждый
следующий агент): остальные вызовы с телом в argv — `body=` у комментариев
и issue (pool_issue.py, claim_task.py, pulse_guard.post_issue_comment) —
ограничены самим GitHub 65 536 символами, то есть вдвое ниже потолка
одного аргумента, и структурно недосягаемы; файлы контента через Contents
API пишет только write_registry ниже; соседний писатель данных
(scripts/lib/data_branch_writer.py) ходит git-клоном и push'ем, argv не
касается вовсе.
