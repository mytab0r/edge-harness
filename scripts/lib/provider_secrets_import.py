...
# ключе (задача #777, major 5 гейта PR #778 — живое наблюдение: один и тот же
# шлюз вернул 451 автору находки и 200 гейту минутами позже, проба
# одноразовая, повторов нет). Поэтому 403 — ХУЖЕ ранг (TIER_SUSPECT), но НЕ
# выброс из слота; выброс остаётся только за подтверждённым 401.
...
def select_accounts(
    ...
    for family, family_routes in routes_by_family.items():
        ...
        empty_reason_budget = len(dead_by_probe)
...
def render_report(
    ...
    if no_probe:
        ...
        order = ["жива", "квота", "ключ неверен", "доступ запрещён (403)", "сеть недоступна", "неизвестно"]
        ...
        if slot.account is None:
            ...
            if name in existing_secrets:
                reason = (
                    ...
                    f"ключом. Газ: `gh secret delete {name} --repo <owner/repo>` вручную "
