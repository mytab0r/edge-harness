...
`dsh_worker_run_is_success` (`scripts/lib/dsh-ci.sh:1212`) сравнивает ДВА
ЛОКАЛЬНЫХ снимка `git rev-parse HEAD` одноразового чекаута воркера в
пределах ОДНОГО job'а (`WORKER_BRANCH_START_SHA`/`WORKER_BRANCH_END_SHA`,
`scripts/worker/task.sh:467`/`:563`) — не `git ls-remote` (уточнено после
находки ревью PR #944, ADR 0017 несёт полную формулировку), сигнал выбран
именно потому, что не зависит от того, успел ли push дойти до origin, но
работа воркера иногда всё же пушит напрямую (обходя песочницу отдельным
клоном), и мгновенное удаление ветки в момент слияния рвало бы эту ветку
из-под ещё выполняющегося job'а. Включать флаг на репозитории нельзя
...
может тронуть ветку, любой job воркера, чья работа опирается на
существование этой ветки на origin, уже десятки раз успел завершиться —
отложенное удаление физически не может совпасть по времени с ещё
выполняющимся job'ом. Не тот же час, что
