...
#
# Зависание САМОГО worker.yml не остаётся непрочитанным: рипер оставляет
# комментарий на арендованной задаче («♻️ Прогон worker.yml (run …) завис …
# — оркестратор счёл его зависшим, отменил …») и строку «🧟 worker run …
# завис» в отчёте пульса (scheduler.py::reap_stalled_worker_run) — эту строку
# читает stall_detector (#201) как обычный отпечаток и эскалирует при
# повторении, отдельным от failure_watch каналом. Оба канала предметно РАЗНЫЕ
# (failure_watch разбирает ЛОГ джоба и классифицирует причину, stall_detector
# видит только текст строки отчёта), но ни один зависший worker.yml не
# проходит незамеченным ни при одном исходе.
#
