...
# маркер несёт пару «задача + PR» (эпизод), issue_marker_times сверяется
# ПЕРЕД повторной посылкой. Задача одна на все попытки, но эпизодов у неё
# может быть несколько: ручной выход ([needs-spec: снято]) и повторное
# исчерпание бюджета на НОВОМ PR — новый эпизод, владелец получает свежую
# эскалацию, а не молчаливое «уже эскалировано ранее» (находка AI-ревью
# PR #408, head cf79dc9; пару составляет scheduler.route_to_needs_spec).
