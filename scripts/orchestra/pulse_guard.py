...
WATCHED_WORKFLOWS = (
    ...
    "plugin-forge.yml",
    "clock-shift-tests.yml",  # #649: единственный потребитель красного класса «тест зависит от настенных часов»
)
...
def failure_watch(repo: str, now: datetime) -> tuple[list[str], list[str]]:
    ...
    for workflow in WATCHED_WORKFLOWS:
        ...
        # Свежий зелёный вытесняет старый красный (находка ревью PR #667,
        # раунд 4): если ПОСЛЕДНИЙ завершённый исход workflow — успех, значит
        # последний известный код уже зелёный, и сигнал по предыдущему красному
        # был бы тревогой о починенном. Для суточного продюсера его длинное
        # окно (см. переопределения ниже) без этого держало бы старый красный
        # «действующим» до суток после фикса. Не-успешный последний исход
        # (cancelled и пр.) семантику не меняет — разбор идёт по последнему
        # провалу, как раньше.
