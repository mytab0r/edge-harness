...
# ── Немедленная починка дефектов CI (#552) ───────────────────────────────────
# failure_watch выше только заводит задачу — чинит её человек вручную, из
# общей очереди пула (~130 задач). Владелец («мониторинг ошибок, который
# нихуё не исправляет?») — задача заведена, конвейер сломан, починка ждёт
# своей очереди наравне со всем остальным. dispatch_defect_rework
# (scheduler.py) адресует worker.yml на неё немедленно — тот же путь, что
# уже доводит открытые PR (#245/#394) и авто-расшивает конфликты
# (dispatch_conflict_rework, #474): один лифтайм-бюджет попыток на отпечаток
# класса (тот же приём, что CONFLICT_REWORK_MAX_ATTEMPTS), не сошлось —
# эскалация тем же каналом (#120 + Telegram), не бесконечный повтор.
DEFECT_DISPATCH_MAX_ATTEMPTS = 1

# Маркер-счётчик попыток — комментарий на самой заведённой задаче (issues/{n}/
# comments, задача — issue, тот же endpoint, что PR у CONFLICT_REWORK_MARKER).
# Отпечаток класса дописывается в текст (по образцу READY_STALL_MARKER/
# CONFLICT_ESCALATION_MARKER): один маркер — один сигнал на класс, а не на
# каждую задачу с одинаковым текстом маркера.
DEFECT_DISPATCH_MARKER = "[ci-failure: авто-починка]"

# Эскалация после исчерпания бюджета — общий канал #120/Telegram (escalate),
# идемпотентна по отпечатку класса (в тексте маркера), как и у конфликтов.
DEFECT_ESCALATION_MARKER = "[ci-failure: эскалация]"
