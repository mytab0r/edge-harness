...
def escalation_dedup_carrier_failed(result: str) -> bool:
    """«Сигнал ушёл, а носитель дедупа нет»: Telegram доставлен, но след в
    задаче НЕ оставлен — открывающий маркер эпизода не записан, следующий
    тик сочтёт эпизод новым и повторит страницу. Пара к
    escalation_channel_failed выше: тот ловит «оба канала молчат» (сигнала
    нет вовсе), этот — «сигнал есть, дедупа дальше не будет». Третья
    комбинация, «Telegram НЕ доставлен, след оставлен», отказом не считается:
    маркер эпизода записан (повторной страницы не будет), а мгновенный пуш
    продублирован постоянным следом в задаче. Единственное место разбора
    строки escalate() — литералы рождаются соседним return'ом, вторая копия
    разбора погасла бы молча при смене формата (found: ревью PR #607,
    head 5824e75 — гейт сторожа квот выбрасывал вердикт доставки)."""
