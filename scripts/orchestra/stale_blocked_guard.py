...
# Привязка к НАЧАЛУ строки (^ + MULTILINE) — находка ревью PR #336: без неё
# регэксп матчил маркер в любом месте прозы (цитата старого маркера в новой
# реплике, «задача заблокирована: #N» внутри предложения) и превращал
# гвардию в cry-wolf — собственный эскалационный комментарий гвардии
# цитирует маркер, что при широком матче стало бы вечным ложным срабатыванием.
STALE_MARKER_RE = re.compile(r"^\s*Блокирован[а-я]*\s*:\s*#(\d+)", re.IGNORECASE | re.MULTILINE)
