...
в Telegram, ответ записан в репозиторий»). Интерактивные кнопки требуют
Telegram-вебхука с авторизацией `callback_query`, которого в репозитории нет
(`cf-worker/src/harness.ts` прямо называет это отдельным нерешённым вопросом —
инбокс сейчас админско-релейный, не прямой вебхук). Эта гвардия реализует
достижимый слайс без кнопок: #254 сам называет комментарий-ответ полноценной
заменой нажатию (п.4 предложения) — «тот же результат достигается
комментарием в задаче руками».
...
def waiting_owner_check(repo: str, now: datetime) -> list[str]:
    ...
    for issue in labeled:
        ...
        if escalation_pending(marker_times, now):
            ...
            delivered = escalate(repo, issue["number"], text)
