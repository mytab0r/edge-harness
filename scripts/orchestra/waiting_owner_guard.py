...
# Разбор ОДНОЙ строки варианта на подпись кнопки (#254, PR #486): фраза БЕЗ
# номера и БЕЗ последствия — build_decision_keyboard сама нумерует по порядку
# (callback_data), последствие остаётся в тексте алерта/теле задачи, не
# дублируется на кнопке (Telegram — узкий экран). Не второй формат, тот же
# VARIANT_LINE_RE, только с захватывающей группой на первую часть.
...
def variant_option_labels(body: str) -> list[str]:
    ...
    та же последовательность строк, что variant_lines(), разобранная дальше
    по формату «N. фраза — последствие» — только фраза, порядок сохранён
    (build_decision_keyboard нумерует callback_data позицией в списке, она
    обязана совпасть с номером, который владелец видит в тексте варианта и
    называет в ответе «РЕШЕНИЕ: N» — оба места читают один и тот же
    письменный порядок). Пустой список — ровно то же условие, что у
    variant_lines(): блока нет, или ни одна строка не разобралась (строка не
    матчит VARIANT_LABEL_RE — практически невозможно, раз она уже прошла более
    узкий VARIANT_LINE_RE, но чистая функция не должна падать на этом крае)."""
    labels = []
    for line in variant_lines(body):
        match = VARIANT_LABEL_RE.match(line)
        if match:
