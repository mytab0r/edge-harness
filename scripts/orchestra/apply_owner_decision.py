...
def _label_state_line(reason: str) -> str:
    """Состояние задачи зависит от ПРИЧИНЫ отказа, и врать здесь нельзя
    (находка ai-ревью PR #1401): при `not_waiting` метки как раз НЕТ — она
    снята или задача закрыта, это и есть причина отказа. Единый текст «задача
    остаётся с меткой» был бы ложным ровно в том случае, ради которого
    проверка `issue_still_waiting` и написана."""
    ...
    return (f"Задача остаётся с меткой `{WAITING_OWNER_LABEL}` — решение по ней не записано.")
