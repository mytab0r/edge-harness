...
def load_conditional_steps(workflow_path: Path) -> dict[str, set[str]]:
    ...
    иначе БЕЗУСЛОВНЫЙ шаг, пропущенный лишь потому, что job оборвался РАНЬШЕ
    (не из-за своего `if:`, а из-за отказа/отмены более раннего шага), ложно
    считается «условным» — живой замер 2026-09-13 (до этого фильтра): «Run
    actions/checkout@v7» (ai-review.yml) — success 178 раз в окне и БЕЗ
    единого `skipped`-исхода в реальности (шаг вообще не несёт `if:`), но при
    падении job'а НА ДРУГОМ шаге все ПОСЛЕДУЮЩИЕ автоматически получают
    `skipped` от самого раннера — тот же признак, что и у настоящего
    условного шага, без разбора source отличить нельзя.
