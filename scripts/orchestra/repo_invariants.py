...
(см. блок-комментарий у бывшего check_free_task_count_mismatch):
  ...
  17. check_frontend_deploy_stale (issue #1041, живой случай 2026-09-12/13):
      ...
      ЗЕЛЁНОГО прогона КАЖДОГО workflow из FRONTEND_DEPLOY_WORKFLOWS с текущим
      main ПО ПУТЯМ,
      ...
      «Алерт не гадает» — дрейф вне них деплою безразличен). Обобщение #1419
      (2026-09-13..21, второй живой случай ТОГО ЖЕ класса в другом workflow):
      deploy-worker.yml падал на каждом прогоне девять суток (устаревший
      worker-configuration.d.ts), а инвариант знал только deploy-dsh-edge.yml —
      с тех пор проверяется каждый деплойный workflow из
      FRONTEND_DEPLOY_WORKFLOWS, отчёт и эскалация несут имя workflow, ключ
      дедупа не даёт здоровому деплою заглушить алерт больного. #1421 закрыл
      тот же класс на уровень выше: список рукописный, и третий деплойный
      workflow стал бы невидим ровно тем же способом, каким был невидим
      второй. Полноту списка держит гвардия
      scripts/lib/deploy_workflow_registry_guard.py — признак «этот workflow
      деплоит» читается из самих файлов .github/workflows (шаг с wrangler), и
      деплой, не попавший ни в FRONTEND_DEPLOY_WORKFLOWS, ни в
      UNWATCHED_DEPLOY_WORKFLOWS (исключение с НАЗВАННОЙ причиной), красит CI
      в тот же день, а не через девять суток по красной вкладке Actions. Незавершённый
  ...
  18. check_pipeline_status_marker_impersonation (#1101, найдено при доводке
      ...
      веской причине: маркер семейства «статус конвейера», опубликованный
      не токеном job'а, — ФАКТ постороннего писателя в канале. До #1242
      подделка ещё и двигала решение (conveyor_gate читал маркеры #120
      без trusted_login, поддельный RESUME работал виртуальным success);
      после закрытия того PR чтение идёт с require_job_token=True и
      решение подделка больше не меняет — детектится сам факт подделки
      (см. блок-комментарий у ESCALATING_INVARIANTS ниже).
      escalate_if_new дедуплицирует по множеству id нарушителей —
...
def check_wip_gate_false_zero(now: datetime, wip_markers: list[tuple[datetime, str]],
    ...
    Три булевых поля результата (`claimed_closes_gate`/`actual_closes_gate`/
    `literal_false_zero`) — качественная сигнатура ТИПА расхождения, не
    просто побочный продукт вычисления: `run_escalations`/`escalation_state_key`
    (issue #1261) строят по ним дедуп-ключ эскалации, потому что сырые
    `claimed_count`/`actual_count` дрейфуют почти на каждый вызов вместе с
    обычной активностью репозитория, а тип расхождения — нет. Одно место
    правды для этой математики, не пересчёт в дублирующей форме на стороне
    эскалации."""
    if not wip_markers:
        return []
    ...
    return [{
        ...
        "claimed_closes_gate": claimed_closes_gate,
        "actual_closes_gate": actual_closes_gate,
        "literal_false_zero": literal_false_zero,
    }]
...
# последний ЗЕЛЁНЫЙ прогон КАЖДОГО деплойного workflow (FRONTEND_DEPLOY_
# WORKFLOWS) стоит на коммите X, main
...
# Оба деплойных workflow морды (#1419, обобщение #1041): deploy-dsh-edge.yml
# собирает морду из апстрима dsh-edge, deploy-worker.yml деплоит cf-worker —
# инцидент #1419 показал, что красный деплой ВТОРОГО из них девять суток не
# виден никому, потому что инвариант 17 знал только первый. Список, не
# скаляр: третий деплойный workflow добавляется одной строкой здесь.
FRONTEND_DEPLOY_WORKFLOWS = ("deploy-dsh-edge.yml", "deploy-worker.yml")
# Деплойные workflow, СОЗНАТЕЛЬНО оставленные без наблюдения инварианта 17:
# имя файла → причина. Пусто — и это факт, а не забывчивость: на 2026-09-22
# деплоят ровно два workflow выше, оба под наблюдением.
#
# Список существует, потому что «не наблюдаем» обязано быть выбором с
# названной причиной, а не умолчанием (AGENTS.md, «Тормоз без газа не
# принимается»): без него единственный способ убрать workflow из-под
# инварианта — стереть строку выше, и следующий читатель не отличит решение
# от опечатки. Полноту ОБОИХ списков держит гвардия
# scripts/lib/deploy_workflow_registry_guard.py (#1421): деплойный workflow,
# не попавший ни в один из них, красит CI — список больше не может молча
# разойтись с тем, что в репозитории реально деплоит.
UNWATCHED_DEPLOY_WORKFLOWS: dict[str, str] = {}
...
def _frontend_deploy_watched_paths(
    ...
    """`on.push.paths` из самого файла названного деплойного workflow — не
    задублированы литералом здесь (AGENTS.md, «Одно место правды»): список
    путей, реально триггерящих деплой, мог измениться в workflow без синхронной
    правки этого файла, и тогда сравнение сверяло бы не то, что деплой
    действительно слушает. `workflow` — имя файла в .github/workflows/
    (элемент FRONTEND_DEPLOY_WORKFLOWS): каждый деплой читает СВОИ пути,
    один workflow не наследует пути чужого.
    ...
    *.cjs` молча матчилась бы как «не покрыто» — реальный дрейф по такому
    ...
    try:
        ...
        raise RuntimeError(f"PyYAML недоступна: {error} — инвариант 17 не может прочитать {workflow}") from error
    path = workflow_path or (REPO_ROOT / ".github" / "workflows" / workflow)
    if not path.exists():
        raise RuntimeError(
...
def decide_frontend_deploy_stale(
    ...
    """Чистая функция (без сети). `runs` — прогоны ОДНОГО деплойного workflow
    из FRONTEND_DEPLOY_WORKFLOWS,
...
def _compare_changed_paths(repo: str, base_sha: str, main_sha: str) -> list[str]:
    """Файлы между последним зелёным деплоем и текущим main. Единое место
    для обоих деплойных workflow (compare между одной парой sha в прогоне
    делается ОДИН раз, кэш у вызывающего — check_frontend_deploy_stale).
    ...
    живым прогоном ответа с >300 файлами не подтверждено). Ровно потолочное
    число файлов = сравнение неполно: окно между последним зелёным деплоем
    и текущим main копит ВСЕ файлы (включая docs/), а деплойные пути
    сортируются после — обрезка выбрасывает именно их, и инвариант напечатал
    бы 💚 при реальном дрейфе. RuntimeError → честное «недоступна»,
    НЕ «здорово»."""
    compare = gh(f"repos/{repo}/compare/{base_sha}...{main_sha}")
...
def check_frontend_deploy_stale(repo: str) -> dict:
    """IO-обвязка инварианта 17: для КАЖДОГО деплойного workflow из
    FRONTEND_DEPLOY_WORKFLOWS — последний ЗЕЛЁНЫЙ прогон против текущего main
    + on.push.paths из файла того же workflow (одно место правды, см.
    _frontend_deploy_watched_paths; #1419: инвариант, знавший только
    deploy-dsh-edge.yml, девять суток не видел красного deploy-worker.yml).
    Возвращает dict {workflow: decide-словарь ЦЕЛИКОМ + поле "workflow"} (все
    пять status, не только нарушения) — build_report обязан различить «морда
    свежая», «деплой main в полёте — не проверено» и нарушение по каждому
    деплою отдельно. Сеть недоступна/квота/неполное сравнение — RuntimeError
    наверх: build_report обязан отличить «не проверено» от «здорово» (тот же
    приём, что инварианты 12/15)."""
    ...
    # Кэш compare: у обоих деплоев последний зелёный нередко стоит на одном и
    # том же коммите main — второму compare/ не нужен, ответ идентичен.
    compares: dict[tuple[str, str], list[str]] = {}
    results: dict[str, dict] = {}
    for workflow in FRONTEND_DEPLOY_WORKFLOWS:
        runs = pulse_guard.recent_runs(repo, workflow, per_page=FRONTEND_DEPLOY_RUNS_TO_SCAN)
        runs = sorted(runs, key=lambda r: r.get("created_at") or "", reverse=True)
        ...
        if last_good is not None and last_good.get("head_sha") != main_sha:
            watched_paths = _frontend_deploy_watched_paths(workflow)
            pair = (last_good["head_sha"], main_sha)
            if pair not in compares:
                compares[pair] = _compare_changed_paths(repo, *pair)
            changed_paths = compares[pair]

        results[workflow] = {
            **decide_frontend_deploy_stale(runs, main_sha, changed_paths, watched_paths),
            "workflow": workflow,
        }
    return results
...
def build_report(repo: str, now: datetime,
    ...
    try:
        v17_all = check_frontend_deploy_stale(repo)
    except RuntimeError as error:
        findings[17] = []
        lines.append(f"🚨 [17] проверка свежести деплоев морды недоступна: {error} — "
                      "инвариант пропущен на этом прогоне (это НЕ «морда свежая»)")
    else:
        # Обобщение #1419: отчёт печатается ПО КАЖДОМУ деплойному workflow —
        # красный deploy-worker.yml обязан называться так же громко, как
        # красный deploy-dsh-edge.yml, а не тонуть в общем 💚 другого деплоя.
        findings[17] = []
        ...
        for v17 in v17_all.values():
            workflow17 = v17["workflow"]
            status17 = v17.get("status")
            if status17 == "stale":
                findings[17].append(v17)
                ...
                lines.append(
                    f"🚨 [17] морда устарела относительно main ({workflow17}, "
                    f"#1041/#1419): последний зелёный — {v17['last_deployed_sha'][:8]} "
                    ...
                    f"следующим зелёным прогоном {workflow17} на текущем (или более новом) main"
                )
            elif status17 in ("unconfirmed", "never-succeeded"):
                findings[17].append(v17)
                if v17.get("latest_conclusion"):
                    lines.append(
                        f"🚨 [17] {workflow17}: ни одного успешного прогона в "
                    ...
                    lines.append(
                        f"🚨 [17] {workflow17}: завершённых прогонов в последних "
                ...
                lines.append(
                    f"⚠️ [17] {workflow17}: деплой текущего main ещё в полёте "
                    f"({v17.get('latest_run_url')}) — свежесть на этом пульсе НЕ проверена; "
                    "следующий пульс пересчитает"
                )
            else:
                lines.append(f"💚 [17] {workflow17}: живая морда стоит на текущем main "
                              f"(или main не менял пути этого деплоя)")
...
# ══════════════════════════════════════════════════════════════════════════
# Дедуп «состояние + окно» (issue #1261, живой случай 2026-09-13/14):
# escalate_if_new дедуплицирует по СТРОКЕ ключа — ключ, несущий волатильный
# компонент (время публикации чужого маркера, живой пересчёт, который
# дрейфует вместе с обычной активностью репозитория), меняется почти на
# каждый вызов, и правило «одна эскалация на состояние» вырождается в
# «эскалация почти на каждый такт»: 36 эскалаций инварианта 16 за ~30 часов
# при НЕИЗМЕННОМ качественном состоянии (claimed=0 во всех 36 наблюдениях,
# actual дрейфовал 26..35) — один непрерывный инцидент, не 36 разных
# (фикстура scripts/orchestra/testdata/issue120_invariant16_escalations_2026-09-13.json,
# `gh api` дословно).
#
# Просто выкинуть волатильный компонент нельзя (AGENTS.md «алерт не
# гадает» встречается здесь с «сигнал должен повториться, а не молчать» —
# два требования тянут в разные стороны): без НИКАКОГО времени в ключе два
# РАЗНЫХ, разнесённых по времени эпизода с совпавшими числами схлопнутся в
# один навсегда, и рецидив через сутки/неделю останется немым (второй
# эпизод найдёт уже использованный ключ в issue_marker_times и не
# эскалирует вовсе). Компромисс — качественная сигнатура состояния
# (не сырые числа, которые дрейфуют без изменения диагноза) + грубое окно
# времени: тот же непрерывный инцидент внутри окна даёт одну эскалацию,
# истечение окна — новое напоминание, ЕСЛИ инцидент не разрешился (газ —
# само наступление нового окна, ручного снятия не требует). Число подобрано
# по сохранённой истории (см. фикстуру выше): 36 тревог схлопываются в 6 при
# окне 6 часов — единицы, не десятки, и не одна немая запись на весь
# инцидент (что дало бы окно длиной в бесконечность).
#
# Слепой коридор окна (назван честно, «не знаешь — пиши не подтверждено»):
# инцидент, который РЕАЛЬНО разрешился и вернулся внутри того же окна,
# эскалации не даст — по одним только данным ключа непрерывный инцидент и
# такой внутриоконный рецидив неотличимы (различение потребовало бы
# запоминать «последний разрешённый» момент состояния вне канала #120).
# Слепота ограничена длиной окна: за его истечением рецидив эскалирует
# штатно, а ход инцидента внутри окна виден в отчёте каждого такта
# (строки [16] build_report), пусть и без повторного канала владельца.
ESCALATION_REMINDER_WINDOW_MINUTES = 360


def wip_gate_state_signature(item: dict) -> str:
    """Качественная сигнатура «тем же состоянием» для дедупа эскалаций
    инварианта 16 (issue #1261): строка собирается ТОЛЬКО здесь, из булевых
    полей находки check_wip_gate_false_zero (которые он же и посчитал —
    одно место правды для этой математики), не из сырых счётчиков — те
    дрейфуют почти на каждый вызов вместе с обычной активностью репозитория,
    не меняя диагноза. Зовётся из run_escalations и из тестов: вторая
    ручная сборка этой f-строки — тот же класс «два места правды», что и
    дублирование самой булевой математики (находки ревью PR #1263)."""
    return (f"{item['claimed_closes_gate']}:{item['actual_closes_gate']}:"
            f"{item['literal_false_zero']}")


def escalation_state_key(moment: datetime, state_signature: str,
                          window_minutes: int = ESCALATION_REMINDER_WINDOW_MINUTES) -> str:
    """Дедуп-ключ «состояние + окно времени», не «состояние + точный момент
    наблюдения» (см. блок-комментарий выше, issue #1261). `state_signature`
    — качественное описание инцидента, УЖЕ очищенное от волатильных сырых
    чисел вызывающей стороной (`check_*` определяет, что для его инварианта
    считается «тем же состоянием»). `moment` огрубляется до абсолютного
    окна `window_minutes` от эпохи — не «день месяца» (иначе один и тот же
    час на разных, далеко разнесённых по времени датах схлопнулся бы
    случайно), поэтому окно уникально во всей истории репозитория."""
    bucket = int(moment.timestamp() // 60) // window_minutes
    return f"{bucket}:{state_signature}"
...
def pipeline_status_marker_key(violations: list[dict]) -> str:
    ...
    НЕ подведён под общий приём ESCALATION_REMINDER_WINDOW_MINUTES/
    escalation_state_key (issue #1261, разбор рядом с фиксом инварианта 16):
    18 эскалаций за 1.5 суток — та же ЦИФРА шума, что у бывшего дефекта
    инварианта 16, но другая причина. У 16 сырые числа в ключе дрейфовали
    почти на каждый вызов БЕЗ изменения диагностируемого состояния (claimed
    оставался 0 во всех 36 наблюдениях). У 18 множество violator-id ПРОВЕРЕНО
    росло на каждой из 18 эскалаций (193→194→199→…→232, история #120,
    2026-09-13/14) — каждая эскалация несла РЕАЛЬНО новую подделку, не
    пересчёт того же самого: новый id в множестве — новый ФАКТ (посторонний
    писатель оставил ещё один маркер в канале), а не новое число про то же
    состояние; окно схлопнуло бы именно эти новые факты. Подделка при этом
    решение конвейера уже не двигает: conveyor_gate читает PAUSE/RESUME
    только от токена job'а (require_job_token=True, #1242 — см.
    блок-комментарий у ESCALATING_INVARIANTS), инвариант 18 — детектор
    самого факта подделки, не влияния на решение. Если частота всё равно
    мешает, это отдельное, осознанное решение (например: троттлинг эскалации
    при сохранении полного списка id в отчёте), не побочный эффект фикса
    16."""
...
def run_escalations(repo: str, findings: dict[int, list]) -> list[str]:
    ...
    if findings.get(16):
        item = findings[16][0]
        # Сигнатура состояния — качественный тип расхождения (одно место
        # правды: wip_gate_state_signature над полями, посчитанными
        # check_wip_gate_false_zero), не сырые claimed_count/actual_count:
        # они дрейфуют почти на каждый вызов вместе с обычной активностью
        # репозитория, хотя диагноз («claimed врёт про backlog») не
        # меняется (issue #1261, см. блок-комментарий
        # у ESCALATION_REMINDER_WINDOW_MINUTES).
        key = escalation_state_key(
            datetime.fromisoformat(item["marker_at"]),
            wip_gate_state_signature(item))
        text = (
    ...
    for item in findings.get(17, []):
        # Обобщение #1419: нарушений может быть до одного на КАЖДЫЙ деплойный
        # workflow — эскалируется каждый, ключ несёт имя workflow, чтобы дедуп
        # одного деплоя не глушил алерт другого.
        workflow17 = item.get("workflow", "неизвестный workflow")
        ...
        if item["last_deployed_sha"] is None:
            # Волатильный компонент в ключе (latest_run_url меняется каждым
            # новым неуспешным прогоном при неизменном диагнозе «успешного
            # прогона нет») — по форме тот же класс, что у 16 (#1261), но
            # СОЗНАННО не подведён под окно: каждая эскалация несёт новый
            # ФАКТ (новый упавший прогон — новое реальное событие деплоя),
            # как у 18 (вердикт там закреплён докстрингом
            # pipeline_status_marker_key, здесь — этим комментарием).
            key = f"no-success:{workflow17}:{item['latest_run_url']}"
            ...
            text = (
                f"🚨 edge-harness: инвариант 17 ({workflow17}, морда, #1041/#1419) — ни одного "
                f"успешного прогона в последних "
            ...
            key = f"{workflow17}:{item['last_deployed_sha']}:{item['main_sha']}"
            if item.get("main_deploy_state") == "failed":
                action17 = (
                    f"Деплой текущего main УПАЛ ({item.get('main_run_url')}) — "
                    f"проверь причину падения (Actions → {workflow17})."
                )
            else:
                action17 = (
                    ...
                    f"{workflow17} вручную (workflow_dispatch)."
                )
            text = (
                "🚨 edge-harness: инвариант 17 (морда устарела относительно main, "
                f"{workflow17}, #1041/#1419) — последний зелёный стоит на "
        ...
        if result:
            lines.append(f"📣 инвариант 17 эскалирован ({workflow17}): {result}")
    if findings.get(18):
        v18 = findings[18]
        ...
        text = (
            ...
            "«Атрибуция событий»). Решение гейта подделка не двигает: "
            "conveyor_gate читает PAUSE/RESUME только от токена job'а "
            "(require_job_token=True, #1242) — маркер вне токена это ФАКТ "
            "постороннего писателя в канале, его владелец обязан видеть. "
