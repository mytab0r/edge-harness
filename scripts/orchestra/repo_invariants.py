...
def stuck_gate_fact_line(item: dict) -> str:
    ...
    ближайшем тике» здесь было бы неверно (тик видит занятость и делает
    continue, не трогая бюджет), поэтому этот случай — отдельная ветка, а не
    молчаливое совпадение с «не исчерпан»."""
    ...
    if total < limit and held_back_by_run is not None:
        budget = (
