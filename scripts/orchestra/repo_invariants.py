...
# (фикстура scripts/orchestra/testdata/issue120_invariant16_2026-09-13.json,
# `gh api` дословно).
#
...
def pipeline_status_marker_key(violations: list[dict]) -> str:
    ...
    пересчёт того же самого. Схлопнуть их окном значило бы проглотить
    именно те новые PAUSE/RESUME-подделки, которые (см. докстринг выше)
    прямо сейчас двигают решение conveyor_gate — не тот рецидив, который
    можно позволить себе не заметить. Если частота всё равно мешает, это
    отдельное, осознанное решение (например: троттлинг эскалации при
    сохранении полного списка id в отчёте), не побочный эффект фикса 16."""
...
def run_escalations(repo: str, findings: dict[int, list]) -> list[str]:
    ...
    if findings.get(16):
        item = findings[16][0]
        # Сигнатура состояния — качественный тип расхождения (уже посчитан
        # в check_wip_gate_false_zero, одно место правды), не сырые
        # claimed_count/actual_count: они дрейфуют почти на каждый вызов
        # вместе с обычной активностью репозитория, хотя диагноз («claimed
        # врёт про backlog») не меняется (issue #1261, см. блок-комментарий
        # у ESCALATION_REMINDER_WINDOW_MINUTES).
        state_signature = (
            f"{item['claimed_closes_gate']}:{item['actual_closes_gate']}:"
            f"{item['literal_false_zero']}"
        )
        key = escalation_state_key(datetime.fromisoformat(item["marker_at"]), state_signature)
        text = (
