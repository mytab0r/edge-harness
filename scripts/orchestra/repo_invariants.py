...
#
# Порог и его арифметика (находка ai-review PR #1247: прежний комментарий
# складывал те же слагаемые в «≈152 мин» — арифметически неверно). Худший
# ЛЕГИТИМНЫЙ прогон под фиксом #1160 (DSH_CHAIN_TOTAL_BUDGET_SECS,
# scripts/lib/dsh-ci.sh, тот же PR): попытка пула ДО цепочки (до
# DSH_TIMEOUT_SECS=7200с — wall пула в бюджет цепочки НЕ входит, что сам
# dsh-ci.sh оговаривает честной оговоркой у chain_start) + суммарный бюджет
# цепочки (9000с + до 300с ожиданий RATE_LIMIT последней итерации — они идут
# после среза таймаута) + оверхед установки/git/отчёта (≤2 мин по замеру,
# комментарий у WORKER_STALL_MINUTES, scheduler.py) ≈ 16320с + 300с ≈
# 277 мин. WORKER_RUN_LONG_RUNNING_MINUTES=200 — сознательно ВНУТРИ этого
# легитимного коридора, и это объявленное решение, а не ошибка счёта:
# инвариант наблюдательный (ничего не отменяет, не закрывает и не снимает —
# read-only строка отчёта, действие остаётся у рипера), его работа — давать
# долгоживущему in_progress-прогону ВИДИМОСТЬ на каждом пульсе (15 мин) и
# загораться РАНЬШЕ рипера (WORKER_STALL_MINUTES=295) при любом классе
# зависания, включая те, что фикс #1160 не покрывает (зависание попытки
# пула, зависание вне dsh вовсе). Цена ложной строки в редком легитимном
# хвосте (пул завис на полные 7200с, история таких прогонов не подтверждена)
# — одна строка отчёта, которую читатель сверяет с фактом прогона; цена
