...
(см. блок-комментарий у бывшего check_free_task_count_mismatch):
  ...
  14. check_worker_false_success_comment (#876) — комментарий воркера несёт
      ...
      находкой. Наблюдательный, не в CI_GATING: Search API сам по себе может
      быть недоступен best-effort, гейтить обязательную проверку
      доступностью стороннего API было бы новым тормозом без содержательного
      смысла.
