...
заново. Инварианты 1, 2, 5 остаются наблюдательными, пока не разгребён долг:
2 обнулит PR #247 (`free_task`: скан по прозе вместо объявления), 1 обнулит
стадия приёмки, 5 — дедупликация по отпечатку (#201/#243). Каждый включается
отдельным шагом, привязанным к обнулению его счётчика, правкой CI_GATING
ниже — не разовым решением «включим всё позже».
...
# (PR #249, 2026-09-03): 3 и 4 — ноль нарушений на момент замера, включены
# сразу, чтобы не дать долгу появиться заново; 1/2/5 — наблюдательные, пока
# не обнулены (см. docstring выше). Включение нового номера — явная правка
# этой константы владельцем/оркестратором после проверки, что счётчик
# инварианта обнулился на живом репозитории.
CI_GATING: frozenset[int] = frozenset({3, 4})
