...
(см. блок-комментарий у бывшего check_free_task_count_mismatch):
  ...
  22. check_ai_rework_never_dispatched (issue #1253): PR несёт
      ...
      PR #1253 (число и датировка находок в описании PR), порог обоснован
      той же датой, не вкусом.
...
def check_ai_rework_never_dispatched(repo: str, now: datetime, open_pulls: list[dict]) -> list[dict]:
    ...
    «Доводка звалась хоть раз, но не помогла» — ДРУГОЕ состояние, НЕ
    находка этого инварианта: за него уже отвечает бюджет
    dispatch_ai_review_rework (AI_REWORK_MAX_ATTEMPTS попыток на отпечаток
    диффа + законная эскалация владельцу, AI_REWORK_ESCALATION_MARKER) —
    второй тормоз здесь не заводим, инвариант проверяет ровно то, что этот
    бюджет структурно не видит: PR, которому адресный диспатч не звонил
    НИКОГДА, поэтому и бюджет по нему ещё не открывался."""
    violations = []
    for pull in open_pulls:
        labels = {label["name"] for label in pull["labels"]}
        ...
        ever_dispatched = any(
            scheduler.AI_REWORK_MARKER in (comment.get("body") or "") for comment in comments
        )
        if ever_dispatched:
