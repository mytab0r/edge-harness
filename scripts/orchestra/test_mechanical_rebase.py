...
def stalled_minutes_ago() -> float:
    """Возраст, заведомо превосходящий порог зависания, ОТ САМОЙ КОНСТАНТЫ
    (`sch.WORKER_STALL_MINUTES`), а не магическим числом рядом: подъём порога
    не переворачивает тест молча (находка ai-review PR #1033). Запас в 10%
    и минимум 30 минут — заведомо больше любой длительности теста."""
    return sch.WORKER_STALL_MINUTES * 1.1 + 30
...
def worker_run_payload(run_id: int, status: str, *, started_minutes_ago: float) -> dict:
    ...
    (`sch.WORKER_STALL_MINUTES`, сегодня 295, и
    `sch.WORKER_CLAIM_TRACE_GRACE_MINUTES`, 10) на порядки больше
    длительности теста, зафиксированная дата фикстуры здесь не «стареет»
    (тот же приём, что докстринг sch._run_is_stalled запрещает для
    ДЛИТЕЛЬНЫХ горизонтов). Числа порогов в тестах НЕ дублируются literal'ом
    (находка ai-review PR #1033: докстринг называл 255 при фактических 295,
    и из-за неверного числа не было видно, что «зависший» случай сидел в
    пяти минутах над порогом — бамп константы молча перевернул бы тест).
    Возраст заведомо зависшего прогона берётся от самой константы —
    `stalled_minutes_ago()`."""
    started = (
...
def test_process_pull_defers_when_a_worker_run_is_still_queued(tmp_path, monkeypatch):
    ...
    assert outcome == "worker-running: queued-run"
    assert branch_tip(origin, "agent/601-drifted-a") == original_tip  # head не тронут


def test_process_pull_proceeds_when_queued_run_is_stalled(tmp_path, monkeypatch):
    """Газ queued-блока гейта (находка ai-review PR #1033, круг 3): queued
    старше WORKER_STALL_MINUTES (возраст по created_at) — зависший, из
    sch.active_worker_runs исключён, гейт ПРОПУСКАЕТ PR. Без этого queued
    держал бы механическую очередь бессрочно (застрявшая очередь раннеров) —
    тот же класс #815, что у зависшего in_progress.
    Мутационное доказательство: верни исключение stalled только для
    in_progress в sch.active_worker_runs — тест краснеет ("worker-running:
    queued-run" вместо "resolved"); свежий queued
    (test_process_pull_defers_when_a_worker_run_is_still_queued) остаётся
    зелёным."""
    origin = build_origin(tmp_path)
    work = clone_workdir(origin, tmp_path)
    p = pull(601, ref="agent/601-drifted-a")
    fake = FakeGh({
        "issues/601/timeline?per_page=100": [],
        "actions/workflows/worker.yml/runs?status=queued": {
            "workflow_runs": [worker_run_payload(778, "queued", started_minutes_ago=stalled_minutes_ago())]},
    })
    patch_gh(monkeypatch, fake)
...
def test_process_pull_defers_when_active_worker_claims_the_same_task(tmp_path, monkeypatch):
    ...
    fake = FakeGh({
        "issues/601/timeline?per_page=100": [],
        # max_pages=1 (граница цены #1100): 1 вызов метаданных + последняя
        # страница — след аренды АКТИВНОГО прогона всегда свежий.
        "issues/601": {"comments": 1},
        "issues/601/comments?per_page=100&page=1": [{"body": CLAIM_COMMENT_BODY}],
    ...
    assert outcome == "worker-running: claimed-this-task"
...
def test_process_pull_defers_when_in_progress_run_is_young_without_claim_trace(tmp_path, monkeypatch):
    ...
    fake = FakeGh({
        "issues/601/timeline?per_page=100": [],
        "issues/601": {"comments": 0},  # следа аренды ещё нет
        "actions/workflows/worker.yml/runs?status=in_progress": {
    ...
    assert outcome == "worker-running: young-no-trace"
    assert branch_tip(origin, "agent/601-drifted-a") == original_tip
...
def test_run_reports_worker_running_and_skips_push(tmp_path, monkeypatch):
    ...
    outcomes[601] == "worker-running: queued-run", доказывая, что гейт — не no-op."""
    origin = build_origin(tmp_path)
    work = clone_workdir(origin, tmp_path)
    p = pull(601, ref="agent/601-drifted-a")
    original_tip = branch_tip(origin, "agent/601-drifted-a")
    ...
    assert outcomes[601] == "worker-running: queued-run"
    assert branch_tip(origin, "agent/601-drifted-a") == original_tip
    # Отчёт называет ПРИЧИНУ фактом (AGENTS.md, «алерт не гадает»), а не
    # угадайку «воркер занят (либо queued, либо молод…)»: строка PR обязана
    # нести конкретный reason ключ исхода.
    assert any("#601" in line and "queued" in line for line in lines)
...
def test_main_returns_zero_on_all_worker_running_queue(monkeypatch):
    ...
    monkeypatch.setattr(
        mr, "run",
        lambda repo, repo_dir: (["…"], {601: "worker-running: queued-run",
                                        602: "worker-running: queued-run"}),
    )
...
def test_run_isolates_one_pr_exception_and_keeps_processing_the_queue(tmp_path, monkeypatch):
    """Класс находки ai-review PR #1033 (круг 5), не только её случай: до
    правки ЛЮБОЕ неожиданное исключение на ОДНОМ PR роняло весь проход —
    process_pull ловит только RuntimeError, а в run() per-PR изоляции не
    было вовсе. Цена: sch.summary не вызывался, отчёта не было, хвост очереди
    не обрабатывался, и крон падал на том же PR каждые 15 минут.

    Здесь первый PR очереди бросает ровно то исключение, с которого находка
    началась (UnicodeDecodeError — наследник ValueError, не RuntimeError), и
    проверяется ВИДИМЫЙ результат: второй PR всё равно обработан, отчёт
    построен, исход первого НАЗЫВАЕТ исключение, а не молчит."""
    origin = build_origin(tmp_path)
    work = clone_workdir(origin, tmp_path)
    first = pull(701, ref="agent/701-drifted-a")
    second = pull(702, ref="agent/702-drifted-b")
    fake = FakeGh({
        "pulls?state=open": [first, second],
        "issues/701/timeline?per_page=100": [],
        "issues/702/timeline?per_page=100": [],
    })
    patch_gh(monkeypatch, fake)

    real_process_pull = mr.process_pull

    def exploding_process_pull(repo, pull_obj, repo_dir):
        if pull_obj["number"] == 701:
            raise UnicodeDecodeError("utf-8", b"\xff", 0, 1, "invalid start byte")
        return real_process_pull(repo, pull_obj, repo_dir)

    monkeypatch.setattr(mr, "process_pull", exploding_process_pull)

    lines, outcomes = mr.run(REPO, work)

    # Исход назван фактом, не проглочен.
    assert outcomes[701].startswith("infra-error: неожиданное исключение")
    assert "UnicodeDecodeError" in outcomes[701]
    # Очередь не заклинена: второй PR обработан этим же проходом.
    assert 702 in outcomes
    assert outcomes[702] != outcomes[701]
    # Отчёт построен (до правки его не было вовсе — трейсбек до summary).
    assert lines and lines[0].startswith("Механический ребейз конфликтов:")
