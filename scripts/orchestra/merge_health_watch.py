...
def render_body(verdict: Verdict, suspects: list[Suspect], truncated: bool) -> str:
    ...
    return "\n".join([
        ...
        # create_pool_issue откажет ДО сетевого вызова. Детектор регрессии
        # зависимостей не знает — явное «ничем», не пропуск. Последней
        # непустой строкой — её читает ai_review.blocked_by_numbers.
        "БЛОКИРУЕТСЯ: ничем",
    ])
