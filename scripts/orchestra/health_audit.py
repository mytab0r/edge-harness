...
def render_body(fp: str, c: health_regression.Classification, snapshot_ref: str) -> str:
    ...
    # create_pool_issue откажет ДО сетевого вызова. Само-аудит зависимостей
    # не знает — явное «ничем», не пропуск. Последней непустой строкой —
    # её читает ai_review.blocked_by_numbers.
    lines += ["", "БЛОКИРУЕТСЯ: ничем"]
    return "\n".join(lines)
