...
def test_false_positive_prose_about_the_rule_passes_contract(monkeypatch):
    ...
    # оставляла оба контрактовых теста выше зелёными по случайности. Первая
    # версия этого теста ("Closes/Fixes/Resolves запрещены контрактом — не
    # пиши их.") вовсе не содержала "#" в строке — старая логика (двухполовинная:
    # сначала фильтр `"#" in line`, потом `startswith`) отсеивала её ДО
    # startswith и тоже давала зелёный код, так что тест ничего не различал.
    # Здесь #10 стоит в строке, но не вплотную к слову — подлинная старая
    # логика (startswith + "#" в строке) красит это как директиву, новая
    # (семантика GitHub, #N вплотную) — нет.
    body = (
        "#413\n\n"
        "Closes/Fixes/Resolves рядом с #10 запрещены контрактом — не пиши их.\n"
        "Задачу закрою отдельным комментарием после мержа, с уликами.\n"
    )
    routes = {
