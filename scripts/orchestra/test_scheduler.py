...
def test_after_merge_never_calls_create_pool_issue_for_review_findings(monkeypatch):
    ...
    #
    # Доказано исполнением (не по памяти, AGENTS.md «Рецепт мутации —
    # исполни, не вспоминай»): scheduler.py на ref (состояние ДО #1262) не
    # несёт review_findings вовсе — тест падает AttributeError на первой же
    # попытке построить маршруты реестра, до того как дело доходит до
    # заглушки create_pool_issue. Это даже более сильный красный, чем
    # регрессия вызова: старая версия физически не может пройти этот тест.
    #
    # MUTATION-PROOF
