...
Ограничитель разбирается как ПОЛНОЕ слово (с дефисом: `<<EOF-1` читает данные
до «EOF-1», «EOF» её не закрывает), а форма `<<`, которую сканер не разобрал
(`<<\СЛОВО`, кавычки с пробелом внутри), — громкий отказ «неопознанная форма»:
сканер, не понявший открытие, не знает и где кончаются данные (раунд 3 ревью:
`cat <<\EOF` и частичное слово проходили все рубежи с зелёным bash -n).
...
_HEREDOC_START_RE = re.compile(
    r"(?<!<)<<(?P<dash>-?)\s*(?P<q>['\"]?)(?P<delim>[\w-]+)(?P=q)")
# Ограничитель — [A-Za-z0-9_-]+ (ПОЛНОЕ слово, включая дефис: `cat <<EOF-1`
# читает данные до строки «EOF-1», а «EOF» её НЕ закрывает — прогон bash),
# а не [A-Za-z_][A-Za-z0-9_]*: `cat <<2` — легальный bash (прогон: тело
# печатается, строка «2» закрывает). Пропуск открытия глушит ВСЕ рубежи
# heredoc'а разом, поэтому то, что этот регексп НЕ разобрал, ловит громкий
# отказ `_shell_heredoc_unknown_before` (ниже) — молчаливого сведения нет.
# Маркерам конфликта широкий класс двери не открывает: `<<<<<<< HEAD` не
# матчится и с ним — левая граница отбрасывает все позиции внутри марки, а
# класс не содержит `<`.

# Вхождение `<<`, которое строгий регексп НЕ разобрал (`<<\ОГРАНИЧИТЕЛЬ`,
# кавычки с пробелом внутри, несловесный хвост ограничителя): сканер не
# понимает, открыт ли здесь heredoc, — по контракту модуля это ГРОМКИЙ
# отказ, а не молчаливое сведение (находка ai-ревью PR #1392, раунд 3:
# `cat <<\EOF` и `cat <<EOF-1` с частичным словом проходили все рубежи с
# зелёным bash -n). Herestring `<<<` и маркеры конфликта (`<<<<<<<`) не
# матчатся: у первого `<<` сдвоено с третьим `<`, у вторых все `<<` сдвоены
# с соседними.
_UNRECOGNIZED_HEREDOC_RE = re.compile(r"(?<!<)<<(?!<)")
...
def _shell_heredoc_unknown_before(lines: list, index: int) -> str | None:
    """Первая строка до `index` с вхождением `<<`, которое строгий регексп не
    разобрал, — текст отказа; либо None. Один и тот же пропуск глушит ВСЕ
    проверки heredoc'а (перехунковую, сторону-открыватель, итоговый файл):
    сканер, не понявший форму открытия, не знает и о том, что данные
    продолжаются. Поэтому неопознанная форма — отказ в каждой из трёх."""
    for lineno, line in enumerate(lines[:index]):
        stripped = _ARITH_SPAN_RE.sub(" ", line)
        recognized = {m.start() for m in _HEREDOC_START_RE.finditer(stripped)}
        for match in _UNRECOGNIZED_HEREDOC_RE.finditer(stripped):
            if match.start() not in recognized:
                return (f"строка {lineno + 1}: неопознанная форма heredoc'а "
                        f"({line.strip()!r})")
    ...
    for label, side in (("ours", ours), ("theirs", theirs)):
        ...
        if left_open is not None:
            return (f"сторона {label} открывает heredoc (<<{left_open}) и не "
                    ...
                    "файлу стало бы её телом")
        unknown = _shell_heredoc_unknown_before(lines + [""], len(lines) + 1)
        if unknown is not None:
            return (f"сторона {label}: {unknown} — вставка несёт форму `<<`, "
                    "которую сканер не разбирает: не понять, открыла ли она "
                    "heredoc, значит не знать, где кончаются её данные")
...
def resolve_file_text(text: str, suffix: str) -> tuple[str | None, str]:
    ...
    for start, end, ours, theirs in hunks:
        if suffix == ".sh":
            ...
            unknown = _shell_heredoc_unknown_before(lines, start)
            if unknown is not None:
                return None, (f"строки {start + 1}-{end + 1}: {unknown} — "
                              "сканер не понимает, открыт ли здесь heredoc, а "
                              "по контракту модуля любое сомнение — полный "
                              "отказ, не молчаливое сведение")
        ...
        if languages[path] == ".py":
            ...
            if left_open is not None:
                return _refuse(f"{path.name}: итоговый файл оставляет "
                               ...
                               "проверка отдельная")
            unknown = _shell_heredoc_unknown_before(
                resolved_lines + [""], len(resolved_lines) + 1)
            if unknown is not None:
                return _refuse(f"{path.name}: {unknown} — сканер не понимает "
                               "форму `<<`, поэтому не знает, где кончаются "
                               "данные; по контракту — полный отказ")
