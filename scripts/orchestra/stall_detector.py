...
def _render_body(fingerprint: str, evidence: list[str], run_url: str | None) -> str:
    lines = [
        "Автоматически заведено детектором простоя (#201): отпечаток причины "
        f"держался дольше {STALL_PERSIST_MINUTES} мин подряд и не совпал ни с "
        "одной уже открытой автозадачей.",
        "",
