...
Правила (нарушение = проверка красная, такой PR не слить):
  ...
  2. У PR определена задача пула. Источник — `task_ref.pr_task_candidates` (#394):
     основной — имя agent-ветки (`agent/<N>-<slug>`, ставит только
     `scripts/git/task-branch`, подделать нельзя), запасной — декларация
     первой строкой тела (`task_ref.declared_tasks`, для PR без такой
     ветки — ручной пуш, боты). Оба узких источника, ни один не «любое
     упоминание #N в прозе» (класс #187/#195).
  3. Задача #N существует как issue (не PR), открыта, помечена меткой `task`
     ...
     Исключение — переориентация в процессе реворка (#394): если ветка
     называет ЗАКРЫТУЮ задачу, а тело объявляет ДРУГУЮ, которая пригодна —
     используется задача из тела (см. `task_ref.pr_task_candidates`).
  ...
  5. У этой задачи нет ДРУГОГО открытого PR — второй PR на ту же задачу закрывается
     ...
     (тоже только для пригодной задачи). «Другой PR» — тот, у которого эта же
     задача среди `task_ref.pr_task_candidates` (ветка ИЛИ декларация тела), симметрично
     правилу 2.
...
def main() -> int:
    ...
    # Задача PR — task_ref.pr_task_candidates (#394): ветка первична, декларация первой
    # строкой тела — запасной путь (нет ветки agent/<N>-slug) и одновременно
    # подхватывает переориентацию в реворке (ветка называет закрытую задачу,
    # тело уже объявляет новую узкую). Прямого вызова task_ref.declared_tasks
    # здесь больше нет — единственная точка входа на вопрос «какая задача у
    # PR» это task_ref.pr_task_candidates/task_ref.resolve_pr_task, не разбросанные
    # обращения к declared_tasks.
    candidates = task_ref.pr_task_candidates(pull)
    if not candidates:
        problems.append(
            "Не удалось определить задачу PR: имя ветки не формата "
            "agent/<N>-<slug>, и тело не объявляет номер первой строкой. "
            "Заведи ветку через scripts/git/task-branch <N>-<slug> (основной "
            "источник) либо, если ветка не такой формы, поставь номер задачи "
            "первой строкой тела как #N."
        )
    ...
    if candidates:
        ...
        if eligibility and len(candidates) > 1:
            # Первый кандидат (обычно ветка) непригоден — пробуем следующего
            # (обычно декларация тела). Живой случай — переориентация в
            # реворке (см. докстринг task_ref.pr_task_candidates): ветка ещё называет
            # закрытую задачу, тело уже объявляет открытую-преемницу.
            fallback_number = candidates[1]
            fallback_issue = gh(f"repos/{repo}/issues/{fallback_number}")
            fallback_eligibility = task_eligibility_problems(fallback_issue, fallback_number)
            if not fallback_eligibility:
                issue_number, issue, eligibility = fallback_number, fallback_issue, []
        if eligibility:
            ...
            # Симметрично своему PR (#394): конфликт, если у чужого PR эта
            # задача среди task_ref.pr_task_candidates — ветка agent/<N>-slug ИЛИ
            # декларация первой строкой тела, а не просто любое упоминание
            # её номера в прозе описания (#195 — второй экземпляр асимметрии
            # #187: своя декларация уже была узкой, чужая гонялась по всему
            # тексту).
            others = []
            ...
            for other in pulls:
                if other["number"] == args.pr:
                    continue
                if issue_number in task_ref.pr_task_candidates(other):
