...
«закрыт(а|о|ы)?») рядом с `#N`. Русские слова НЕ входят в фиксированный
английский список ключевых слов GitHub (close/closes/closed/fix/fixes/
fixed/resolve/resolves/resolved — без перевода на другие языки, официальное
поведение) — автозакрытие при мерже здесь СТРУКТУРНО невозможно, а не
просто запрещено соглашением, как английские директивы. Открытый корень
(`закры[а-я]*`) отклонён экспериментом на живом теле PR #986: фраза
«зависит закрытие #806» (существительное) ложно совпала бы, а тот же PR
прямо пишет, что #806 «не закрывается автоматически этим PR». Прошедшее
время («закрыл(а|и)?») исключено отдельно (находка ai-review PR #1046,
круг 4, живой пул 323 слитых PR): ловило прозу о ЧУЖОМ прошлом действии
(«тот же класс, что закрыла #294» — PR #576; «приёмка закрыла #320» —
PR #358), не заявление ЭТОГО PR о себе. Подробности и мутационные тесты —
`scripts/lib/task_ref.py`/`test_task_ref.py`.
...
ветки при обычной приёмке):
  ...
  3. `referenced_change_archived` — если тело PR называет путь
     `openspec/changes/<id>`, этот change обязан быть уже заархивирован
     (`openspec/changes/archive/<id>` существует, исходный каталог — нет).
     Находка ai-review PR #1046 (круг 4): без этой проверки живой кандидат
     #218 (открытая задача, PR #956) назвал бы активный, ещё не
     заархивированный `openspec/changes/merge-reaction-registry` и получил
     бы улику `ok` — тело называет незавершённую работу, а не факт слияния
     плюс архивации. `None` (тело не называет такой путь вовсе, или путь не
     существует ни активным, ни архивным) — проверка неприменима, не
     гейтит закрытие; отсутствие пути в теле не улика САМА ПО СЕБЕ, а
     нейтральный исход, ни ok, ни fail.
...
def run(repo: str) -> list[str]:
    ...
    for number, pull in sorted(candidates.items()):
        ...
        try:
            ...
            # Находка ai-review PR #1046 (круг 4, живой прогон на 323 слитых
            # PR, кандидат #294 из тела PR #576): 404 — СТРУКТУРНЫЙ и
            # ОКОНЧАТЕЛЬНЫЙ ответ («номер вообще не issue этого репозитория»
            # — например, #N был PR, ставшим недоступным, или числом из
            # прозы, случайно похожим на кандидат), он не изменится на
            # следующем прогоне и не обязан краснить шаг месяцами (AGENTS.md,
            # «тормоз без газа не принимается»). Отличаем от транзиентного
            # сбоя (сеть, рейт-лимит — класс #454) тем же приёмом, что уже
            # применяют repo_invariants.py/scheduler.py: точная подстрока
            # ответа gh CLI, а не догадка.
            if "404" in str(error) or "Not Found" in str(error):
                lines.append(f"⏭️ #{number}: issue не найден (404, PR #{pr_number}) — не кандидат, не задача пула")
            else:
                lines.append(f"⚠️ #{number}: не удалось прочитать issue — {error}")
            continue
        if task_issue is None or "pull_request" in task_issue:
            # Структурный и окончательный исход: номер существует, но это PR,
            # не issue (GET issues/N отдаёт объект PR с ключом pull_request)
            # — не изменится на следующем прогоне (находка ai-review PR
            # #1046, круг 4, тот же довод, что и для 404 выше).
            lines.append(f"⏭️ #{number}: не issue задачи (PR #{pr_number} называет PR, не задачу) — не кандидат")
            continue
