...
# комментарий worker.yml у timeout-minutes (#877/#880):
#   - легитимный worst case DSH-работы прогона: до 10 попыток
#     (anthropic-oauth-pool пробуется первым + 9 записей default-chain)
#     × 20 мин (DSH_TIMEOUT_SECS=1200) + ОБЩИЙ бюджет RATE_LIMIT 30 мин
#     (WORKER_RATE_LIMIT_MAX_WAIT_SECS=1800 — расходуется один раз на прогон,
#     потраченное пулом передаётся цепочке) = 230 мин;
...
#     стоит ≤ 1.7 мин;
#   - порог = 230 + 25 = 255 мин: выше легитимного максимума с запасом, на
#     порядок превосходящим измеренный оверхед, и на 25 мин ниже
#     280-минутной стены job'а (worker.yml timeout-minutes). Пульс orchestra
#     идёт каждые 15 минут (orchestra.yml cron */15), в 25-минутное окно ниже
#     стены попадает минимум один пульс: рипер отменяет прогон, снимает
...
# Прежний порог 200 (выведен из замера старого мира) после пересчёта бюджета
# НИЖЕ нового легитимного максимума 230: легитимная цепочка «десять зависаний
# по 20 мин» (или девять зависаний и успех на десятой попытке) была бы убита
# рипером на 200-й минуте, не дойдя до финала и отчёта, — тот же исход #877
# («отчёт не написан»), другим ножом: не стена GitHub, а преждевременная
# отмена оркестратором (блокирующая находка ai-review PR #880, четвёртый
# раунд). Честная граница применимости: порог держится на том же допущении
# «ретраи RATE_LIMIT быстрые», что и бюджет 230 мин (раздел «Не подтверждено»
# в шапке task.sh) — на оси медленных ретраев легитимный прогон числом сверху
# не ограничен, и отличить его от зависшего, оставаясь ниже 280-минутной
# стены, невозможно; там выбран более дешёвый нож — рипер, освобождающий
# задачу сразу, а не стена, после которой задача ждёт TTL.
WORKER_STALL_MINUTES = 255
