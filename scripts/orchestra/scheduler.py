...
#
# Действующий расчёт (арифметика модели ДО #1160 — история выбора порога;
# о связывающем ограничении длины прогоны см. #1160 ниже):
...
# #1160 (суммарный бюджет цепочки DSH_CHAIN_TOTAL_BUDGET_SECS=9000с,
# lib/dsh-ci.sh) сузил худший ЛЕГИТИМНЫЙ прогон до ~302 мин (нога пула —
# нож 7200с + бюджет ожиданий 1800с = 150 мин + бюджет цепочки 150 мин +
# оверхед ≤2 мин; арифметика живёт ОДНИМ местом — комментарий инварианта 21,
# repo_invariants.py, этот файл и worker.yml ссылаются туда): «до ДВУХ
# полных попыток» выше и «НЕ защищает третью» ниже — история модели, вторая
# и третья полные попытки цепочки в одном прогоне теперь недостижимы. Порог
# 295 остаётся без пересчёта, но честно: в теоретическом углу «обе ноги
# сожжены полностью» он ~7 мин НИЖЕ края (~302) — осознанный размен (угол
# не наблюдался ни разу; цена повышения — короче окно возврата задачи в пул
# до 340-минутной стены), разбор — в том же комментарии инварианта 21.
