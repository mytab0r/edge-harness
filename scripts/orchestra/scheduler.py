...
def main() -> int:
    ...
    # Сторож живости событийного будильника (#510/#519) — класс, отличный от
    # heartbeat_check выше: тот видит «пропал пульс ЦЕЛИКОМ» (ни одного тика по
    # ЛЮБОМУ каналу), этот — «событийный канал молчит, пока расписание
    # маскирует поломку» (образцовый случай #510: два часа простоя, пульс
    # оставался зелёным). Применимость — только когда ai-review.yml/
    # pr-review.yml реально завершились в окне (не ложная тревога).
    lines += event_wake_check(repo, now)
