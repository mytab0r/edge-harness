...
def dispatch_ai_review_rework(
    ...
    не завершился (status in_progress/queued, conclusion пока не заполнен
    GitHub — тот же None, что у «атрибуции нет вовсе» в исходе (2), но
    ДРУГОЙ факт) — эскалация откладывается до известного исхода, не
    занятостью воркера вообще (busy-гейт снят этим же PR намеренно), а
    именно ожиданием исхода СВОЕГО последнего прогона. Раньше
    `last_worker_run_conclusion` читался только для ТЕКСТА эскалации —
    ...
    for pull in ordered_pulls:
        ...
        if attempts >= AI_REWORK_MAX_ATTEMPTS:
            ...
            if run is not None and run.get("status") in ("in_progress", "queued"):
                # Находка ревью PR #1260 (эскалация-до-итога-прогона):
                # «атрибуции нет вовсе» и «атрибуция есть, но исход прогона
                # ещё не известен» — разные факты (AGENTS.md, «алерт не
                # гадает»). GitHub заполняет `conclusion` только ПОСЛЕ
                # завершения прогона — пока собственный last_worker_run
                # in_progress/queued, run_conclusion тоже None, и раньше это
                # неотличимо трактовалось веткой ниже как «атрибуции нет»,
                # эскалация уходила владельцу на прогон, чья попытка доводки
                # ЭТОЙ задачи ещё физически идёт. Это НЕ возврат снятого
                # busy-гейта воркера (issue #1253 снял его намеренно —
                # решение эскалировать не обязано ждать, пока освободится
                # воркер ДЛЯ ДРУГОЙ задачи): здесь откладываем только пока
                # не известен исход СОБСТВЕННОГО последнего прогона этой
                # задачи, не занятостью воркера вообще.
                observations.append(
