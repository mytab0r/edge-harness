...
Обязанности:
  ...
  15. WIP-лимит перед взятием НОВОЙ задачи (#464, решение владельца
      ...
      придёт сам), и держит закрытым взятие НОВОЙ задачи, пока их больше
      WIP_LIMIT. Доводку уже открытых PR гейт НЕ трогает (находка ревью
      ...
      отдельный сигнал тем же каналом, что предохранитель конвейера: значит
      доводка идёт (воркер её диспетчит), но не успевает за темпом появления
      находок ревью — не «приток задач», а именно это.
"""
...
# поредеть» силами доводки, которую воркер всё это время ведёт (см. текст
# эскалации в wip_gate ниже — это НЕ «доводка стоит», а «доводка не успевает»).
...
def wip_gate(repo: str, now: datetime, pulls: list[dict]) -> tuple[list[str], list[str], bool]:
    ...
    if stuck_hours > WIP_GATE_STUCK_HOURS:
        ...
        if not stuck_markers:
            # Текст сигнала (находка ревью PR #466): раньше означал «гейт
            # держит dispatch_worker закрытым целиком, конвейер доводки
            # стоит» — неверно уже с этой правки, dispatch_worker при
            # закрытом гейте продолжает диспетчить воркера на доводку (см. её
            # докстринг). Теперь сигнал означает другое: доводка ИДЁТ (и
            # слияния идут независимо, см. merge_loop), но не успевает за
            # темпом появления находок ревью — если бы дело было в притоке
            # новых задач, взятие новых само по себе не объясняло бы, почему
            # очередь доработки не редеет так долго.
            escalation = escalate(
                repo, WATCHDOG_ISSUE,
                ...
                "Воркер всё это время диспетчируется на доводку уже открытых PR (см. "
                "dispatch_worker), слияния идут независимо от этого гейта (merge_loop) — если "
                "очередь всё равно не редеет так долго, значит доводка идёт, но не успевает за "
                "темпом появления находок ревью: другая проблема, требует внимания владельца.",
            )
            actions.append(
...
def dispatch_worker(
    ...
    try:
        ...
        if not wip_allowed:
            ...
            declared = declared_pr_task_numbers(pulls)
            rework_free = sorted(
                (issue for issue in free_task.free_candidates(pool) if issue["number"] in declared),
...
def main() -> int:
    ...
    # задачи: пока открытых PR, реально ждущих доработки, больше WIP_LIMIT
    # (см. wip_gate), dispatch_worker не берёт задачу без открытого PR, но
    # доводку уже открытых по-прежнему диспетчирует (находка ревью PR #466 —
    # см. докстринг dispatch_worker). `pulls` — тот же снимок, что merge_loop
    # уже довёл до актуального состояния этим прогоном (#443/#456: второго
    # обхода нет). conveyor_gate (#120) — единственный тормоз, закрывающий
    # dispatch_worker целиком: тот сигналит о сломанном самом конвейере, а не
    # о размере очереди доработки.
