...
Обязанности:
  ...
  8. Архив сессий раннера после мержа (#119) с durable-очередью на метке
     ...
     метка снимается только на успехе либо на терминальном исходе, где
     повтор не поможет («сессии нет», «формат не мигрируется» — #1430), и
...
# Классификация исхода вызова морды (#1433) — одно место правды вместо трёх
# рукописных копий, каждая из которых знала про свой отказ и не знала про
# чужие: архив знал `session-not-found`, заметки знали `HTTP 404`, и обе
# трактовали неподдерживаемый формат сессии как сломанную возможность, ожидая
# повтора, которого не будет никогда (#1430).
_mo_spec = importlib.util.spec_from_file_location(
    "morde_outcome", Path(__file__).resolve().parents[1] / "lib" / "morde_outcome.py")
morde_outcome = importlib.util.module_from_spec(_mo_spec)
_mo_spec.loader.exec_module(morde_outcome)
...
# openspec/changes/orchestrator-core-v2/design.md, раздел 2 — перечень стадий
# и легальных переходов; раздел 5 — порядок переноса точек чтения. Это
# ХОДЯЧИЙ СКЕЛЕТ (tasks.md, Этап 0): build_observed_pr/build_observed_task
# читают ТОЛЬКО то, что уже доступно в снимках pulls/pool, которые main()
# и так строит каждый прогон (open_pulls/open_task_issues) — ни одного
# дополнительного обхода Issues API этот код не добавляет; для PR нужен ОДИН
# точечный запрос к одиночному PR (mergeable_state недоступен в списочном
# ответе, тот же класс, что mark_conflicts уже учитывает выше). Флаги, не
# вычисленные на этом этапе (checks_red/contract_ok/stale_ready/gate2_error —
# design.md §2.2, tasks.md Этап 3-4), в снимок НЕ пишутся вовсе — честное
# отсутствие поля, не фальшивое False (design.md §1.2, приём CheckResult
# #1096: «не смог посмотреть» — отдельное состояние, не то же самое пустое
# значение).

# Стадии PR (design.md §2.2). Терминальность — атрибут значения (см.
# PR_STAGE_TERMINAL ниже), не отдельный список, поддерживаемый руками.
PR_STAGE_NO_TASK = "no_task"
PR_STAGE_DRAFT = "draft"
PR_STAGE_AWAITING_GATE1 = "awaiting_gate1"
PR_STAGE_AWAITING_GATE2 = "awaiting_gate2"
PR_STAGE_READY = "ready"
PR_STAGE_MERGED = "merged"
PR_STAGE_CLOSED = "closed"
PR_STAGE_TERMINAL = {PR_STAGE_MERGED, PR_STAGE_CLOSED}

# Стадии задачи (design.md §2.1).
TASK_STAGE_POOL = "pool"
TASK_STAGE_LEASED = "leased"
TASK_STAGE_HAS_PR = "has_pr"
TASK_STAGE_DONE = "done"
TASK_STAGE_ABANDONED = "abandoned"
TASK_STAGE_REOPENED_INVALID = "reopened_invalid"
TASK_STAGE_TERMINAL = {TASK_STAGE_DONE, TASK_STAGE_ABANDONED}
...
class ObservedPR(SimpleNamespace):
    ...
    `conflict` — трёхзначный (True/False/None), не двоичный: design.md §2.2
    прямо требует, чтобы "не знаю" (mergeable_state is None/"unknown") НЕ
    трактовался как «нет конфликта» — то же решение, что уже принял
    mark_conflicts (CONFLICT_CLEAR_STATES, снятие метки только по явно
    неконфликтным состояниям)."""
...
def build_observed_pr(repo: str, pull: dict) -> ObservedPR:
    """Снимок наблюдаемого состояния ОДНОГО PR — design.md §2.2.

    `pull` — ответ одиночного эндпоинта (`gh api repos/.../pulls/N`), НЕ
    элемент списка `open_pulls`: `mergeable_state` живёт только там (тот же
    класс, что mark_conflicts уже учитывает, см. её докстринг).

    Честная граница Этапа 0 (не вся design.md §2.2 сразу): `needs_rework` и
    `conflict` вычислены (дешёвые, уже есть готовые предикаты в
    review_labels.py); `checks_red`/`contract_ok`/`stale_ready`/`gate2_error`
    — НЕ вычислены здесь (задача этапов 3-4 tasks.md, требуют либо чтения
    check-runs, либо сопоставления с другими открытыми PR той же задачи) —
    их ключи просто отсутствуют в `flags`, не False."""
    number = pull["number"]
    labels = pull.get("labels") or []

    if pull.get("merged") or pull.get("merged_at"):
        stage = PR_STAGE_MERGED
    elif pull.get("state") == "closed":
        stage = PR_STAGE_CLOSED
    elif task_ref.resolve_pr_task(pull) is None:
        # Ветка не резолвится на задачу — вне ядра этого проекта (design.md
        # §2.2: боты вроде dependabot). Не ошибка, отдельная легальная стадия.
        stage = PR_STAGE_NO_TASK
    elif pull.get("draft"):
        stage = PR_STAGE_DRAFT
    elif not review_labels.gate1_open(labels):
        # design.md §2.2: awaiting_gate1 включает И «нет review:ok/large», И
        # «review:large БЕЗ review:large-ok» — ровно то, что gate1_open
        # проверяет (не gate1_decided, см. её докстринг в review_labels.py:
        # #432, смешение «гейт 1 решил» и «гейт 1 открыл слияние»).
        stage = PR_STAGE_AWAITING_GATE1
    elif review_labels.merge_label_gate(labels) is None:
        stage = PR_STAGE_READY
    else:
        stage = PR_STAGE_AWAITING_GATE2

    mergeable_state = pull.get("mergeable_state")
    if mergeable_state == "dirty":
        conflict = True
    elif mergeable_state in review_labels.CONFLICT_CLEAR_STATES:
        conflict = False
    else:
        # None/"unknown" — «не знаю», НЕ «нет конфликта» (design.md §2.2).
        conflict = None
    label_names = {label["name"] for label in labels}
    needs_rework = bool(label_names & {review_labels.REVIEW_CHANGES, review_labels.AI_CHANGES})

    return ObservedPR(
        repo=repo,
        number=number,
        stage=stage,
        flags={"conflict": conflict, "needs_rework": needs_rework},
    )


def build_observed_task(repo: str, issue: dict, pulls: list[dict]) -> ObservedTask:
    """Снимок наблюдаемого состояния ОДНОЙ задачи — design.md §2.1.

    `issue` — ответ `gh api repos/.../issues/N` (не элемент списка
    `open_task_issues`: закрытая задача, например `done`/`abandoned`, там
    не встретится вовсе — список несёт только `state=open`). `pulls` —
    список открытых PR (`open_pulls`), уже прочитанный main() этим же
    тактом — второго обхода Pull Requests это не добавляет.

    Честная граница Этапа 0: стадия `leased` определяется наличием
    `assignees`, БЕЗ проверки живости git-ref lock (ADR 0006) — design.md
    §1.6 называет аренду отдельным механизмом, снимок только ЧИТАЕТ её
    факт; полная проверка (жив ли сам ref) — не в объёме ходячего скелета,
    задача последующего этапа."""
    number = issue["number"]
    state = issue.get("state")
    state_reason = issue.get("state_reason")

    if state == "closed":
        # "not_planned" — единственный исход, отличный от "выполнена"
        # (design.md §2.1: done/abandoned — оба терминальны, но разный
        # исход важен консьюмерам, например health_audit).
        stage = TASK_STAGE_ABANDONED if state_reason == "not_planned" else TASK_STAGE_DONE
    elif state_reason == "reopened":
        # GitHub несёt `state_reason == "reopened"` для issue, закрытой и
        # затем переоткрытой (см. reject_reopened_tasks выше в этом файле,
        # #369) — то же самое поле, не второе вычисление.
        stage = TASK_STAGE_REOPENED_INVALID
    elif any(task_ref.resolve_pr_task(pull) == number for pull in pulls):
        stage = TASK_STAGE_HAS_PR
    elif issue.get("assignees"):
        stage = TASK_STAGE_LEASED
    else:
        stage = TASK_STAGE_POOL

    label_names = {label["name"] for label in issue.get("labels") or []}
    flags = {
        "blocked": BLOCKED_LABEL in label_names,
        "waiting_owner": "waiting:owner" in label_names,
    }
    return ObservedTask(repo=repo, number=number, stage=stage, flags=flags)


# ── Клиент HARNESS_URL/HANDS_TOKEN (#1287) ───────────────────────────────────
# orchestra.yml СЕГОДНЯ не имел доступа к собственному edge-harness DO
# (design.md §7.1, факт 3: grep по orchestra.yml до этого change — ноль
# совпадений HARNESS_URL/HANDS_TOKEN). Ходячий скелет заводит проводку — те
# же секреты, что worker.yml уже использует для /api/heartbeat (design.md
# §7.1, вывод: «ноль новых секретов»), не новый канал.
HANDS_TOKEN = os.environ.get("HANDS_TOKEN", "")
HARNESS_URL = os.environ.get("HARNESS_URL", "")


def post_entity_snapshot(kind: str, observed) -> tuple[bool, str]:
    """POST снимка ОДНОЙ сущности в Harness DO — design.md §5, спека п.42/44.

    `kind` — "task" | "pr" (маршрут `/api/{kind}s-snapshot` для задачи,
    `/api/pr-snapshot` для PR — таблица api-spec.json, cf-worker/api-spec.json).
    Возвращает (ok, detail): ok=False с честной причиной, если HANDS_TOKEN/
    HARNESS_URL не заданы («возможности нет», не тихий провал, AGENTS.md) —
    вызывающий код решает, считать ли это фатальным (на Этапе 0 — нет,
    ходячий скелет не должен ронять весь прогон).

    Прод-запись гейтится ТОЙ ЖЕ `_guard_raw_subprocess_write`, что и
    остальные сырые вызовы этого файла (update_branch/`_morde_rpc`) — второй
    копии решения «пишем ли мы вообще вне GitHub Actions» не заводим
    (найдено RAW_WRITE_CENSUS/test_raw_write_transport_census_stays_gated:
    без этой строки POST в Harness DO ушёл бы в сеть даже при локальном
    запуске `python scheduler.py` вне CI, тот же класс, что #950/#951)."""
    if not HANDS_TOKEN or not HARNESS_URL:
        return False, "HANDS_TOKEN/HARNESS_URL не заданы — снимок не отправлен"
    path = "tasks-snapshot" if kind == "task" else "pr-snapshot"
    if not _guard_raw_subprocess_write(f"POST /api/{path} (снимок {kind}#{observed.number}, Harness DO)"):
        return False, "вне GitHub Actions — DRY-RUN, снимок не отправлен"
    url = f"{HARNESS_URL.rstrip('/')}/api/{path}"
    body = json.dumps({
        "repo": observed.repo,
    ...
    try:
        ...
        return False, f"сеть недоступна: {error}"
    written = payload.get("written")
    return True, f"stage={observed.stage} written={written}"
...
def write_walking_skeleton_snapshot(repo: str, pulls: list[dict]) -> list[str]:
    """Этап 0 (tasks.md «Ходячий скелет»): пишет снимок ОДНОЙ пары
    задача+PR через новый путь целиком, best-effort — отказ (нет
    HANDS_TOKEN/HARNESS_URL, сетевая ошибка) не должен ронять остальной
    прогон main(), это ходячий скелет, не точка отказа старого пути."""
    lines = []
    try:
    ...
    try:
        ...
        observed_task = build_observed_task(repo, issue, pulls)
        ok, detail = post_entity_snapshot("task", observed_task)
        lines.append(f"{'✅' if ok else '⚠️'} снимок задачи #{WALKING_SKELETON_TASK_NUMBER} (ходячий скелет): {detail}")
    except Exception as error:  # noqa: BLE001
        lines.append(f"⚠️ снимок задачи #{WALKING_SKELETON_TASK_NUMBER} (ходячий скелет) не удался: {error}")
    return lines
...
def conflict_rework_attempts(repo: str, pr_number: int, task_number: int) -> int:
    ...
    живёт до 340 минут (#1067, было 280). Потерянная засчитанная попытка возвращала бы
...
def dispatch_conflict_rework(
    ...
    for pull in ordered_pulls:
        ...
        if attempts >= CONFLICT_REWORK_MAX_ATTEMPTS:
            ...
            # ставится СРАЗУ на dispatch, а сам worker.yml идёт до 340 мин, #1067) —
...
def _morde_rpc(
    opener: urllib.request.OpenerDirector, method: str, payload: dict, *, mutating: bool = True,
) -> dict:
    """RPC-конверт морды — третий, отдельный от gh()/subprocess транспорт.

    `mutating=True` (умолчание, все вызывающие ДО #1085 — workspace.
    archiveSession) — тот же класс прод-записи вне GitHub Actions
    (2026-09-11): гейтится ТОЙ ЖЕ prod_writes_allowed, что и gh()/
    send_telegram/_guard_raw_subprocess_write — второй копии решения не
    заводим. Вызывающий (archive_runner_sessions) игнорирует возвращаемое
    значение — `{}` безопасен как заглушка dry-run.

    `mutating=False` (#1085, session.history — первый READ через этот
    транспорт): гейт НЕ применяется вовсе — чтение не меняет состояние
    морды, гейтить его тем же классом, что архивацию сессии, было бы
    категориальной ошибкой. Находка ai-review PR #1089 (некритичное
    замечание): _session_progress_tip раньше звал этот RPC с умолчанием
    `mutating=True`, и вне CI получал `{}` из гейта — интерпретировал это
    как «события сессии пусты» (чужая причина: путал «читать не пытались»
    с «прочитали и там пусто»). READ обязан реально уйти в сеть везде, где
    есть конфигурация, вне зависимости от CI/не-CI."""
    if mutating and not _guard_raw_subprocess_write(f"POST /api/{method} (морда dsh-edge) payload={payload}"):
        return {}
    body = json.dumps({
        "type": "client-request",
        "rpcId": "orchestra",
...
# ── Терминальные отказы морды по СОДЕРЖАНИЮ отказа (#1430) ───────────────────────
# «Повтор поможет» и «повтор не поможет никогда» лечатся по-разному: первое
# ждёт следующего прохода, второе требует решения. Различать их обязан код,
# и по коду ответа это невозможно принципиально: и «формат сессии не
# мигрируется», и «исчерпана квота DO» приходят одним HTTP 500
# Классификация исхода вызова морды — ОДНО место правды:
# scripts/lib/morde_outcome.py (#1433). Маркеры терминальных отказов
# (включая немигрируемый формат сессии #1430 и признак «норма/потеря»),
# человекочитаемые причины и умолчание «незнакомый отказ = сломанная
# возможность» живут там; потребитель здесь — модуль `morde_outcome`,
# загружаемый выше рядом с этим комментарием.
...
# терминального исхода, где повтор не поможет («сессии нет», «формат не
# мигрируется» — #1430). At-least-once: лишний повтор архивации уже
# архивированной сессии безвреден (идемпотентно), потерянная сессия — нет.
SESSION_ARCHIVE_PENDING_LABEL = "session:archive-pending"

# Метка-СВОЙСТВО: к сессии этой задачи возвращаться незачем (#1432). Ставится
# после успешной архивации ЛИБО после иного терминального исхода, где повтор
# не поможет: «сессии раннера нет» (задача без прогона воркера) или «формат
# сессии не мигрируется» (#1430 — сессия в морде ЕСТЬ и не убрана, но вернуть
# номер в очередь значило бы вечный бессмысленный повтор). Все три означают
# «возвращаться сюда больше незачем»; в классе «формат не мигрируется» факт
# неубранной сессии назван строкой ⚠️ в отчёте, а не скрыт — свойство только
# снимает задачу с повторов.
#
...
# обещания не исполнял и кричал на КАЖДОМ проходе, включая первый.
#
# Два часа — это ~6 проходов пульса подряд (120 / 20.2; такт пульса: медиана 20.2 мин,
# замер #1184). Порог объявлен здесь один раз и читается отсюда же алертом —
...
def _mark_session_archived(repo: str, number: int) -> None:
    """Свойство «возвращаться к сессии незачем» (#1432) — ставится ПОСЛЕ
    факта, не вместо него: сначала сессия убрана, либо её заведомо нет, либо
    повтор заведомо бесполезен (терминальный исход, #1430), только потом
...
def archive_pending_age_hours(repo: str, number: int, now: datetime) -> float | None:
    """Сколько часов запись висит в очереди — по событию `labeled` этой метки.

    Состояние очереди целиком в метке (#1417), памяти о прошлом прогоне нет —
    значит и возраст обязан читаться из самого GitHub, а не из счётчика в
    коде. Берётся ПОСЛЕДНЕЕ событие `labeled` с нашей меткой: запись могли
    снять и поставить заново (успешная уборка с последующим новым мержем), и
    возраст считается от последней постановки, а не от первой в истории.

    Обход страниц обязателен (класс #308): у старой задачи событий больше
    сотни, и единственная страница отдала бы самые свежие, среди которых
    нашего `labeled` может не быть вовсе.

    None — возраст установить не удалось (события не отдались или события
    `labeled` с этой меткой в истории нет). Вызывающий обязан трактовать это
    как «просрочено», а не как «свежее»: fail-safe в сторону громкого, тот же
    выбор, что у `active_run_kind` в scripts/gh/wake_orchestra.sh (#1408).
    Обратное умолчание пряталo бы застрявшую очередь при любом сбое API."""
    try:
        events = review_labels.list_pages(
            f"repos/{repo}/issues/{number}/events?per_page=100", gh)
    except (RuntimeError, OSError, ValueError):
        return None
    stamps = [event.get("created_at") for event in events
              if event.get("event") == "labeled"
              and (event.get("label") or {}).get("name") == SESSION_ARCHIVE_PENDING_LABEL
...
def archive_queue_is_stale(repo: str, numbers: list[int],
    ...
    уборка — это штатная работа очереди (следующий проход повторит сам), и
    красить ею весь прогон оркестратора нельзя — красный прогон orchestra
    читается пульсом как «конвейер мёртв» и уводит владельцу ложный сигнал
    (живой случай 2026-09-21: четыре сообщения за полчаса про одну причину,
    ...
    for number in numbers:
        ...
        if age is None:
            stale_lines.append(
                f"🚨 #{number}: возраст записи в очереди {SESSION_ARCHIVE_PENDING_LABEL} "
                "установить не удалось — считаю просроченной (fail-safe в сторону громкого)")
        elif age > SESSION_ARCHIVE_QUEUE_STALE_HOURS:
            stale_lines.append(
...
def archive_runner_sessions(repo: str, task_numbers: list[int]) -> tuple[list[str], bool]:
    ...
    успехе либо на терминальном исходе, где повтор не поможет («сессии нет»,
    «формат не мигрируется» — #1430). Что осталось убрать, видно
    ...
    for number in task_numbers:
        try:
            ...
            # Единственный по-настоящему жёсткий сбой этой функции (#1422):
            # без метки повторять будет нечему, факт работы потерян, и лечится
            # это только руками. Всё остальное здесь — штатная работа очереди.
    ...
    try:
        ...
        # запись поставлена выше, повтор случится сам. Прогон не красим, иначе
        # пульс прочитает это как «конвейер мёртв» (#1422).
        lines.append(f"⚠️ морда dsh-edge недоступна для архива сессий: {error} — очередь по метке "
                     f"{SESSION_ARCHIVE_PENDING_LABEL} сохранена, повтор на следующем проходе")
        return (lines, hard_failure)
    archived, _still_queued = _archive_with_opener(repo, opener, task_numbers)
    # Возраст только что поставленной записи заведомо меньше порога — свежесть
    # очереди проверяет догоняющий проход, который видит её целиком.
    return (lines + archived, hard_failure)


def _archive_with_opener(repo: str, opener, task_numbers: list[int]) -> tuple[list[str], list[int]]:
    """Общее тело для первой попытки (после мержа) и для повтора по очереди —
    одно место правды, а не две копии одного цикла.

    Возвращает (строки, номера, оставшиеся в очереди). Номера, а не флаг
    «жёсткий сбой» (#1422): неудачная уборка сама по себе больше не красит
    прогон — запись лежит в очереди и повторится. Красит её ВОЗРАСТ, а
    возраст умеет считать только вызывающий, который видит очередь целиком.

    Из очереди уходят и терминальные исходы (#1430): «сессии нет» — убирать
    нечего; «формат сессии не мигрируется» (`morde_outcome.is_terminal`, #1433) —
    убирать не получится никогда, и держать номер в очереди значило бы вечный
    бессмысленный повтор плюс ложное «очередь не тает» через порог
    SESSION_ARCHIVE_QUEUE_STALE_HOURS."""
    lines = []
    ...
    for number in task_numbers:
        ...
        try:
            ...
            if morde_outcome.is_terminal(error):
                # Терминальный исход, а не сбой: повтор не поможет никогда —
                # держать такую запись в очереди значило бы копить вечный
                # хвост. Причина берётся из общего места правды (#1433), а не
                # переписывается здесь: иначе два вызывающих объяснят один и
                # тот же отказ по-разному.
                reason = morde_outcome.terminal_reason(error)
                if morde_outcome.is_loss(error):
                    # Потеря: сессия в морде ЕСТЬ (потому её формат и не
                    # мигрируется) — отчёт остаётся видимым с номером и
                    # причиной, но из очереди номер уходит, повторов больше
                    # нет (#1444).
                    done = (f"⚠️ #{number}: сессия {session_id} не заархивирована — "
                            f"повтор не поможет никогда ({reason}); снимаю с очереди "
                            f"{SESSION_ARCHIVE_PENDING_LABEL}")
                else:
                    # Норма: «сессии нет» — задача без прогона воркера (ручной
                    # PR, работа агента в чате) сессии не имеет вовсе.
                    done = f"🗄️ #{number}: {reason} — архивировать нечего"
            else:
                lines.append(f"⚠️ #{number}: сессия {session_id} не заархивирована "
                             f"({error}) — остаётся в очереди "
                             f"{SESSION_ARCHIVE_PENDING_LABEL}, повтор на следующем проходе")
...
class SweepSearchFailed(RuntimeError):
    """Search API не отдался при поиске закрытых задач без обеих меток
    (#1432). Отдельный класс, а не голый RuntimeError: вызывающий обязан
    отличить «поиск сломан» от «убирать нечего» — они лечатся по-разному, и
    именно их неразличимость делала отказ поставщика очереди невидимым
    (находка ai-review PR #1435)."""
...
def closed_tasks_needing_archive(repo: str, limit: int = CLOSED_TASK_SWEEP_LIMIT) -> list[int]:
    ...
    `limit` — СВОБОДНОЕ МЕСТО в очереди, а не фиксированный потолок прохода:
    вызывающий (`sweep_closed_task_sessions`) вычитает из
    `CLOSED_TASK_SWEEP_LIMIT` то, что уже лежит непорученным, и передаёт
    остаток. Дефолт `CLOSED_TASK_SWEEP_LIMIT` — случай пустой очереди.
    Различение важно: потолок здесь не оптимизация, а условие включения
    сразу по двум причинам. Первая — на момент запуска в репозитории лежат
    сотни давно закрытых задач, и попытка пометить все за проход упёрлась бы
    в лимит GitHub API. Вторая, найденная ai-review PR #1435, — газ: приток в
    очередь обязан быть не больше её проходимости, иначе лежащая морда даёт
    автономный рост и тот же упор в лимит, только растянутый на сутки.
    Очередь повторяется на каждом пульсе, хвост растает за несколько часов."""
    ...
    try:
        ...
        # Громко, не пустым списком (находка ai-review PR #1435). Прежнее
        # оправдание «очередь и так повторяется» было фактически неверным:
        # повторяется только уже помеченное, а умирает здесь ПОСТАВЩИК
        # нового — то есть накопление, ради которого задача и заведена,
        # возобновляется без единого сигнала. Search API у GitHub несёт
        # собственную квоту и падает отдельно от core, так что случай
        # реальный, а не теоретический. Соседний путь того же механизма
        # (retry_pending_session_archives) тот же класс отказа уже отдаёт
        # громко — здесь была единственная тихая дверь.
        raise SweepSearchFailed(str(error)) from error
...
def sweep_closed_task_sessions(repo: str) -> list[str]:
    ...
    **Газ: приток не больше проходимости** (находка ai-review PR #1435, и она
    про живой отказ, а не про теорию). До #1432 приток в очередь ограничивала
    частота мержей. Свип сделал его автономным: 20 меток за пульс независимо
    от того, убирается ли хоть что-то. Морда лежит сутками — живой случай
    2026-09-21 — и тогда за сутки в очередь ложится ~1400 записей, а
    догоняющий проход на КАЖДОМ такте считает возраст КАЖДОЙ из них
    (`archive_pending_age_hours` — запрос таймлайна на запись). Это тысячи
    запросов GitHub в час, упор в rate limit, красный оркестратор и вставшие
    слияния: механизм уборки душил бы конвейер ровно на том отказе, ради
    которого очередь и заведена.

    Поэтому свип сначала смотрит, сколько уже лежит в очереди (один запрос по
    метке), и ставит только до `CLOSED_TASK_SWEEP_LIMIT` суммарно. При живой
    морде очередь тает и темп 20/пульс сохраняется; при лежащей — рост
    останавливается на потолке."""
    if not DSH_EDGE_URL or not DSH_EDGE_ACCESS_KEY:
        return []  # конфигурации нет — очередь не наша забота
    try:
        depth = len(pending_session_archive_issues(repo))
    except RuntimeError as error:
        # Глубину не узнали — ставить вслепую нельзя, это и есть отсутствие
        # газа. Громко: молчаливый пропуск неотличим от «очередь пуста».
        return [f"⚠️ очередь уборки: глубину не узнать ({error}) — постановка пропущена"]
    room = CLOSED_TASK_SWEEP_LIMIT - depth
    if room <= 0:
        return [f"🧹 очередь уборки полна ({depth}/{CLOSED_TASK_SWEEP_LIMIT}) — новых не ставлю, "
                "разбирается причина, по которой она не тает"]
    try:
        numbers = closed_tasks_needing_archive(repo, room)
    except SweepSearchFailed as error:
        return [f"⚠️ поиск закрытых задач для уборки не отдался ({error}) — "
                "постановщик очереди встал, накопление возобновится молча, если не починить"]
    if not numbers:
        return []
    queued, failed = [], []
    for number in numbers:
...
def retry_pending_session_archives(
    ...
    Красит прогон (#1422) НЕ сам факт неудачной уборки, а ВОЗРАСТ записи:
...
def append_session_notes(notes: list[tuple[int, str]]) -> tuple[list[str], bool]:
    ...
    archive_runner_sessions. Исходов отказа здесь ТРИ (#1430), и лечатся они
    по-разному: сессии нет (`HTTP 404` — задача без раннера, например ручной
    PR) — норма, строка не появляется; терминальный отказ СОДЕРЖАНИЯ
    (`morde_outcome.is_terminal`, #1433: например немигрируемый формат сессии) —
    заметка потеряна, повтор не поможет никогда: строка ⚠️ с номером задачи и
    причиной, прогон НЕ красится (красный прогон orchestra пульс читает как
    «конвейер мёртв», а повтор здесь не лечит ничего); всё остальное (сеть,
    таймаут, исчерпанная квота DO) — сломанная возможность: 🚨 и жёсткий сбой,
    как раньше."""
    if not notes:
        return ([], False)
    if not DSH_EDGE_URL or not DSH_EDGE_ACCESS_KEY:
    ...
    for number, text in notes:
        ...
        try:
            ...
            if morde_outcome.is_terminal(error):
                # Повтор не поможет никогда — то же место правды, что у архива
                # выше (#1433). Раньше здесь знали только про HTTP 404 и ждали
                # повтора там, где морда отказывается мигрировать формат
                # сессии: красный прогон на исходе, которого не изменит ни
                # один повтор (#1430).
                #
                # Отчёт — по документу исхода: «сессии раннера нет» (норма,
                # задача без прогона воркера) молчит, иначе настоящая потеря
                # утонула бы в норме; потеря — строка с номером и причиной
                # (#1444), но прогон не красим: ждать некого.
                if morde_outcome.is_loss(error):
                    reason = morde_outcome.terminal_reason(error)
...
def after_merge(
    ...
    if archive_targets:
        # Гейт архива — archive_targets, НЕ task_numbers (находка ai-review
        # PR #1418). Прежняя редакция стояла внутри `if task_numbers:` —
        # списка упоминаний тела, — и слитый PR с телом без упоминаний
        # открытых задач пула (путь санкционированный: контракт берёт номер
        # из ветки, тело не читает вовсе) не делал ни попытки, ни метки, ни
        # строки отчёта: сессия harness-<N> молча оставалась активной
        # навсегда, ровно та потеря информации, которую #1417 снимает.
        # Вторая половина того же гейта: PR без agent-ветки (archive_targets
        # пуст) не ходит в морду вовсе — прежде archive_runner_sessions(repo,
        # []) делал холостой логин, а при лежащей морде отдавал hard_failure,
...
# легитимного прогона он больше не служит.
#
# Замер живых длительностей worker.yml (100 последних прогонов, gh run list
# --workflow=worker.yml, снято 2026-09-09 — в мире «150 мин таймаута DSH на
# ОДНУ попытку», до #727/#877):
#   - success с реальной DSH-работой (длительность > 5 мин, 24 прогона):
#     медиана ≈ 73 мин, p75 ≈ 140 мин, МАКСИМУМ 151.1 мин — легитимный
#     success достигал этого значения РОВНО ПЯТЬ РАЗ (~20% выборки), что
#     соответствовало тогдашнему 150-минутному таймауту DSH за одну попытку.
#     После #877 попытки такого размера не существует ни у одного потребителя
#     (комментарий docs/decisions/0005-dispatch-tail-campaign.md) — переносить
#     этот максимум на новый мир нельзя: легитимный максимум выводится из
#     бюджета ниже, а не из этого списка;
#   - остальные success (без реальной работы — "свободных задач нет",
#     no-op) укладываются в 0.3–1.7 мин — единственный прямой замер оверхеда
#     прогона вне DSH-работы (установка/git/отчёт);
...
# наблюдаемого разброса легитимных success того времени (16.6–151.1 мин).
#
# Действующий расчёт — та же арифметика, что легла в шапку task.sh и
# комментарий worker.yml у timeout-minutes (#877/#880 → #1067):
#   - #1067 (инцидент 2026-09-13, прогон 34735752165) отказался от модели
#     «до 10 попыток × равный таймаут» — она не позволяла дать единственному
#     реально работающему провайдеру (GLM) достаточно времени ни при каком
#     бюджете job'а (см. разбор — комментарий у DSH_TIMEOUT_SECS,
#     scripts/worker/task.sh). Новая модель бюджетирует легитимный worst case
#     как «до ДВУХ попыток в одном прогоне заняли полный DSH_TIMEOUT_SECS»
#     (120 мин, task.sh): 2 × 120 мин + ОБЩИЙ бюджет RATE_LIMIT 30 мин
#     (WORKER_RATE_LIMIT_MAX_WAIT_SECS=1800 — расходуется один раз на прогон,
#     потраченное пулом передаётся цепочке) = 270 мин;
#   - оверхед вне DSH-работы входит в сравниваемую величину (возраст считается
#     от run_started_at, то есть вместе с установкой) и по замеру выше
#     стоит ≤ 1.7 мин;
#   - порог = 270 + 25 = 295 мин: выше легитимного максимума с запасом, на
#     порядок превосходящим измеренный оверхед, и на 45 мин ниже 340-минутной
#     стены job'а (worker.yml timeout-minutes, #1067). Пульс orchestra идёт
#     каждые 15 минут (orchestra.yml cron */15), в 45-минутное окно ниже
#     стены попадает несколько пульсов: рипер отменяет прогон, снимает
#     аренду задачи (release-full) и открывает пул ДО молчаливой стены
#     GitHub. Ножи не равны: после стены возврат задачи в пул шагом job'а
#     не гарантирован — в худшем случае задача ждёт 24-часовой TTL-сборщик
#     (#121), всё это время держа единственного воркера репозитория.
# Прежний порог 255 (пересчитан #880 под модель «10 × 20 мин») после отказа от
# этой модели (#1067) НИЖЕ нового легитимного максимума 270: легитимный
# прогон «первая попытка зависла на полном таймауте, вторая реально доделала
# работу» (ровно то, что показывает замер GLM в task.sh) был бы убит рипером
# на 255-й минуте, не дойдя до финала и отчёта, — тот же класс исхода, что
# #877/#880 уже чинили однажды, здесь же и пересчитан заново. Честная граница
# применимости: порог принимает ДВЕ полных попытки таймаута как легитимные,
# но НЕ защищает третью (см. «принятый остаточный риск» — комментарий
# task.sh) — такой сценарий не наблюдался ни разу в замере, и если случится,
...
# максимума, заметно ниже возрастного потолка (295). Не подтверждённый
# остаточный риск (тот же класс, что уже принят у WORKER_STALL_MINUTES выше):
# ВТОРАЯ попытка подряд, тоже молчащая все 120 минут БЕЗ маркера смены
# провайдера между ними, читалась бы как непрерывная тишина ≥150 мин и была
# бы убита раньше своего естественного финала — сценарий не отличим от
# истинного зависания текущими данными, и если он живой, это факт для
# пересчёта порога, не повод считать признак сломанным заранее.
#
# КРИТИЧНО (находка ai-review PR #1089, дважды): базовое наблюдение (первый
# маркер по run_id) обязано появляться СРАЗУ, как только прогону есть с чем
# сравнить (коррелированная задача найдена) — НЕ только начиная с возраста
# WORKER_SILENCE_MINUTES. Ранняя версия гейтила саму ПРОВЕРКУ тишины
# (`reap_stalled_worker_run`) возрастом ≥150 — значит первый маркер физически
# не мог появиться раньше 150-й минуты, а тишина (маркер+150) не могла
# сработать раньше 300-й — то есть НИКОГДА раньше возрастного потолка (295).
# Симуляция ревьюера это подтвердила: возрастной нож бил на 295-й минуте в
# каждом достижимом состоянии, тишинный — ни разу. Фикс: `reap_stalled_worker_
# run` резолвит task_number и опрашивает сессию КАЖДЫЙ пульс с первого,
# без возрастного гейта на саму проверку (гейт остаётся только на решение
# «что считать признаком тишины» — сама разница seq между пульсами).
#
# Второе следствие того же разбора: приоритет между двумя ножами перевёрнут.
# Раньше возраст вычислялся БЕЗУСЛОВНО первым и убивал прогон на 295-й минуте
# даже при растущей сессии — прямое противоречие тексту задачи («растущий seq
# — прогон жив, сколько бы он ни шёл, возраст не важен до стены GitHub»).
# Теперь: пока признак тишины ДОСТУПЕН (сессия коррелирована и морда
# отвечает) — решение принимает ОН один (растёт — жив без всякого потолка по
# возрасту; не растёт WORKER_SILENCE_MINUTES — завис); возрастной потолок
# применяется ТОЛЬКО когда признак тишины НЕДОСТУПЕН (задача не
# коррелирована, морда/сеть/маркеры отказали) — честный фоллбэк на
# WORKER_STALL_MINUTES, а не вторая, перебивающая первую проверка.
WORKER_SILENCE_MINUTES = 150

# Маркер-носитель состояния «последний увиденный seq сессии по этому run_id»
# между пульсами (#1085) — РОВНО ОДИН комментарий в WATCHDOG_ISSUE на run_id,
# редактируемый на месте (edit_issue_comment/PATCH, тот же приём, что #1100
# уже применяет для quota_alert.record_reading — без этого приёма каждый
# растущий пульс писал бы НОВЫЙ комментарий, а с фиксом выше (проверка с
# первого пульса, не с 150-й минуты) это означало бы комментарий в #120
# каждые 15 минут всю жизнь КАЖДОГО активного прогона). Момент «последний
# раз видели рост» — поле `updated_at` этого же комментария (GitHub меняет
# его при PATCH; `created_at` при редактировании на месте не сдвигается —
# использовать его для отсчёта тишины после первого же PATCH было бы
# неверно, тишина считалась бы от рождения комментария, а не от последнего
...
def _session_progress_tip(session_id: str) -> tuple[int | None, str | None]:
    ...
    try:
        ...
        _morde_login(opener)
        value = _morde_rpc(
            opener, "session.history", {"sessionId": session_id, "maxMessages": 1}, mutating=False,
        )
    except (RuntimeError, OSError, urllib.error.URLError, ValueError) as error:
...
def _worker_silence_reason(
    ...
    подряд. Вызывается КАЖДЫЙ пульс с первого, где у прогона уже есть
    коррелированная задача (см. reap_stalled_worker_run, находка ai-review
    PR #1089) — без этого база появлялась бы только к возрасту
    ...
    try:
        comments = all_issue_comments(repo, WATCHDOG_ISSUE, max_pages=3)
    except RuntimeError as error:
        return None, (
    ...
    if not own:
        ...
        return None, None
    # Один комментарий на run_id по конструкции (редактируется на месте, не
    # плодится) — если их всё же несколько (гонка двух пульсов), берём
    # СТАРШИЙ id как каноничный, не гадаем какой «правильный».
    marker = max(own, key=lambda comment: comment.get("id", 0))
    match = re.search(r"seq=(\d+)\]", marker.get("body") or "")
    if match is None:
        return None, (
            f"⚠️ маркер прогресса воркера в #{WATCHDOG_ISSUE} повреждён "
            f"(seq не разобран из {marker.get('body')!r})"
        )
    marker_seq = int(match.group(1))
    if seq > marker_seq:
        try:
            edit_issue_comment(repo, marker["id"], f"{marker_prefix}seq={seq}]")
        except RuntimeError as error:
            return None, f"⚠️ маркер прогресса воркера в #{WATCHDOG_ISSUE} не обновлён: {error}"
        return None, None
    updated_raw = marker.get("updated_at") or marker.get("created_at")
    if not updated_raw:
        return None, f"⚠️ маркер прогресса воркера в #{WATCHDOG_ISSUE} без updated_at/created_at"
    silence_minutes = minutes_between(parse_time(updated_raw), now)
...
def reap_stalled_worker_run(
    ...
    тишина физически не могла сработать раньше 2×порога=300-й минуты, то
    есть НИКОГДА раньше возрастного потолка 295; см. разбор у
    WORKER_SILENCE_MINUTES выше):
      1. Тишина сессии harness-<N> — ГЛАВНЫЙ нож, когда доступен. Задача
         резолвится (_stalled_run_task_number) КАЖДЫЙ пульс с первого, не с
         возраста ≥150: без коррелированной задачи проверять нечего, но сама
         попытка коррелировать не гейтится возрастом. Пока признак доступен:
         сессия растёт (или это первое наблюдение) → прогон жив, возрастной
         потолок НЕ применяется вовсе, сколько бы прогон ни шёл (до жёсткой
         стены GitHub, timeout-minutes самого job'а — её этот код не
         контролирует и не обязан); сессия не растёт WORKER_SILENCE_MINUTES
         подряд → завис, решение принято раньше возрастного потолка.
      2. Возраст (WORKER_STALL_MINUTES) — ФОЛЛБЭК на случай, когда признак
         тишины НЕДОСТУПЕН: задача не коррелирована (адресный прогон на PR
         с уже открытым PR, либо аренда сгорела до следа), морда/сеть/
         маркеры отказали. Честная деградация (AGENTS.md, «алерт не
         гадает») — решение остаётся на прежнем, медленном, но надёжном
         пороге, названном явно в тексте действия, а не молчаливым
         предположением «жив».
...
def _create_task_replacement(repo: str, pull: dict, task_number: int) -> int:
    ...
    body = (
        ...
        # #720: объявление связи — ПОСЛЕДНЕЙ непустой строкой (его читает
        # ai_review.blocked_by_numbers), в том числе поверх цитаты тела
        # исходного PR: чужая строка «БЛОКИРУЕТСЯ: …» в цитате не должна
        # стать объявлением ЭТОЙ задачи. Замена зависимостей не знает —
        # явное «ничем», не пропуск (иначе create_pool_issue откажет).
        "БЛОКИРУЕТСЯ: ничем\n"
    )
...
def main() -> int:
    ...
    # tasks.md Этап 0) — best-effort, не влияет на wip_gate/merge_queue/старый
    # путь ниже: пишет ТОЛЬКО в новые таблицы Harness DO, ничего не читает из
    # них обратно на этом этапе. Отказ (нет HANDS_TOKEN/HARNESS_URL, сеть) не
    # прерывает прогон — см. докстринг write_walking_skeleton_snapshot.
    #
    # Печатается в лог прогона (stdout), НЕ в `lines` отчёта — тот же приём и
    # то же обоснование, что у announce_write_mode() выше: `lines` читает
    # stall_detector.detect_and_act как источник СИМПТОМОВ простоя по
    # незнакомому отпечатку причины (⚠️/🚨-строки). Находка живым прогоном
    # теста (test_main_labels_old_unclaimed_task_end_to_end,
    # test_main_makes_zero_mutating_calls_on_fully_empty_queue): добавление
    # строки ходячего скелета в `lines` меняло текст отчёта, который
    # detect_and_act постит в issue #120, и это ломало census-тесты FakeGh
    # (незнакомый маршрут в комментарии) — ходячий скелет не должен влиять на
    # ЧУЖУЮ логику отчёта о простое, только на свою собственную видимость.
    for skeleton_line in write_walking_skeleton_snapshot(repo, pulls):
        print(skeleton_line)
    ...
    # запись, положенная ПОЗЖЕ последнего алерта, значит очередь растаяла и
    # набралась заново — сигнал уходит снова. Закрывающего маркера нет
    # сознательно и холостого хода на пустой очереди тоже: почему — см.
    # комментарий у SESSION_ARCHIVE_STALE_MARKER.
