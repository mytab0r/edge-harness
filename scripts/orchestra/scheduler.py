...
# lib/dsh-ci.sh) сузил худший ЛЕГИТИМНЫЙ прогон до ~277 мин (пул ≤120 мин +
# ~155 мин цепочки + ≤2 мин оверхеда — комментарий у timeout-minutes,
# worker.yml): «до ДВУХ полных попыток» выше и «НЕ защищает третью» ниже —
# история модели, вторая/третья полные попытки в одном прогоне теперь
# недостижимы. Порог 295 остаётся корректным без пересчёта: выше нового
# коридора (~277) и ниже 340-минутной стены.
