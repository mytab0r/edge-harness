...
def after_merge(repo: str, pull: dict, other_pulls: list[dict] | None = None) -> tuple[list[str], bool]:
    ...
    # Кандидаты этого PR (ветка + декларация тела, #394) — приёмка
    # (merged_pr_map/accept_merged_tasks) видит задачу «своей», если её номер
    # среди них; используется ниже, чтобы напоминание говорило правду про
    # ИМЕННО этот случай (задача из ветки, задача из тела-преемницы при
    # реворке, или просто упомянутая в прозе).
    own_candidates = task_ref.pr_task_candidates(pull)
    for task_number in task_refs:
        ...
        try:
            ...
            # Приёмка видит задачу «своей», если её номер — среди
            # task_ref.pr_task_candidates этого PR (#394: ветка ИЛИ
            # декларация первой строкой тела, merged_pr_map регистрирует под
            # обоими). Обещание «приёмка закроет её сама» для ЛЮБОГО
            # упоминания было ложным: задача, упомянутая только в прозе, не
            # среди кандидатов, а через ACCEPTANCE_PENDING_HOURS её снимает
            # reap_stale с причиной «PR не появился» — ровно тот класс
            # неверных причин, что этот PR чинит для объявленных задач
            # (находка AI-ревью PR #253).
            if int(task_number) in own_candidates:
                reminder = (
                ...
                reminder = (
                    ...
                    "первой строкой тела PR и не назвал её именем ветки — стадия приёмки её "
                    "не увидит и не закроет сама. Если критерий требует реального пост-мерж "
                    "прогона (канарейка/E2E), проведи его и оставь улику; закрывай задачу "
                    "только через PR, объявляющий её номер первой строкой тела или веткой "
...
# Дисклеймер о неполноте в теле закрывающего PR (#335): task_ref.pr_task_candidates
# (#394 — ветка + декларация первой строкой) отвечает «к какой задаче
# относится PR», не «закрывает ли он её целиком» — приёмка раньше читала
# только декларацию и закрывала задачу вопреки собственному тексту PR. Живой
# случай: PR #303
...
def merged_pr_map(pulls: list[dict]) -> dict[int, dict]:
    ...
    `task_ref.pr_task_candidates` (#394, то же соглашение, что уже применяет
    contract_check при авто-назначении) — регистрируется под КАЖДЫМ узким
    кандидатом (имя agent-ветки И декларация первой строкой тела), не только
    первым: докрытие после того, как задача закрыта раньше срока («закрытая
    задача не переоткрывается»), оформляется НОВОЙ узкой задачей в теле, а
    ветка (создаётся один раз, scripts/git/task-branch) продолжает называть
    старую — живые PR #388/#384/#359/#167 на 2026-09-06. Регистрация под
    обоими числами безвредна: accept_merged_tasks ищет только среди ОТКРЫТЫХ
    задач пула, а закрытая старая задача там уже не значится — находит его
    только запись под настоящим (новым) номером. Узкая семантика по-прежнему
    ...
    best: dict[int, dict] = {}
    for pull in pulls:
        for number in task_ref.pr_task_candidates(pull):
