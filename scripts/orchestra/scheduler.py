...
# «Internal runtime error» — различие только в ТЕЛЕ отказа.
#
# Маркер → человекочитаемая причина. Одна таблица на оба вызывающих ниже
# (заметки-итоги и архив сессий): вторая рукописная копия условия — отложенный
# рецидив, живой случай — PR #1431, где различение сделали в заметках и забыли
# в архиве сорока строками ниже. Системный дом классификации (общий модуль для
# всех вызовов морды) — задача #1433; пока она не слита, эта таблица —
# единственное место, где терминальные маркеры перечислены.
#
...
# молчаливого накопления. Тот же fail-safe, что у active_run_kind в
# scripts/gh/wake_orchestra.sh (#1408).
SESSION_TERMINAL_FAILURES: tuple[tuple[str, str], ...] = (
    # #1430, живой прогон 35647962282 (2026-09-22): поле `truncated` в data
    # записал наш же стример (дефект #1404), фикс #1405 (8b46e8d4) убрал его
    # из НОВЫХ событий — но уже записанные сессии остались в хранилище морды
    # такими, какие есть, и апстримная миграция v0→v1 будет отказывать всегда:
    # формат в хранилище сам не изменится. Префикс, а не полное имя класса:
    # покрывает и `SessionFormatUnsupportedMigrationError`, и любую будущую
    # `SessionFormatUnsupported*`; `SessionPersistenceCorruptionError`
    # (повреждение, а не честное «билд не умеет читать такой лог», ADR 0027)
    # сюда НЕ входит и остаётся сломанной возможностью.
    ("SessionFormatUnsupported",
     "морда отказывается мигрировать формат сессии — формат в хранилище сам не изменится"),
)


def session_failure_terminal_reason(error: object) -> str | None:
    """Человекочитаемая причина терминального отказа морды, None — если отказ
    не терминальный (вызывающий обязан различать, а не печатать пустую
    причину рядом со сломанной возможностью)."""
    text = str(error)
    for marker, reason in SESSION_TERMINAL_FAILURES:
        if marker in text:
            return reason
    return None
...
def session_failure_is_terminal(error: object) -> bool:
    """True — повтор этого отказа не поможет никогда (см. таблицу выше)."""
    return session_failure_terminal_reason(error) is not None
...
def _archive_with_opener(repo: str, opener, task_numbers: list[int]) -> tuple[list[str], list[int]]:
    ...
    нечего; «формат сессии не мигрируется» (`session_failure_is_terminal`) —
    ...
    for number in task_numbers:
        ...
        try:
            ...
            elif session_failure_is_terminal(error):
                # Тот же терминальный класс во втором месте (#1430, находка
                # ревью PR #1431): немигрируемый формат сессии archiveSession
                # откажется читать всегда — держать номер в очереди значило бы
                # бессмысленный RPC на каждом пульсе, а через
                # SESSION_ARCHIVE_QUEUE_STALE_HOURS ещё и ложное «очередь не
                # тает» владельцу. Строка остаётся видимой (⚠️ с номером),
                # повторов больше нет.
                reason = session_failure_terminal_reason(error)
...
def append_session_notes(notes: list[tuple[int, str]]) -> tuple[list[str], bool]:
    ...
    (`session_failure_is_terminal`, например немигрируемый формат сессии) —
    ...
    for number, text in notes:
        ...
        try:
            ...
            if session_failure_is_terminal(error):
                # Терминальный исход по СОДЕРЖАНИЮ отказа (#1430): заметка
                # потеряна, и повтор не поможет никогда. Не молчим — факт с
                # номером и причиной остаётся в отчёте, — но и не красим
                # прогон: «повтор поможет» это исход, который ЖДАЛИ, а здесь
                # ждать некого.
                reason = session_failure_terminal_reason(error)
