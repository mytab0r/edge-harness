...
# зависит от вызывающего канала) — ОБЩИЙ на весь прогон ЦЕПОЧКИ (#877,
# dsh_run_with_provider_chain в lib/dsh-ci.sh расходует его по факту через
# DSH_RUN_WAITED_SECS, не выдаёт заново каждому провайдеру), не на попытку и
# не на провайдера — иначе N провайдеров могли бы выжечь N × 30 мин суммарно
# (находка ai-review PR #880 на первой версии этого же комментария).
#
...
# ai-review, отдельно не решается здесь. rc=124 (наш таймаут) теперь ОТДЕЛЁН
