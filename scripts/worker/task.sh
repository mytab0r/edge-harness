...
# сломанным заранее.
#
# ВСЁ ВЫШЕ — модель бюджета ДО #1160 (история выбора DSH_TIMEOUT_SECS=7200;
# про ОДНУ попытку она верна и сейчас), сам бюджет длины прогона она больше
# не описывает. #1160 связал длину прогона суммарным wall-clock бюджетом
# ВСЕЙ цепочки DSH_CHAIN_TOTAL_BUDGET_SECS (9000с/150 мин, lib/dsh-ci.sh):
# вторая полная попытка 7200с в одном прогоне невозможна (9000с < 2×7200с —
# арифметика «2 × 7200с + 1800с = 270 мин, запас 20 мин» выше описывала мир
# до #1160), «ТРЕТЬЯ полная попытка» из остаточного риска выше недостижима
# даже теоретически, а нож дедлайна режет каждую попытку и каждый ретрай
# (DSH_CHAIN_DEADLINE_EPOCH). Худший легитимный прогон теперь: попытка пула
# ДО цепочки (до 7200с — её wall в бюджет цепочки не входит) + ~9300с
# цепочки + ≤2 мин оверхеда (замер scheduler.py, не «~50 мин» из этой
# шапки) ≈ 277 мин из 340 (worker.yml) — текущая арифметика живёт в
# комментарии worker.yml у timeout-minutes и у инварианта 21
# (repo_invariants.py), не здесь.
#
...
# (бюджет короткого окна кончился), all_providers_exhausted (цепочка ниже
# исчерпана целиком) и chain_budget_exhausted (#1160 — суммарный бюджет
# цепочки кончился до конца списка) — все четыре причины возвращают задачу
# в пул СРАЗУ
