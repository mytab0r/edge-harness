...
# зависит от вызывающего канала) — суммарный ретрай ОДНОГО провайдера ВНУТРИ
# его попытки в цепочке (#727 ниже), не на попытку.
#
...
# Новое значение — 1200с (20 мин): 10 попыток × 20 мин = 200 мин, оставляет
# ~80 мин из 280-минутного бюджета job'а на установку/git/пост-обработку и на
# ретрай временного RATE_LIMIT (WORKER_RATE_LIMIT_MAX_WAIT_SECS выше) —
# арифметика по фактам конфигурации, не гипотеза о типичной длительности
# успешного вызова (данных о ней нет, «не подтверждено»). Патологический
# случай «зависло почти у самого края таймаута КАЖДОГО из 10 провайдеров
# ПОДРЯД» теоретически всё ещё возможен и не решён здесь (тот же непокрытый
# класс уже принят в #421 для ai-review) — задокументирован, не тихо
# проигнорирован. rc=124 (наш таймаут) теперь ОТДЕЛЁН от «провайдер молчит»
# в самом сообщении (dsh_chain_should_advance, lib/dsh-ci.sh, #877) — решение
# «пробовать следующего» то же самое, текст честнее.
#
