...
#
# DSH_TIMEOUT_SECS за попытку ОДНОГО провайдера (#877 → #1067, инцидент
# 2026-09-13, прогон worker.yml 34735752165): #877/#880 снизили значение
# 9000с → 1200с (20 мин) арифметикой «10 попыток × равный бюджет ≤ бюджет
# job'а» — посылка оказалась ложной. Живой факт: GLM (ПЕРВЫЙ в цепочке) убит
# нашим же таймаутом РОВНО на 1200с (rc=124, «попытка длилась 1200с») —
# провайдер не завис, ему просто не хватило времени на реальную работу.
#
# Замер 14 УСПЕШНЫХ завершений GLM (run id → длительность, секунды, оба
# режима — 9000с-эпоха и 1200с-эпоха, во всех работа шла до естественного
# конца, не до таймаута): 34684847419:1021, 34659778292:4773,
# 34664417551:1050, 34665425472:8345, 34672645510:1653, 34674625124:1224,
# 34675645431:4959, 34679516420:5642, 34683783248:1191, 34685782465:3385,
# 34698299225:1015, 34702891472:700, 34712269623:863, 34728868781:1070.
# Медиана ~1207с, максимум 8345с (139 мин). При пороге 1200с из этих 14
# успели бы только 7 (50%) — работа обрывается монетой, а не исключением.
# Из тех же прогонов ни разу не заработал ни один провайдер, КРОМЕ первого
# (пул/GLM) — остальные либо отказывают за секунды (битый конфиг/квота),
# либо ограничены ОБЩИМ бюджетом RATE_LIMIT (1800с, тратится один раз на
# ВЕСЬ прогон, см. ниже) — «10 попыток × равный бюджет» смешивает РАБОЧИЙ
# провайдер с девятью запасными, которые в каждом сэмпле вели себя иначе.
#
# Уравнение «N провайдеров × T ≤ бюджет job'а» при N=9 (пул+8 записей
# default-chain, ZAI исключён — #1067) не допускает T, покрывающего
# наблюдаемый разброс GLM НИ ПРИ КАКОМ разумном бюджете job'а (даже T=1333с,
# максимум при бюджете 280 мин, закрывает те же ~50% случаев, что и текущие
# 1200с) — жёсткое «все N могут зависнуть одновременно» физически несовместимо
# с «первому провайдеру нужно время на реальную работу», это не вопрос
# калибровки числа. Новая модель принимает частичный, не полный «худший
# случай»: бюджет строится на «до ДВУХ попыток в одном прогоне легитимно
# занимают полный DSH_TIMEOUT_SECS» (первый провайдер завис ИЛИ не успел,
# следующий реально сделал работу — оба случая видны в сэмпле выше), а не на
# «все N зависли разом» — такое не встретилось НИ РАЗУ ни в одном из
# прогонов, по которым есть данные.
#
# Новое значение — 7200с (120 мин): покрывает 13 из 14 замеров (92.9%,
# промах только на самом длинном — 8345с). Бюджет (timeout-minutes подрос до
# 340 мин, см. worker.yml): 2 × 7200с (два полных долгих прогона в одном
# запуске) + 1800с (общий бюджет RATE_LIMIT, тратится РОВНО один раз, не за
# провайдера) = 16200с (270 мин) из 17400с (340 мин минус те же ~50 мин
# оверхеда на установку/git/отчёт, что были приняты ещё в #877) — 1200с
# (20 мин) запаса на неучтённые быстрые отказы остальных ~7 записей цепочки
# (каждая наблюдалась падающей за секунды, не минуты).
#
# Принятый остаточный риск (не подтверждено — гипотетический, не измеренный
# сценарий): ТРЕТЬЯ попытка в том же прогоне, тоже занявшая полный
# DSH_TIMEOUT_SECS, бюджет выше не покрывает — job упрётся в свой собственный
# timeout-minutes (340) и будет убит GitHub'ом ДО отчёта. Это громкий, а не
# тихий отказ (прогон виден как cancelled/timed_out в Actions) и дальше
# ловится существующим рипером зависших прогонов (WORKER_STALL_MINUTES,
# scripts/orchestra/scheduler.py) — задача возвращается в пул, а не висит на
# 24-часовом TTL. Такой сценарий (3 полных таймаута в одном прогоне) не
# встретился НИ РАЗУ ни в одном из наблюдаемых прогонов — если он когда-нибудь
# случится, это новый факт для пересчёта, не повод считать текущий выбор
# сломанным заранее.
#
# rc=124 (наш таймаут) отделён от «провайдер молчит» в самом сообщении
# (dsh_chain_should_advance, lib/dsh-ci.sh, #877) — решение «пробовать
# следующего» то же самое, текст честнее.
#
...
# blocked) → зелёный; цепочка провайдеров отказала (квота/лимит/любой отказ
# всех записей, #1084) → job красный, но
...
# Гвардия дублей прогонов — СЛОТ-ОСОЗНАННАЯ (#827, блокирующая находка
# ai-review PR #831). Раньше она перечисляла ВСЕ прогоны worker.yml по
# репозиторию и выходила no-op при любом чужом id. С N=2 это обнуляло фичу
# целиком: планировщик адресует второй диспатч в свободный слот ровно тогда,
# когда первый занят — то есть второй прогон в 100% случаев видел прогон
# первого слота и умирал зелёным no-op ПОСЛЕ полного checkout/setup/auth.
# Молча: отчёт пульса писал «worker.yml запущен адресно», прогон зелёный,
# работы нет, раннер-минуты сожжены.
#
# Теперь дублем считается только прогон ТОГО ЖЕ слота. Слот читается из
# run-name (`worker (slot N)`, см. .github/workflows/worker.yml) — тем же
# признаком, что и зеркало scheduler.py::_run_slot; прогон без метки слота
# (ручной dispatch без inputs.slot) считается слотом 1 там и тут.
#
# Дубль РАБОТЫ над одной задачей (в отличие от дубля прогонов) закрывает
# атомарная аренда #121 — claim ниже; эта гвардия про бессмысленный второй
# прогон В ОДНОМ слоте.
if [ "$DRY_RUN" != "1" ] && [ -z "${WORKER_SKIP_DUPGUARD:-}" ]; then
  my_slot="${WORKER_SLOT:-1}"
  others=""
...
"
  ...
  while IFS="$(printf '\t')" read -r id title; do
    [ -z "$id" ] && continue
    [ "$id" = "$mine" ] && continue
    # Слот из run-name; без метки — слот 1 (зеркало scheduler.py::_run_slot).
    slot=$(printf '%s' "$title" | sed -n 's/.*(slot \([0-9]\{1,\}\)).*/\1/p')
    [ -z "$slot" ] && slot=1
    [ "$slot" = "$my_slot" ] && dup="$dup$id "
  done <<EOF_RUNS
$others
EOF_RUNS
  if [ -n "$dup" ]; then
    echo "Живёт другой прогон воркера В ТОМ ЖЕ слоте $my_slot (id: $dup) — выхожу no-op, чтобы не делать ту же работу дважды (#121, #827)."
    exit 0
  fi
fi
...
# warning (#572, было exit 1 = «job красный»): контракт исхода запуска в шапке
# файла (см. «Итог запуска» выше) морды как условие красного не называет, а
# связка «морда легла → красный» этому контракту противоречила де-факто.
# Живая улика #572: 500 «Internal runtime error.» из dsh-edge
# `appendHarnessEvents` (dsh-edge/patches/0004-harness-ingest.patch —
# `openAgentForTurn` внутри дописывания в чужую сессию тянет полный
# агент/провайдера ради текстовой строки) не про задачу воркера, а про
# недоступность ВТОРИЧНОЙ витрины — но роняла ПЕРВИЧНЫЙ результат (PR так и не
# пытались открыть, если ложились тут). DSH_EDGE_MORDA_AVAILABLE — единственный
# флаг, которым весь остальной блок узнаёт, включать ли шаги морды дальше;
# работа над задачей идёт независимо.
HARNESS_SID="harness-$number"
HARNESS_TITLE="#$number: $title"
DSH_EDGE_MORDA_AVAILABLE=1
# Реально использованный id может отличаться от HARNESS_SID (#809: фоллбэк на
# испорченной холодной загрузке, session.create ИЛИ session.rename, #871) —
# читаем возврат функции, не подставляем исходный HARNESS_SID вручную (находка
# ревью PR #638, второй раунд: >/dev/null здесь уже один раз возвращал именно
# этот антипаттерн — на испорченной сессии успешный фоллбэк на новый id тихо
# адресовал бы дальнейший дрен транскрипта на несуществующий/исходный id).
# Отказ морды не путается с отказом чтения ответа: при DSH_EDGE_MORDA_AVAILABLE
# пустом ниже id не используется вовсе (дрен/verify пропускаются целиком).
HARNESS_SID_ACTUAL="$HARNESS_SID"
...
elif ! HARNESS_SID_ACTUAL=$(dsh_edge_session_begin "$HARNESS_SID" "$HARNESS_TITLE"); then
...
# реальная работа (dsh_run_with_pool_then_chain выше) уже отработала — топить
# успешный результат из-за витрины было бы ровно тем самым «морда роняет
# исполнителя», ради чего и заведена эта правка (см. обоснование в разделе 5b
# выше). Диагноз плагина стрима ([ -f "$SPOOL_FILE" ]) — от доступности морды
# НЕ зависит (находка ревью PR #638): плагин мог не сработать даже при живой
# морде, и это отдельная, местная поломка (dsh-hands-streamer), не витрина —
# гейтить её на DSH_EDGE_MORDA_AVAILABLE значило бы прятать поломку плагина
# именно тогда, когда морда и так недоступна.
if [ "$rc" -eq 0 ]; then
...
if [ -n "$DSH_EDGE_MORDA_AVAILABLE" ]; then
  ...
  # «Ноль событий при успешном прогоне» различает «морда есть, приём сломан»
  # от «морды нет вовсе» (см. AGENTS.md, «Алерт не гадает») — при недоступной
  # морде ноль событий ожидаем и не диагностичен, гейт имеет смысл только
  # когда сама доставка отработала (DSH_EDGE_MORDA_AVAILABLE ещё не сброшен
  # веткой else выше).
...
# Цепочка провайдеров отказала — квота (#422), бюджет RATE_LIMIT или
# исчерпание всей цепочки (#727/#797; с #1084 исчерпание — универсальный
# исход отказа: конкретная причина — от STREAM_CLOSED до credentials —
# называется в $reason, обёртка её не угадывает) — не сбой
...
(снимет и протухший замок, и назначение), либо сними аренду вручную:
python3 scripts/lib/claim_task.py release-full $number — снимает только СВОЮ
аренду (замок + назначение); чужой/неизвестный держатель откажет, назвав
держателя (#1190). Свой же замок с прошлого умершего прогона у нового прогона
держателем не совпадёт (новый run id / путь дерева) — тогда release $number
--force, явный обход проверки владения. Хвосты логов ниже
