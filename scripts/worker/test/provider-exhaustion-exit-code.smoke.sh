...
# #1500: сводка обязана быть СОВМЕСТИМА с нулём флага. Прежняя («6 из 8 без
# настоящей попытки») ему противоречила: шесть неопробованных — это как раз
# «повтор имеет смысл», флаг при них равен единице. Стенд кормил состояние,
# которого прод не порождает, и потому не ловил бы расхождение текста с
# разбором. Форма взята из настоящей сводки (_dsh_chain_report_exhausted).
export WORKER_CHAIN_OUTCOME_SUMMARY="реально без квоты: 0 из 2; не пробованы по-настоящему (наш бюджет ожидания исчерпан): 0; мёртвая конфигурация (нет секрета/неподтверждённый id/снятая моделью): 0; ключ отвергнут провайдером: 2; форма запроса отвергнута: 0; причина не установлена (404 — модель или маршрут, #1494): 0; причина не установлена (класс отказа не распознан, #1500): 0; транзиентных отказов: 0"
...
# #1500: «исчерпана целиком — все реально без квоты» здесь больше не
# утверждается. Ноль флага теперь значит и мёртвые ключи, и отвергнутую форму
# запроса, и неустановленную причину — при любом из них утверждение про квоту
# было бы ложью, уходящей человеку. Проверяется то, что осталось верным и
# обязательным: текст называет РАЗБОР по классам и не обещает повтора.
grep -qF "ключ отвергнут провайдером: 2" "$CALLS" \
  || fail "1) в комментарии нет разбора по классам — владелец не узнает, что чинить (#1307/#1500): $(head -8 "$CALLS")"
grep -qF "НЕ объявлен полезным" "$CALLS" \
  || fail "1) комментарий не говорит, что повтор не объявлен полезным (#1500): $(head -8 "$CALLS")"
if grep -qF "все реально без квоты" "$CALLS"; then
  fail "1) комментарий утверждает «все реально без квоты» при нулевом флаге — квоты в этом сценарии нет вовсе, это ложь человеку (#1500): $(head -8 "$CALLS")"
fi
