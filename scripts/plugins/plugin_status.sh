...
#
# Правило fail loud: финальный статус (ready/failed) обязан доехать до журнала —
# иначе скрипт выходит с ошибкой и красит job (деплой без статуса = silent-wrong).
# Промежуточные статусы (deploying) ретраятся, но job не красят.
#
...
max_seq=0
while :; do
  resp=$(api "$HARNESS_URL/api/events?task_id=$TASK_ID&after=$after&limit=256")
