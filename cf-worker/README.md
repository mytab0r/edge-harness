...
все открытые сессии браузеров. `GH_DISPATCH_TOKEN` — узкий fine-grained PAT
(Contents+Actions RW только на этот репозиторий, [ADR 0008](../docs/decisions/0008-narrow-dispatch-token.md));
широкий PAT конвейера живёт отдельно (репо-секрет `GH_PIPELINE_PAT`) и в воркер
не попадает.
