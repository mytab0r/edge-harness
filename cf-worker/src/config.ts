...
export const AUTOMATIONS = {
   ...
   *  ТЕМП повторов, а не разрыв цикла (связка trigger=journal + task=pool
   *  отклоняется отдельно на PUT, см. parseAutomationConfig — кулдаун её не
   *  лечит: цикл длиннее кулдауна кулдаун вообще не режет). Каденс — как у
   *  пульса, которым тикают и расписания. */
  journalCooldownMs: 30 * 60_000,
  /** Троттлинг записи отказа неаутентифицированного webhook'а в журнал: не чаще
   *  одного события на reason за окно, счётчик проглоченных — в data (находка
   *  AI-ревью PR #241, класс силового расхода квоты строк журнала — #320). */
  webhookRejectThrottleMs: 5 * 60_000,
