...
const spec = {
  ...
  "routes": [
    ...
    },
    {
      ...
      "auth": true,
      "summary": "Готовность хранилища DO SQLite (#575): реальный SQL-раундтрип (не /api/status — тот кэширован и не выполняет живой запрос на каждый вызов). 200 {ok:true} — хранилище отвечает; отказ (например, исчерпание суточной квоты rows_read/rows_written) — тот же storage_quota_exceeded/internal, что и storageErrorResponse на любом другом маршруте."
    }
  ]
};
