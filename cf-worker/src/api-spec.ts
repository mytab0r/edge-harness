...
const spec = {
  ...
  "routes": [
    ...
    },
    {
      ...
      "auth": true,
      "summary": "Разбор новых сообщений: классификация (directive/chat/doc_edit/raw), группировка; для директив и doc_edit — issue под GH_ISSUES_TOKEN (kind в теле issue; не задан токен или сеть — повтор до LIMITS.messageMaxAttempts, потом честный failed; raw уходит в ignored на ручной триаж). Тело {limit, retry_failed: true} — вернуть failed в new с обнулёнными попытками. Возвращает {processed, results}. Тот же разбор ведёт пульс DO (alarm) — ручной вызов не обязателен."
    },
    {
      "name": "automations",
      ...
      "auth": true,
      "summary": "Список автоматизаций (#116) с конфигом, last_fired_ts и последним прогоном (задача со статусом из очереди). Форму конфига валидирует src/automations.ts; секция «Автоматизации» морды читает это через прокси /api/harness/*."
    },
    {
      "name": "automation",
      "path": "/api/automations/",
      ...
      "auth": true,
      "rest": true,
      "summary": "PUT — создать/заменить автоматизацию {automation_id} (тело {config}, жёсткая валидация, потолок LIMITS.automationsMax); DELETE — удалить. Запись конфига не запускает прогон — прогон порождают только триггеры."
    },
    {
      "name": "webhook",
      "path": "/api/webhooks/",
