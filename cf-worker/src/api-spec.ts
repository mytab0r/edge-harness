...
const spec = {
  ...
  "routes": [
    ...
    },
    {
      ...
      "auth": true,
      "rest": true,
      "summary": "Финализация сообщения оркестратором (Bearer HANDS_TOKEN): CAS-запись результата по claimed_ts (processing_ts). Тело: {status: \"done\"|\"failed\", result: object, claimed_ts: number}. Возвращает {accepted: boolean, action: \"finished\"|\"failed\"|...}. CAS защищает от двойного ответа: если ватчдог уже вернул сообщение в new и его забрала другая проходка, наша запись не проходит — accepted: false без ошибки, джоб молчит (спека orchestrator-dispatch требование 4)."
    }
  ]
};
