...
export class Harness extends DurableObject<Env> {
   ...
   * новый не заводим: он уже воркер-секрет, недоступный агентам, и уже
   * синхронизирован в GitHub Actions repository secret тем же именем
   * (deploy-worker.yml/telegram-webhook.yml), workflow может проверить им же
   * значением. Подписывается СОДЕРЖИМОЕ `${issueNumber}:${option}` — подпись
   ...
   * «РЕШЕНИЕ: N» — не другое решение, а дубль комментария; риск «протухшей
   * кнопки другого раунда» (та же пара issue+option значит другое в новом
   * раунде) — существующий пробел apply_owner_decision.py, закрыт отдельно
   * проверкой issue_still_waiting, вне области этой подписи. Секрет не
