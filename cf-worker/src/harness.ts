...
const SCHEMA = [
  ...
  // значений (design.md §2; терминальность не хранится битом здесь — она
  // атрибут ЗНАЧЕНИЯ stage, не отдельная колонка, которая могла бы
  // разойтись с ним. Сегодня терминальность в коде — два рукописных сета
  // PR_STAGE_TERMINAL/TASK_STAGE_TERMINAL (scheduler.py:393,402), читателей
  // из этой схемы у неё нет; is_terminal(stage) как одно место правды
  // вводит Этап 4 orchestrator-core-v2 (п.45 дельта-спеки). Имя STAGE_META,
  // стоявшее здесь раньше, фантомно — в дереве его никогда не было,
  // находка ревью PR #1290).
...
export class Harness extends DurableObject<Env> {
  ...
  #tickPulseAlert(now: number, lastPulse: PulseStatus): void {
    ...
    if (decision === "incident") {
      void this.#telegramApi("sendMessage", {
        chat_id: this.env.TELEGRAM_CHAT_ID,
        // «Пульс не тикает» — поломка: само не починится, пока DO не проснётся
        // (реестр категорий, #1461). Тот же класс события, что и алерты
        // хранилища в #tickStorageReadyAlert выше.
        text: decorateAlert("breakage", pulseAlertText(lastPulse)),
      });
    } else if (decision === "recovery") {
      void this.#telegramApi("sendMessage", {
        chat_id: this.env.TELEGRAM_CHAT_ID,
        text: decorateAlert("breakage", "✅ edge-harness: пульс оркестратора снова в норме"),
      });
    }
  }
   ...
   * (ADR 0015, заменяет ADR 0011): job .github/workflows/inbox-issue.yml
   ...
   *  сбоя dispatch'а или упавшего job'а inbox-issue). processed_ts тоже чистится
   *  здесь (находка ревью PR #586): без этого строка возвращалась в 'new' со
   *  старым processed_ts, и инвариант «processed_ts ≠ NULL ⇒ терминальный статус»
   *  (на нём держится фильтр по status в ретеншене, 14.10) переставал быть
   *  структурным — держался бы только фильтром, без обнуления здесь. */
