...
- Нативная Merge Queue — недоступна, не пересматривается здесь. Обоснование —
  отдельный PR #340 (не смёржен на момент постановки этой задачи; тот же
  принцип форвард-ссылки, что у research/23 выше, см. issue #341). Номер ADR
  здесь намеренно не назван: PR #340 предлагает его как «ADR 0012», но в main
  этот номер уже занят другим решением (`docs/decisions/0012-orchestra-event-
  trigger-merge-loop.md`) — вопрос о нумерации решает PR #340 при слиянии,
  фиксировать здесь устаревший номер опаснее, чем не называть его вовсе.
