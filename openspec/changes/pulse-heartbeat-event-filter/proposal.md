...
(Переписано под финальную схему — находка AI-ревью PR #318, третий раунд:
прежняя редакция этой секции описывала дизайн ПЕРВОГО раунда, который
второй раунд ревью сам же отверг как недостаточный при truncation, ≥100
`contract`-прогонов между тиками; читатель при архивации change'а иначе
восстановил бы отброшенный дизайн как задуманный.)

- `pulse_guard.orchestra_tick_runs(repo, per_page=100)` — РЕШАЮЩАЯ,
  серверная фильтрация: отдельный запрос `?event=...&per_page=100` на
  каждое легитимное значение `ORCHESTRA_TICK_EVENTS` (`schedule`,
  `workflow_dispatch`), результаты слиты и отсортированы. Страница на
  каждый запрос не тратится на `contract`-прогоны — GitHub фильтрует их
  до пагинации, не после, поэтому труncation при ≥100 `contract`-прогонов
  между тиками невозможна структурно.
- `pulse_guard.real_orchestra_ticks(runs)` — чистая функция-фильтр,
  allowlist по той же константе `ORCHESTRA_TICK_EVENTS` (не denylist
  `!= "pull_request"` — находка ревью PR #318, второй раунд), остаётся
  как вторая линия защиты и чистый юнит, тестируется отдельно от IO.
- `heartbeat_check()` вызывает `orchestra_tick_runs`; пустой результат
  ПОСЛЕ серверного фильтра (оба события отдали 0 success) — самостоятельный
  тревожный случай: кричит `🚨 HEARTBEAT_NO_TICKS` (Telegram на каждый
  прогон, комментарий в задачу — один на эпизод), а не молчит `ℹ️`.
- Тест на прод-форме данных (реальные таймстампы и id из замера 2026-09-05),
  доказан мутацией (снял фильтр → тест покраснел).

**Сделано этим же PR (находка ревью #318, дешёвая часть исходной задачи
#133):** `plugins-src/runner-bridge/server/index.js` — описания
`runner_task`/`runner_status` теперь честно называют известное ограничение
(issue #133: egress воркера иногда получал 403 от `api.github.com`), а
`describeFailure` при 403 без JSON-тела возвращает агенту именно это
объяснение вместо голого «HTTP 403», чтобы агент говорил пользователю
правду сразу, а не после того, как выдал делегирование как гарантию. Это НЕ
диагностика причины и НЕ обход маршрута (оба остаются в «Что вне рамок»
ниже) — только честное описание известного ограничения в самих
инструментах, третий из трёх пунктов исходной задачи. Первые два пункта не
выполнены — #133 этим PR **не закрывается**.

**Честно о доставке (находка AI-ревью PR #318, второй раунд):** правка
`server/index.js` меняет ИСХОДНИКИ плагина `runner-bridge`
(`plugins-src/runner-bridge/`, версия поднята до `0.1.2` в `package.json`/
`PLUGIN_VERSION`). До морды dsh-edge этот текст не доедет этим PR —
`dsh-edge/plugins.json` (манифест, по которому идёт деплой) всё ещё пинит
`runner-bridge-0.1.1.tgz`, и обновление пина требует отдельного прогона
`plugin-forge.yml` (собирает tarball, публикует релиз, открывает свой
авто-PR с новым sha256) и слияния ЭТОГО авто-PR. Видимый результат, что
доставка состоялась: в логе `deploy-dsh-edge.yml` после мержа авто-PR
качается `runner-bridge-0.1.2.tgz`, не `0.1.1.tgz`.
...
- **Обход «морда dsh-edge → cf-worker → GitHub»**, названный в #133 как
  ...
  IP/colo-зависимый и мигрирующий — docs/research/21, раздел «GitHub API из
  egress Cloudflare Workers: измеренный блок 403», строки 166-171 и 444;
  было указано research/30 — находка ревью PR #318, п.3, там про этот блок
  ничего нет); 2) живой замер этой
