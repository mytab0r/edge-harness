...
## Правка после слияния (issue #1074, 2026-09-13)

Аудит 2026-09-13 нашёл, что пп. 3 («trusted_login» в инварианте 16) и «Гейт
прод-записи» выше решали свой вопрос неполно: `trusted_login` в
`repo_invariants.fetch_wip_gate_markers` защитил решение гейта, но сделал
наблюдательный инвариант слепым к чужеродным маркерам, а
`SCHEDULER_ALLOW_PROD_WRITES=1` продолжал разрешать писать в тот же канал
вне CI. Живой случай: 26 ложных close-маркеров WIP-гейта в #120 за ночь
2026-09-12/13 (после мержа этого change) остались незамеченными инвариантом
16 именно из-за первого решения, и появились именно из-за второго. Обе
правки — в дельта-спеке (`specs/journal-tasks-hands/spec.md`, разделы
«ПРАВКА (issue #1074, 2026-09-13)»), код — issue #1074.
