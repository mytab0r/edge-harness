...
1. Прямое чтение `/proc/<pid-dsh>/environ` — в живом замере запрещено (все
   предки job-цепочки нечитаемы). **Механизм запрета не атрибутирован**
   (контрольный эксперимент: same-uid не-потомок environ читает, так что yama
   как объяснение не годится) — это свойство среды раннера, не гарантия ядра;
   изоляция на него не опирается, зонд prepare предупреждает об изменении.
   Вектор из названия задачи **не подтвердился**.
