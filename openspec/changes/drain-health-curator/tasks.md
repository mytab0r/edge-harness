...
- [x] pm не трогает: задачи/PR с исполнителем, `waiting:owner`, `blocked`
      ...
      граница, что pm.md); исполнитель/`waiting:owner` вдобавок детектируются
      постфактум по событиям `issues/events`
      (`pm_dispatch.py::find_boundary_violations`, находка ревью PR #870 —
      красит шаг с номерами, направление fail-loud); `blocked` и «ветка
      моложе явного порога» остаются только словом — без версионированного
      числового порога постфактум-проверка невозможна (честная граница
      design.md §2.3); фактическое соблюдение AGENT'ом внутри headless-сессии
      этими проверками НЕ предотвращается — они детекция, не предотвращение.
- [x] Громкая гвардия усечения окна проверки: лимит страниц `issues/events`
      исчерпан, а самый старый прочитанный новее начала окна — код 2
      «проверка не состоялась», не зелёный (находка ревью PR #870;
      test_check_closures_loud_when_window_truncated, доказано снятием
      гвардии).
- [x] Гвардия повторного диспатча pm.yml: маркер идемпотентности не
      сохранился (сбой поста после успешного POST), но прогон pm.yml уже
      стартовал после открытия эпизода — повтор не выполняется, маркер
      восстанавливается (`pm_groom_run_started_after`, находка ревью PR #870;
      test_dispatch_pm_groom_skips_when_run_already_started_after_episode,
      доказано снятием гвардии).
