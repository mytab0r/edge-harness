...
## Превентивный рефреш долгоживущих токенов (#1130, решение владельца)

**Вопрос владельца дословно:** «Какой опус он там пытается рефрешить, если
мы поставили долгоживущие OAuth токены, которые не надо рефрешить?»
...
**Рассмотренный, отвергнутый вариант:** проставлять `expiresAt` при
экспорте секрета (`anthropic_oauth_import.py` уже умеет копировать это поле
из krouter-дампа, если оно там есть). Отвергнут владельцем-оркестратором —
меняет контракт секрета и требует РУЧНОГО перевыпуска `ANTHROPIC_OAUTH_1/2`,
то есть участия владельца, которого этот класс правок избегает.
