...
- [ ] Владелец кладёт `ANTHROPIC_OAUTH_1`/`ANTHROPIC_OAUTH_2`
      ...
      `worker.yml`/`hands.yml`/`ai-review.yml`: в логе видно монтаж
      `anthropic-oauth-pool`, импорт аккаунтов, попытку пула ПЕРВОЙ; при
      успехе — `DSH_CHAIN_PROVIDER=anthropic-oauth-pool` в отчёте прогона.
      Без секретов эта проверка невозможна — не входит в критерий готовности
      самого PR (Scope/Out), фиксируется отдельным комментарием в #838 после
      того, как секреты появятся.
