...
- [x] Итерация #1383 — третья поддержанная форма `.sh` (+ файлы без расширения по
      шебангу): направление выбрано ЗАМЕРОМ живого прогона 35491299405 (из семи
      отказов по типу файла `.sh` дал три, названная в постановке «дешёвой»
      гипотеза «без расширения» — один). Критерий симметричен питоновскому
      (`bash -n` каждой стороны + непересечение верхнеуровневых имён в нулевой
      колонке; тот же класс по итоговому файлу). Четыре рубежа heredoc'а
      (блокирующие находки ai-review PR #1392, раунды 1-4): конфликт внутри
      открытого heredoc'а, сторона-открыватель, хвост итогового файла и громкий
      отказ на неразобранной форме `<<`; закрытие и разбор ограничителя — по
      правилам настоящего bash, каждое утверждение проверено прогоном. Честная
      граница (у `.sh` нет третьего рубежа — соглашения «сосед-тест» для шелла
      нет) ПЕЧАТАЕТСЯ в лог и защищена отдельным тестом. Дельта-спека несёт эту
      итерацию требованием, а не умолчанием (блокирующая находка того же ревью,
      раунд 4: спека заявляла «поддержаны только `.py`/`.md`»).
- [ ] Остаток отказов по ТИПУ файла после итерации #1383: `.yml` ×2 и `.json` ×1 из
      того же замера (прогон 35491299405) НЕ закрыты сознательно — у YAML/JSON
      «верхнеуровневое имя» это ключ отображения, и правило непересечения там своё,
      механического переноса критерия шелла не существует. Хвост не имеет права
      испариться вместе с архивацией этой дельта-спеки (некритичная находка ai-review
      PR #1392, раунд 11): после слияния — НОВАЯ узкая задача со ссылкой на #1383,
      критерий её завершения — в очереди конфликтов исчезают отказы «тип файла
      .yml/.json не поддержан» на живых PR, замер тем же прогоном.
- [ ] Замер «после» по итерации #1383 — на первом же прогоне
      `conflict-mechanical-rebase` после слияния привести ОБА числа: «N сведены
      аддитивным слиянием» И новый состав причин отказа. Одного числа мало:
      отказ по типу файла был ПЕРВЫМ, за ним у тех же PR могут стоять другие
      причины (AGENTS.md, «Проверяй видимый результат, а не шаг»).
