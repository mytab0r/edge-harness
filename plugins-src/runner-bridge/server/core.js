...
// GitHub REST отклоняет запрос без User-Agent кодом 403 без JSON-тела
// (проверено живым curl 2026-09-11, issue #133) — это и есть причина 403,
// не блок egress-IP датацентра. Тот же класс, что и `GITHUB.userAgent` у
// cf-worker (cf-worker/src/config.ts) — своя константа здесь, а не импорт
// оттуда: runner-bridge собирается отдельным пакетом плагина, без общей
// зависимости с cf-worker (см. докстринг файла выше про peer-зависимости).
const GITHUB_USER_AGENT = 'edge-harness-runner-bridge'
 ...
 *
 * Причина 403 из issue #133 установлена и закрыта (2026-09-11, живой curl):
 * это не блок egress-IP, а стандартный отказ GitHub REST на запрос без
 * `User-Agent` — `githubFetch()` теперь всегда его ставит (см. GITHUB_USER_AGENT
 * выше). Ветка ниже остаётся на случай ДРУГОГО источника голого 403 без тела
 * (например, отдельный админ-блок GitHub на сам токен/IP) — если она сработает
 * вновь после фикса, гипотеза про User-Agent тут ни при чём, ищи заново. */
export async function describeFailure(response) {
  ...
  if (response.status === 403 && detail === '') {
    return 'HTTP 403 без сообщения от GitHub — не известный класс (issue #133 закрыт '
      + 'отсутствовавшим User-Agent, он теперь всегда ставится); причина новая, не отказ доступа токена.'
  }
    ...
    if (response.status !== 201) {
      ...
      return {
        ...
        message: `runner_task: GitHub не создал задачу (${reason}). `
          ...
          + 'сейчас недоступно. Если причина похожа на временный сбой '
          + 'сети/лимита GitHub — повтор уместен; если на отказ токена/прав '
          + '(403/401 с сообщением) — повтор не поможет, нужен владелец.',
      }
    }
