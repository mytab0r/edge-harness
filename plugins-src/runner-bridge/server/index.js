 ...
 * мог мигрировать или исчезнуть (docs/research/30). Не подтверждено, что он
...
async function describeFailure(response) {
  ...
  if (response.status === 403 && detail === '') {
    return 'HTTP 403 без сообщения от GitHub — похоже на известный блок egress-IP воркера '
      + '(issue #133, не переизмерен, мог исчезнуть или мигрировать), а не на отказ доступа '
      + 'токена. Скажи пользователю честно: делегирование сейчас недоступно из-за '
      + 'инфраструктурного ограничения, не из-за его запроса; повтор вряд ли поможет.'
  }
