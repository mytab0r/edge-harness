...
## Третий потребитель: роль владельца с внешними интеграциями

Широкий PAT используется и для **операций вне GitHub**, выполняемых в деплое
морды. Пример: секрет вебхука Telegram — его генерирует job в workflow деплоя,
используя широкий токен для синхронизации в Cloudflare. Это вне репозитория и
вне данного ADR (решение владельца, не конвейера), но объясняет, почему сузить
токен до `Contents:write` на самом деле нельзя: есть и другие потребители (не
внутри репо). Узкий `GH_DISPATCH_TOKEN` закрывает только dispatch-путь морды;
все остальные операции остаются на широком `GH_PIPELINE_PAT` с дополнительным
контролем (см. гвардия узости).
