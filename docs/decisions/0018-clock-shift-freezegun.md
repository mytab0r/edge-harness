...
`test_clock_shift_suite.py` — доли секунды) дублирован шагом в `repo-ci.yml`
(находка второго гейта ревью PR #667: без этого шага правка `conftest.py`
проходила `test` без единой проверки, поломку ловил только завтрашний
красный прогон) — freezegun ставится и там, но только как зависимость
этого узкого шага, не всего прогона `scripts/`.
