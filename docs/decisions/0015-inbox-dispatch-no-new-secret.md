# ADR 0015. Инбокс создаёт issues репозиторным dispatch'ем в свой job — без нового секрета
...
4. `claimed_ts` — момент атомарного захвата сообщения, эхо через
   ...
   перезаписывает чужой результат. Граница обещания — CAS защищает журнал
   DO, а не факт создания issue (см. «Последствия»).
...
- CAS по `claimed_ts` защищает только журнал DO (перезапись `result`), не
  факт создания issue: старт job'а верхней границей не ограничен (хвост
  очереди Actions, `docs/research/21-github-actions.md` — «изредка часы»).
  Если ватчдог (`messageStuckProcessingMs`) уведёл сообщение в `new`, а
  первый job всё же запустился и создал issue, ретрай уедет вторым dispatch'ем
  и создаст вторую — задвоение в этом окне возможно, дедупа на `message_id`
  нет; единственный след дубля — `accepted: false` в логе старого job'а.
