...
Внешний webhook автоматизации (#116). Аутентификация — не кука/Bearer, а подпись X-Harness-Signature: sha256=<hex HMAC-SHA256(raw body, AUTOMATION_WEBHOOK_SECRET)>. Без подписи или с неверной — 401 и событие automation_webhook_rejected в журнале (громко). Валидная подпись + включённая автоматизация → repository_dispatch harness-automation, 202 {task_id}; секрет не задан — 500, автоматизация выключена — 409.
