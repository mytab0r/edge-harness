...
**Почему аккаунт может «работать у владельца» и не работать в CI (#1311).**
Anthropic **ротирует** refresh-токен: на каждом успешном рефреше выдаётся
новый, и плагин его сохраняет (`refreshToken: next.refresh_token ||
oauth.refreshToken`, `lib/pool.js`). Живая сессия Claude Code на машине
владельца рефрешится своим чередом и уводит токен вперёд, а в секрете
`ANTHROPIC_OAUTH_<slot>` лежит **снимок**, сделанный в момент экспорта. Как
только живой клиент уходит дальше снимка, снимок перестаёт приниматься —
аккаунт при этом совершенно жив, «не работает» только наша копия. Признак в
логе: `аккаунт anthropic-N: REFRESH_FAILED`. Лечение — тот же свежий экспорт
выше; снимать его лучше непосредственно перед импортом, а не «когда-нибудь
раньше».
