...
- [28. Цепочка провайдеров: замер против диагноза](research/28-provider-chain-truth-table.md) —
  четыре записи цепочки из восьми отвечают HTTP 200 прямым вызовом и те же четыре не отвечают
  через `dsh`: отказывает путь вызова, не провайдеры. Плюс два вердикта, названных неверно
  (429 назван `AUTH`, живой 200 назван `dead_model`) — класс «алерт не гадает».
