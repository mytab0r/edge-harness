...
- [40. Утечка ключа через model-shell](research/40-model-shell-key-exposure.md) — замер
  #140: docker-эскейп читал ключ живьём; в изолированной конфигурации точная пара
  model-shell→environ dsh READABLE (замер 5) — закрытие: ключ вообще не живёт в env
  dsh (локальный прокси-держатель в домене транспорта) + отдельный uid без docker.
