...
- [26. Bot API: темы в личке](research/26-telegram-bot-api-threads.md) — замер #1463:
  темы в личном диалоге с ботом работают целиком (create/send/edit/unpin/delete),
  переключатель в @BotFather не понадобился, tdlib/telegram-bot-api#847 не
  воспроизвёлся. Дословный вывод прогона + перезапуск одной командой.
...
- [28. Цепочка провайдеров: замер против диагноза](research/28-provider-chain-truth-table.md) —
  пять записей цепочки из восьми отвечают HTTP 200 прямым вызовом и те же пять не отвечают
  через `dsh`: чинить надо путь вызова, а не записи цепочки. Плюс вердикт `dead_model` при
  живом 200 на том же id — класс «алерт не гадает».
