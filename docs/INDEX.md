...
- [33. Пределы параллелизма воркера](research/33-worker-parallelism-limits.md) — N=1
  обоснована GitHub-нативной сериализацией (`concurrency: group: worker`), не только
  квотой; хронология квоты DO rows_read (потолок ~33 прогона/сутки до фикса #1165,
  переизмерение после фикса ещё не снято).
