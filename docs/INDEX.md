...
- [40. Утечка ключа через model-shell](research/40-model-shell-key-exposure.md) — замер
  #140: прямой `/proc/<pid>/environ` не читается в живом замере (механизм запрета не
  атрибутирован), docker-эскейп читал ключ живьём; закрытие — отдельный uid для dsh
  без docker (изоляция адаптера).
