...
финальном коде —
[cf-inventory#34011893138](https://github.com/mytab0r/edge-harness/actions/runs/34011893138),
SHA `669babb`, 2026-09-06: 12/12 секций успешно, включая цикл по
`CF_OWN_WORKERS` (деплои/bindings обоих воркеров, находка ревью PR #328,
п.2) и `permissions: {}`/`persist-credentials: false` (п.3). Более ранний
прогон [#33993667102](https://github.com/mytab0r/edge-harness/actions/runs/33993667102)
(SHA `0b85fd5`) заявлялся как «финальный код», но был недостижим из мерж-хеда
(история переписана после прогона) — не доказывал 151 строку изменений
`scripts/cf/` между деревом прогона и мерж-хедом (находка ревью PR #328, п.1);
более ранние прогоны выполняли код без `cf_bindings_names` и печатали значения
plain_text bindings в лог — их логи удалены (ревью PR #328, находка 1
предыдущего раунда), эта ссылка — единственная действующая.
