...
| 2 | только отсутствующий `HANDS_TOKEN` — единственная ветка скрипта, выходящая этим кодом (проверка до всякой другой работы) | красный, вердикт `deploy` → откат |
...
остаётся виной деплоя (умолчание консервативное нарочно). Код 2 обещает ровно
то, что даёт скрипт, и не больше: любая ДРУГАЯ авария самой канарейки (нет
браузера Playwright, необработанное исключение) этим кодом не выходит — node
завершается с кодом 1, то есть «деплой плохой», и откат остаётся (проверено
живым прогоном скрипта без установленных зависимостей: exit 1).
