...
Признак не выдуман канарейкой: она читает код ошибки, который воркер ставит
сам (`storageErrorResponse` → `storage_quota_exceeded`,
[`cf-worker/src/harness.ts`](../../cf-worker/src/harness.ts)). Зонд
`/api/ready` идёт ПЕРВЫМ, до запуска браузера, — иначе до него не доходит
исполнение: при 500 от API страница не инициализируется, и канарейка умирает
на таймауте локатора, как в прогоне 35639422589 (откатили корректный код,
причиной была исчерпанная суточная квота DO, #1411).
...
**Не подтверждено / честный потолок:** распознаётся ОДИН отказ — тот, что
воркер назвал сам. Страница Cloudflare `error code: 1101` (исключение до
нашего обработчика) причины не несёт и считается виной деплоя, как раньше.
Канарейка `deploy-dsh-edge.yml` устроена иначе и пока не различает исходы —
задача #1440.
