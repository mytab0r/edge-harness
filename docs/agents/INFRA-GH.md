...
| `deploy-worker.yml` | `push` на `main` (`cf-worker/**`, `.github/workflows/deploy-worker.yml`), `workflow_dispatch` | Деплой морды (edge-harness) в Cloudflare: проверка секретов → гвардия актуальности типов → гвардия узости `GH_DISPATCH_TOKEN` → `wrangler deploy` → синк секретов воркера → канарейка UI на проде | `CLOUDFLARE_API_TOKEN`, `CLOUDFLARE_ACCOUNT_ID`, `HANDS_TOKEN`, `GH_DISPATCH_TOKEN`, `SESSION_SECRET`, `GH_ISSUES_TOKEN` (опционален, отсутствие = `::warning::`, ADR 0011) | `HARNESS_URL`, `GH_DISPATCH_TOKEN_KIND` |
