...
- **`scripts/gh/lib.sh`** — `source` в bash-скрипте, вызов `gh_net_setup || exit 1`.
  Перебирает порты `1084/1083/1085`, держит первый рабочий на весь процесс. Если
  `HTTPS_PROXY` уже задан снаружи — не трогает.
...
- **`scripts/gh/queue.py`** — вся очередь открытых PR одной строкой на каждый: метки,
  ...
  молча теряет хвост). Живой прогон, 2026-09-05: 19 открытых PR, из них минимум три с
...
- **`scripts/gh/pr_blockers.py <N>`** — то же самое для одного PR подробнее (state, draft,
  ...
  ```
  PR #262: 'cloud-orchestrator-convergence: спека облачного оркестратора, три канала в один вход'
  ...
  ```
  Оба питоновских скрипта сами перебирают прокси-порты, если `HTTPS_PROXY` не задан снаружи
  (та же логика, что в `lib.sh`, продублирована намеренно — два runtime'а, python и bash, не
  делят процесс; список портов меняется в обоих файлах разом, если сменится).

Все три скрипта на отказе называют причину (нет прокси, gh упал, PR не найден) и не
возвращают пустой/silent-wrong результат.
