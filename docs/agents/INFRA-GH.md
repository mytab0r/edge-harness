...
| `plugin-forge.yml` | `workflow_dispatch` (`plugin_path`, `plugin_id`, `task_issue`) | Форж плагинов dsh-edge: исходники → релиз `plugins-<id>-v<version>` (tarball + checksums) + PR с обновлённым `dsh-edge/plugins.json`. Не деплоит морду — деплой делает `deploy-dsh-edge.yml` после мержа PR | `GH_PIPELINE_PAT`, `HANDS_TOKEN` | `HARNESS_URL` |
