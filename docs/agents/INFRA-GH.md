...
**Логин `mytab0r` в GitHub-событиях** — токен оркестратора и агентов, не действие человека-владельца.
Все события репозитория (коммиты, слияния, метки, комментарии, push) под этим логином — работа автоматизации
([AGENTS.md § Атрибуция событий](../../AGENTS.md)). Владелец не выполняет операции руками: слияния PR делает
оркестратор, закрытие задач и постановку меток делают гвардии и агенты. По логину GitHub НЕЛЬЗЯ отличить
оркестратора от агента и не определить, является ли событие автоматическим. Используй машинные признаки
(переменные окружения `GITHUB_ACTIONS`/`GITHUB_RUN_ID`, поле `actor` в API actions/runs, маркеры в теле комментариев).
Правило: никогда не принимай решения по логину актора.
