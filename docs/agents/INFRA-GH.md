...
## Первым делом — три грабли, на которых теряют время каждый день

1. **Прямые запросы к `api.github.com` рвутся.** Перед любым `gh`/сетевым `git` —
   `export HTTPS_PROXY=socks5://127.0.0.1:1084` (не прошло — `1083`, потом `1085`).
   Не нужно городить это самому — `scripts/gh/lib.sh` (bash) и встроенный автоподбор
   в `scripts/gh/*.py` уже перебирают порты. См. «Инструменты» ниже.
2. **`gh issue view`/`pr view`/`pr create` идут через GraphQL** и падают чаще, чем
...
3. **heredoc в bash-командах блокируется классификатором окружения агента.** Многострочный
...
4. **`python3 ... | gh ...` и любой subprocess, читающий вывод `gh`, падает на кириллице**
...
Единая точка входа в сеть — не городить `export HTTPS_PROXY` в каждой команде вручную.
Список портов — по одному месту правды на рантайм (два рантайма, bash и python, не делят
процесс: `scripts/gh/lib.sh::GH_NET_PROXY_PORTS` для bash, `scripts/gh/net.py::PROXY_PORTS`
для python — оба перебирают `1084/1083/1085`). Имя репозитория скрипты не хранят вовсе:
`gh api` сам резолвит плейсхолдер `repos/{owner}/{repo}/...` в текущий репозиторий.

- **`scripts/gh/lib.sh`** — `source` в bash-скрипте, вызов `gh_net_setup || exit 1`.
  Держит первый рабочий порт на весь процесс. Если `HTTPS_PROXY` уже задан снаружи — не
  трогает. Тем же массивом портов пользуется `scripts/gh/infra_digest.sh` (дайджест граблей,
  печатается при входе в agent-ветку и из `scripts/git/task-branch`, и из доводки уже
  открытого PR в `scripts/worker/task.sh` — единый текст, не вторая копия).
...
Все скрипты на отказе называют причину (нет прокси, gh упал, PR не найден) и не возвращают
...
| Симптом | Смотреть |
|---|---|
| `gh`/`git` виснет или `dial tcp` | Прокси — попробуй порты 1084/1083/1085, `scripts/gh/lib.sh` |
