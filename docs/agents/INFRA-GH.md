...
| `pm.yml` | `workflow_dispatch` (input `reason` опционален) | Автодиспетч роли pm на триаж PR-беклога (#869, `scheduler.py::dispatch_pm_groom`): drain_gate держит тревогу дольше `PM_DISPATCH_AFTER_HOURS` ИЛИ wip_gate держит stuck-эпизод дольше своего порога. DSH headless через тот же `scripts/hands/dsh_task.sh`, что `hands.yml` — closes-only (дубль/устарело/битая связь), потолок закрытий проверяется механически шагом `pm_dispatch.py check-closures` ПОСЛЕ прогона (факт по `issues/events`, не декларация модели) | `GH_PIPELINE_PAT`, `DEEPSEEK_API_KEY`, `NVIDIA_API_KEY`, `HANDS_TOKEN`, `DSH_EDGE_ACCESS_KEY` | `HARNESS_URL`, `DSH_EDGE_URL`, `DSH_PROVIDER_CHAIN`, `DSH_EDGE_MODEL_CATALOG` |
