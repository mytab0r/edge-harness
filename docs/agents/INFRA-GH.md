...
| `automation.yml` | `repository_dispatch` (`harness-automation`), `workflow_dispatch` (`automation_id`) | Руки автоматизаций (#116): конфиг читается из журнала по `AUTOMATION_ID` (одно место правды), исполнение — раннер (`scripts/automations/run.sh`); `kind=hands` работает через `dsh_task.sh`, `kind=pool` заводит задачу пула (`issues: write`), `kind=digest` — только чтение журнала + отчёт в каналы. `concurrency` — по `automation_id`, не по nonce `task_id` (находка AI-ревью PR #241) | `HANDS_TOKEN`, `DEEPSEEK_API_KEY`, `SLACK_BOT_TOKEN` (опционален), `TELEGRAM_BOT_TOKEN`/`TELEGRAM_CHAT_ID` (опциональны) | `HARNESS_URL`, `DEEPSEEK_BASE_URL`, `DEEPSEEK_MODEL` |
