...
| `merge-health-watch.yml` | `schedule` (`17 * * * *`, часовой), `workflow_dispatch` | Детектор регрессии здоровья конвейера, привязанный к слиянию (issue #967, живой корень #878/#937): rolling worker_success_rate «недавнее окно» (6ч) vs «длинный baseline» (72ч) — часы, не сутки, в отличие от суточного снимка `pipeline_health.py`. При регрессии называет ВСЕ PR, слитые в окне атрибуции (12ч), не гадает одного; заводит/комментирует задачу `task`+`area:process` и эскалирует тем же каналом, что предохранитель конвейера (issue #120 + Telegram). Ни одного git push — только чтение Actions/Issues/Search API | `github.token` (issues:write), `TELEGRAM_BOT_TOKEN`, `TELEGRAM_CHAT_ID` | — |
