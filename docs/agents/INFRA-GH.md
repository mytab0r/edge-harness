...
| `deploy-worker.yml` | `push` на `main` (`cf-worker/**`, `.github/workflows/deploy-worker.yml`), `workflow_dispatch` | Деплой морды (edge-harness) в Cloudflare: проверка секретов → гвардия актуальности типов → гвардия узости `GH_DISPATCH_TOKEN` → `wrangler deploy` → синк секретов воркера → канарейка UI на проде | `CLOUDFLARE_API_TOKEN`, `CLOUDFLARE_ACCOUNT_ID`, `HANDS_TOKEN`, `GH_DISPATCH_TOKEN`, `SESSION_SECRET`, `TELEGRAM_BOT_TOKEN`/`TELEGRAM_WEBHOOK_SECRET`/`TELEGRAM_CHAT_ID` (опциональны, #254, ADR 0014 — отсутствие любого закрывает вебхук-путь `::warning::`, деплой не красится) | `HARNESS_URL`, `GH_DISPATCH_TOKEN_KIND` |
