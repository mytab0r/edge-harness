...
|---|---|---|---|---|
| `worker.yml` | `workflow_dispatch` (task опционален) | Автономный воркер: берёт задачу из пула (или конкретную), DSH headless по `WORKER-PLAYBOOK.md`, открывает PR. Один прогон на репозиторий (`concurrency: worker`) | `GH_PIPELINE_PAT`, `secrets[vars.DSH_PROVIDER_KEY_SECRET]` (#716), `HANDS_TOKEN`, `DSH_EDGE_ACCESS_KEY`, `TELEGRAM_BOT_TOKEN`, `TELEGRAM_CHAT_ID` | `HARNESS_URL`, `DSH_EDGE_URL`, `DEEPSEEK_BASE_URL`, `DEEPSEEK_MODEL`, `DSH_PROVIDER_KEY_SECRET` |
| `hands.yml` | `repository_dispatch` (`harness-task`), `workflow_dispatch` (ручной прогон под `manual-<run_id>`) | «Руки» слайса dsh-in-job: исполняет одну задачу из морды, стримит журнал в DO. Аренда задачи — `github.token` (`contents:write`, снимается до старта DSH) | `HANDS_TOKEN`, `secrets[vars.DSH_PROVIDER_KEY_SECRET]` (#716, значение — DEEPSEEK_API_KEY-переменная окружения), `DSH_EDGE_ACCESS_KEY` | `HARNESS_URL`, `DSH_EDGE_URL`, `DEEPSEEK_BASE_URL`, `DEEPSEEK_MODEL`, `DSH_PROVIDER_KEY_SECRET` |
...
| `ai-review.yml` | `workflow_run` (завершение `pr-review`), `workflow_dispatch` (input `pr`) | Гейт 2: AI-ревью диффа. Trust-зона между job'ами: недоверенный DSH-шаг без токенов, доверенный `verdict` — свежая VM, свой чекаут main. Метки `ai:ok`/`ai:changes-requested`/`ai:failed`, тот же вердикт — Commit Status API `harness/ai-review` (#345) | `github.token`, `secrets[vars.DSH_PROVIDER_KEY_SECRET]` (#716 — имя секрета с ключом провайдера объявлено переменной, не жёстко `secrets.DEEPSEEK_API_KEY`) | `DEEPSEEK_BASE_URL`, `DEEPSEEK_MODEL`, `DSH_PROVIDER_KEY_SECRET` |
...
| `deploy-dsh-edge.yml` | `push` на `main` (`dsh-edge/**`, `.github/workflows/deploy-dsh-edge.yml`), `schedule` (`37 4 * * *`, страховка), `workflow_dispatch` (input `version`) | Деплой морды dsh-edge: source-build с плагинами по манифесту (основной путь) либо npm-префаб (fallback при пустом манифесте). Синк `GH_RUNNER_TOKEN` (=`GH_PIPELINE_PAT`) и опциональных секретов интеграций (#115) в воркер dsh-edge | `CLOUDFLARE_*`, `GH_PIPELINE_PAT`, `HANDS_TOKEN` (обязателен при наличии плагинов в манифесте — без него шаг `exit 1`), `DSH_EDGE_ACCESS_KEY`, `secrets[vars.DSH_PROVIDER_KEY_SECRET]` (#716), `JIRA_*`, `CONFLUENCE_*`, `BITBUCKET_*`, `SLACK_BOT_TOKEN`, `TELEGRAM_*` (интеграционные — опциональны, отсутствие не красит деплой) | `HARNESS_URL`, `DEEPSEEK_BASE_URL`, `DEEPSEEK_MODEL`, `DSH_EDGE_PROVIDER_NAME`, `DSH_EDGE_MODEL_CATALOG`, `DSH_PROVIDER_KEY_SECRET` |
| `llm-provider-models.yml` | `workflow_dispatch` (inputs `base_url`, `secret_name`) | Проверка точного id модели у OpenAI-compatible эндпоинта без экстраполяции (#716, `docs/runbooks/switch-llm-provider.md`): ключ передаётся ИМЕНЕМ секрета (`secret_name`), индексация `secrets[inputs.secret_name]` — значение никогда не попадает в inputs/логи, печатается только список моделей | `secrets[inputs.secret_name]` (динамический — любой секрет репозитория, указанный вызывающим) | — |
