...
| --- | --- | --- | --- |
| Nano | 30B общ. / 3B активных (MoE) | не подтверждено | `nvidia/nemotron-nano-3-30b-a3b` (#798, подтверждено 2026-09-09 живым ответом каталога) |
...
- **Naming внутри одного семейства не единообразен — не полагаться на образец соседнего tier.** Super и Ultra несут `nemotron-3-<tier>-...` (тир ПОСЛЕ номера поколения), а подтверждённый (#798) id Nano — `nemotron-nano-3-...` (тир ДО номера поколения, слова переставлены). Экстраполяция id одного tier по образцу другого в этом семействе не гарантирована даже при видимой закономерности — только сверка с `/v1/models` (#737).
...
- **Публичный листинг моделей NVIDIA NIM — поведение изменилось.** Снимок до 2026-09-09: `GET https://integrate.api.nvidia.com/v1/models` без ключа отдавал 451, `build.nvidia.com` — 403. Живая проверка 2026-09-09T16:56:42Z (заголовок `Date` ответа) с той же машины БЕЗ ключа вернула HTTP 200 и полный список из 80 id (см. `scripts/lib/confirmed-provider-models.json`, запись `NVIDIA-nano`, #798) — либо NVIDIA открыла листинг публично между этими датами, либо ответ зависит от исходящего IP/региона (451 — код, обычно означающий гео-ограничение, не отсутствие авторизации), не проверено, какой из двух вариантов верен. Соответствие HuggingFace-slug → managed API id по-прежнему проверяется только запросом к этому эндпоинту; completion-вызов (не листинг) продолжает требовать ключ независимо от этого — не проверялось этим фактом.
