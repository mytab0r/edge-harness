...
Фикс (issue #575, PR — см. ссылку в самом PR, не дублируется здесь второй
копией): `#msgCounts()` — тот же кэш-геттер, что `#taskCounts()`, с
...
разбор `harness.ts`, ловит любой новый `GROUP BY` вне кэш-геттера ДО того, как
он снова начнёт жечь квоту (мутационно доказано: инлайн `GROUP BY` обратно в
`#status()` красит гвардию).
