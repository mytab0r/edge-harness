...
## API v4: что реально отдаёт токен репозитория (прогон 2026-09-08, issue #322)

Проверено живыми GET-запросами в CI ([запуск на финальном коде](https://github.com/mytab0r/edge-harness/actions/runs/34176257689),
SHA `86c6499`, 2026-09-08, 12/12 секций успешно), не по докам. **Это
...
обновления этой ссылки (перегонялся дважды: после правки heredoc «Расход по
квотам» в status.sh — прогон `#34173437319`/`3826fbf`, а до той правки —
`#34171250101`/`3dea828`; после правки факта в worker-logs.sh — текущий
запуск выше).
