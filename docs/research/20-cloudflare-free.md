...
«Замер факта: rows_read в проде») показывает: к 16:34 UTC 09-05 накоплено
только 1 346 393 rows_read (26.9 % от суточного лимита), пиковый час утра —
05:00 с 496 129. Сжечь 5 000 000 к 04:35 UTC при таком профиле физически
невозможно — часовой пик такого порядка держался бы почти 10 часов подряд,
а не один час. **Вывод: квота rows_read НЕ объясняет тишину пульса с 04:35
UTC 09-05** — либо время начала тишины указано неверно, либо причина другая
(кандидат — egress 403, #133). Гипотеза `#status()` как причины исчерпания
квоты 2026-09-03 (день письма Cloudflare) остаётся в силе — она не опровергнута,
опровергнута только привязка к тишине пульса 09-05. Открытая задача — найти
настоящую причину тишины 09-05 (см. текст задачи ниже).
...
| 2026-09-05 (частично, снято к 16:34 UTC) | 1 346 393 | 26.9 % | 05:00 (496 129) | 13 811 |
...
**Чистое сравнение — сутки 2026-09-06, полностью на фиксе.** Тот же прогон
(`python scripts/measure/do_rows_read.py --days 1`, дата `2026-09-06`) даст
первый суточный итог целиком после мержа #321 и будет прямым сравнением с
базой 08-30…09-05 выше. Не выполнено в рамках этого PR — 2026-09-06 на момент
снятия ещё не наступил.
