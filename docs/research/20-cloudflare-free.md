...
**Подключено (issue #693, 2026-09-08):** `cf-worker/wrangler.jsonc` несёт `triggers.crons: ["*/5 * * * *"]` — второй, независимый от alarm() DO источник тактов (замер #689: DO-пульс выдал ≤2 диспатча за ~20.5 ч при ожидаемых ~82; причина — `alarm()` может выйти без будильника при двойном отказе `setAlarm`, восстановление требует внешнего трафика к объекту). Бюджет на аккаунте: **5 триггеров, занят 1** (у `dsh-edge`, второго воркера этого же аккаунта, `crons` не заведены вовсе — проверено по `.github/workflows/deploy-dsh-edge.yml`). Интервал 5 мин, не 15 (=`HEARTBEAT.selfOrchestrationMs`): порог «alarm подвис» (`pulseStale()`) — 2×15=30 мин, 5-минутный тик cron гарантирует наихудшее время починки не больше 35 минут вне зависимости от внешнего трафика к DO — сам инцидент 2026-09-07 (issue #693), который эта мера чинит, длился неограниченно (нет трафика — alarm не восстанавливается вовсе); 35 минут не совпадают с его фактической длительностью, это верхняя граница новой, ограниченной сверху версии того же простоя. Расход: 288 запросов/сутки (requests, не rows) — на здоровом alarm cron-тик не пишет ни одной строки (только чтение `pulse`, `WHERE id=1`); запись (`rows_written`) происходит лишь когда `pulseStale()` истинно (alarm реально подвис) — тогда те же ≤1 строка/тик, что и у самого alarm(). См. `cf-worker/src/harness.ts::Harness#scheduledTick` (докстринг — полное обоснование дедупликации и границы наблюдаемости отказа) и `cf-worker/src/index.ts::scheduled()`.
