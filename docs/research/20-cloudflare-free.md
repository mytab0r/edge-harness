...
**Результат разбивки (run 34661127775):**
...
Сумма за 09-11 сходится с суточной строкой таблицы выше: 4 235 878 / 60 382 ✓.

Атрибуция namespace — из чисел, не из догадки:

- `646b…` = edge-harness: за 09-11 было 20 прогонов `worker.yml` суммарной
  длительностью 1 021 мин у 19 завершённых + 6 мин этого прогона до полуночи
  (`gh run view --json createdAt,updatedAt` по каждому) → ~1 027 вызовов
  `POST /api/heartbeat` раз в 60 с (`scripts/worker/task.sh`, `HEARTBEAT_SECS=60`);
  1 308 записей ≈ 1,27 строки/вызов; 32 925 чтений ≈ 32 строки/вызов — пост-фиксный
  #575 точечный статус-скан. `hands.yml` (второй писатель heartbeat) 09-11 не
  запускался вовсе (последний прогон 2026-09-05), `telegram-webhook.yml` — тоже.
...
окно 09-12 сходится: 1 326 записей / ~220 событий (340 − 80 минус ~2 мин задержки
аналитики GraphQL; задержка откалибрована по heartbeat: 17 записей `646b…` на ~16
физически отправленных пульсов окна) ≈ **6,0–6,3**. Обе оценки — около 6 записей
хранения на одно событие, принятое мордой.
...
- транскрипт: 513 событий/прогон × 6,0 ≈ 3 080 записей; heartbeat: ~54 мин/прогон ×
  ~1,3 ≈ 70; итого **≈ 3 150 `rows_written`/прогон** — на четверть ниже прежней
...
- **Потолок по `rows_written`: 100 000 / 3 150 ≈ 31 прогон `worker.yml`/сутки**
...
**Карта потребителей DO — по факту грепа `.github/workflows/*.yml`** (в прежней
редакции этого раздела `orchestra.yml`/`pr-review.yml` ошибочно числились
потребителями морды — по ним и была посчитана доля «2,5%», снята этим замером):

- ходят в HTTP API морды (`HARNESS_URL`/`DSH_EDGE_URL`, `/api/*`): `worker.yml`
  (heartbeat + транскрипт), `hands.yml` (`HANDS_URL = vars.HARNESS_URL`; 09-11
  не запускался), `telegram-webhook.yml` (09-11 не запускался), `deploy-worker.yml`/
  `deploy-dsh-edge.yml` (канарейки деплоя — чтение морды, вклад в записи <0,5%),
  `plugin-forge.yml` (ручные запуски);
- не ходят в морду вообще: `orchestra.yml` (GitHub API через `gh`), `pr-review.yml`,
  `ai-review.yml`, `quotas.yml` (GraphQL Cloudflare, не DO), `repo-ci.yml`,
  `worker-ci.yml`, `codeql.yml`, `branch-protection-watch.yml`,
  `dispatch-latency-probe.yml`;
- собственный периодический писатель DO — пульс `alarm()` + cron `*/5` (#693) —
  на здоровом тике не пишет ни строки (только чтение `pulse WHERE id=1`), поэтому
  1 308 записей `646b…` за 09-11 объясняются heartbeat'ом без остатка.

**Не подтверждено:** почему heartbeat стоит ~1,27 строки/вызов за 09-11 и ~1,0–1,1
за окно 09-12 (построчный разбор `harness.ts` вне рамок замера); источник 49 записей
третьего namespace (воркер не наш — дополнительных данных нет); цена события —
среднее по прогонам, а не константа: батчи дрена разного размера.
