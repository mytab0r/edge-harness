...
полуночного сброса того же дня, то есть версия «5 млн rows_read сожжены
заново и очень быстро» казалась правдоподобной ДО замера. Причина
предполагалась в коде: `#status()` (дергается на КАЖДЫЙ heartbeat от job —
раз в 20 с, `HEARTBEAT_SECS` в `scripts/hands/dsh_task.sh`, — и на каждый
принятый батч событий) гонял `GROUP BY status` и вторую выборку без индекса
по ВСЕЙ исторической таблице `tasks` (ретеншена для неё нет, растёт вечно), а
не по актуальному срезу. Фикс — issue #320: индекс `tasks(status,
dispatch_ts)` под точечный watchdog-запрос plus in-memory кэш агрегата
счётчиков по статусу, инвалидируемый только записью (не временем). Подробности
и оценка объёма чтений — в PR к #320.

**Противоречие с замером ниже, зафиксировано честно.** Замер факта (раздел
«Замер факта: rows_read в проде», run 33975023605, фактически снят ~15:31 UTC
09-05 — находка ревью PR #325: метка «к 16:34 UTC» в таблице ниже относится
ко ВТОРОМУ, постфиксовому прогону, не к этому) показывает: к моменту снятия
накоплено только 1 346 393 rows_read (26.9 % от суточного лимита), пиковый
час утра — 05:00 с 496 129. Сжечь 5 000 000 к 04:35 UTC при таком профиле
физически невозможно — часовой пик такого порядка держался бы почти 10 часов
подряд, а не один час. **Вывод: квота rows_read НЕ объясняет тишину пульса с
04:35 UTC 09-05** — либо время начала тишины указано неверно, либо причина
другая (кандидат — egress 403, #133). Гипотеза `#status()` как причины
исчерпания квоты 2026-09-03 (день письма Cloudflare) остаётся в силе — она не
опровергнута, опровергнута только привязка к тишине пульса 09-05. Настоящая
причина тишины 09-05 остаётся открытым вопросом — не задокументирована здесь
отдельной задачей на момент этого замера.
...
**Не подтверждено официальной докой, эмпирически похоже на правду:** ответ
`durableObjectsPeriodicGroups` на range-запрос (`date_geq`/`date_leq`) режется
на 10 000 строк без явного флага обрезки (`has_more` или аналог). Прямого
подтверждения в доке нет — ни на странице метрик, ни в описании introspection.
Числа `--days 7` в этом замере (168 часовых строк максимум) далеко от этого
порога, поэтому обрезка не сработала на реальных прогонах ниже, но при большом
диапазоне (`--days` в несколько месяцев) итог может незаметно занизиться.
`scripts/measure/do_rows_read.py::check_not_truncated` падает громко, если
строк пришло `>= 10000` — это гвардия, а не доказательство точного порога.
...
| 2026-09-05 (частично, снято ~15:31 UTC) | 1 346 393 | 26.9 % | 05:00 (496 129) | 13 811 |
...
`tasks` (восходящий тренд 08-30 → 09-03 в таблице выше это подтверждает —
рост немонотонный: 09-02 ниже 09-01, но общее направление вверх).
...
### Контрольный прогон сразу после мержа #321 (2026-09-05) — чистого сравнения ещё нет

#321 слит и задеплоен в 16:16:42 UTC 2026-09-05
([деплой-run 33977361440](https://github.com/mytab0r/edge-harness/actions/runs/33977361440),
`headSha` = merge commit `2161d04`). Тем же инструментом снят повторный
прогон в 16:34 UTC — [run 33978258705](https://github.com/mytab0r/edge-harness/actions/runs/33978258705),
через 18 минут после деплоя:
...
| **2026-09-03** | **5 264 205** | **105.3 %** | 21:00 (678 075) | 81 578 |
| 2026-09-04 | 882 708 | 17.7 % | 19:00 (244 065) | 11 417 |
| 2026-09-05 (частично, снято к 16:34 UTC) | 2 001 736 | 40.0 % | 16:00 (650 904) | 16 689 |

Разбивка по `namespaceId` за то же окно: `a59c9e7e4ef541878ab82100923cf4e4` —
13 779 963, `646b270ee51f4ca3bcb6badd9de63a98` — 85 763, третий — 0. Тот же
namespace-харнес остаётся основным потребителем — ожидаемо, деплой фикса не
меняет, какой DO читает.

**Это НЕ чистое «было/стало» — честно, не выдумывая вывод.** 2026-09-05
смешивает дофиксовый код (00:00–16:16 UTC, 16 часов 16 минут) и постфиксовый
(16:16–16:34, 18 минут на момент снятия). Пиковый час дня — 16:00 (650 904
rows_read), но это часовой бакет, в котором фикс включился только на 16-й
минуте — то есть даже этот час большей частью дофиксовый, а не доказательство
того, что фикс не сработал. Оценить эффект фикса по суточному итогу 09-05
нельзя: слишком мало постфиксового времени внутри дня, чтобы его вклад был
виден на фоне 16 часов старого кода.

**Чистое сравнение — сутки 2026-09-06, полностью на фиксе.** Первый суточный
итог целиком после мержа #321, прямое сравнение с базой 08-30…09-05 выше.
Не выполнено в рамках этого PR — 2026-09-06 на момент снятия ещё не наступил.
Рецепт (находка ревью PR #325): скрипт всегда строит окно ДО сегодняшней
даты UTC на момент запуска — `--days 1`, снятый 09-06, даёт частичный день
(окно ещё не закрылось), а снятый 09-07 — уже окно для 09-07, не 09-06.
Чтобы получить полную строку 09-06, снимать `--days 2` НА 09-07 ИЛИ ПОЗЖЕ и
брать из таблицы отчёта строку `2026-09-06` (не последнюю).
...
**Ретеншн — не опция, а обязательный элемент дизайна (#306/#305).** Таблица без
удержания растёт вечно, и любой скан со временем дорожает — тот же класс, что
подпалил суточный `rows_read` (#320): цена SELECT/DELETE без LIMIT и без
индекса линейна по числу строк, а число строк ничем не ограничено. Фактические
размеры на 2026-09-05 (оценка по количеству запусков `hands.yml` — 70 за всю
историю репозитория, живого доступа для прямого чтения таблиц DO SQLite нет —
см. «Что не подтверждено»): `tasks` — порядка 70 строк, `events` — порядка
десяти событий жизненного цикла job'а (`task_queued`/`dispatched`/`job_start`/
`first_heartbeat`/`job_end`, `scripts/hands/dsh_task.sh`) на каждый task,
то есть низкие сотни строк суммарно. Обе таблицы малы АБСОЛЮТНО — острая часть
инцидента #320 была не в объёме данных, а в ЧАСТОТЕ полного скана (heartbeat
каждые 20 с на протяжении job'а), что уже пофикшено кэшем/индексом (#320/PR
#321). Ретеншн — защита горизонта: без него та же таблица через месяцы/годы
эксплуатации станет большой сама по себе, и тогда даже единичный SELECT без
LIMIT снова упрётся в rows_read. Политика по каждой таблице и портируемая форма
пакетного удаления (`DELETE ... WHERE id IN (SELECT ... LIMIT ?)`, без
зависимости от сборки SQLite с `SQLITE_ENABLE_UPDATE_DELETE_LIMIT`) —
`openspec/changes/do-sqlite-retention/proposal.md`.
...
### Worker не может `fetch()` другой Worker на `*.workers.dev` того же аккаунта — ошибка 1042 (issue #501)

Живая находка, не документная: воркер `dsh-edge.mytab0r.workers.dev`
(dsh-edge, патч `0005-harness-status-proxy`) делал обычный `fetch()` на
`https://edge-harness.mytab0r.workers.dev/api/events` — оба воркера в одном
Cloudflare-аккаунте, оба на публичном поддомене `*.workers.dev`. Cloudflare
блокирует такой subrequest: ответ — HTTP 404 с телом `error code: 1042`
(не JSON, форма — стандартная страница ошибки edge, а не ответ воркера-адресата).
Прямой запрос к тому же журналу с тем же Bearer-токеном (не из воркера, а с
раннера GitHub Actions) в тот же момент отвечал 200 — значит блокировка
именно на паре «воркер вызывает воркер того же аккаунта через workers.dev»,
не на токене/маршруте/данных. Воспроизведено живым прогоном (прямой запрос
200, запрос через прокси 404 `error code: 1042` — оба сняты в одном workflow,
issue #501).

Фикс — **service binding**: `wrangler.jsonc` вызывающего воркера объявляет
`"services": [{"binding": "NAME", "service": "<script-name-адресата>"}]`,
код зовёт `env.NAME.fetch(...)` вместо глобального `fetch(url)`. Service
binding идёт Worker-to-Worker напрямую (нет публичного DNS/HTTP), поэтому
ограничение 1042 (специфичное для publicly-routed subrequest между двумя
`*.workers.dev` одного аккаунта) на него не распространяется — подтверждено
задачей #501 (до фикса — 404 у каждой строки раздела «Интеграции», после —
чисто). Хостнейм в URL, передаваемом в `service.fetch()`, DNS не резолвится
(запрос уходит напрямую адресату) — использовать плейсхолдер вида
`https://internal/...`, адресат смотрит только на pathname/search.

**Не подтверждено:** держится ли ограничение 1042 для воркеров на **custom
domain** (не `*.workers.dev`) того же аккаунта — не проверяли, поскольку оба
воркера здесь на `workers.dev`; официальной доки с точным условием
блокировки 1042 не нашли (страница ошибки Cloudflare её не документирует
как публичное поведение platform limits, только как код ошибки edge).
...
10. **Фактический размер таблиц `tasks`/`events` этого аккаунта на 2026-09-05.** Нет инструмента для прямого чтения строк DO SQLite вне самого воркера (issue #324 — сборщик метрик квот, не таблиц); число выше — оценка по количеству запусков `hands.yml` (GitHub Actions), не прямое измерение.
