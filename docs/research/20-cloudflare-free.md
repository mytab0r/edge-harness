...
## API v4: что реально отдаёт токен репозитория (прогон 2026-09-06, issue #322)

Проверено живыми GET-запросами в CI ([запуск на финальном коде](https://github.com/mytab0r/edge-harness/actions/runs/34011893138),
SHA `669babb`, 2026-09-06, 12/12 секций успешно), не по докам. Предыдущая
ссылка (`#33993667102`, SHA `0b85fd5`) заявлялась как финальный код, но была
недостижима из мерж-хеда — история переписана после прогона, реальные
изменения `scripts/cf/` (151 строка) в CI не выполнялись ни разу (находка
ревью PR #328, п.1). Более ранние прогоны выполняли код без фильтра
bindings-значений и печатали их в лог — логи удалены (ревью PR #328, находка
1 предыдущего раунда), актуальна только ссылка выше. Инструменты —
`scripts/cf/`, справочник для агентов — [docs/agents/INFRA-CF.md](../agents/INFRA-CF.md).
