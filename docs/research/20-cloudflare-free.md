...
(`dsh-edge/patches/0004-harness-ingest.patch:281-336` — блок кэша `baseTurn`, адрес на момент PR #1173) на КАЖДЫЙ вызов
`POST /api/sessions/:id/ingest` делает полный проход по всей истории сессии
(`openAgentForTurn` + `snapshotEvents()`), только чтобы вычислить `baseTurn`
— O(вся история) чтений НА ВЫЗОВ, независимо от размера батча.
...
ingest — «на КАЖДЫЙ вызов» ниже больше не факт о проде. Разбивка текущих
