...
(`dsh-edge/patches/0004-harness-ingest.patch:260-282`) на КАЖДЫЙ вызов
...
**Не устраняет корень**: `appendHarnessEvents` по-прежнему делает O(N)
пересчёт на каждый вызов — мера первой очереди снижает ЧАСТОТУ срабатывания
причины, не саму причину. Задача на кэширование `baseTurn` вместо полного
`snapshotEvents()` заведена отдельно — issue #1049.
