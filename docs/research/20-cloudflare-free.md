...
**«Кто читает миллионы строк в dsh-edge» (вопрос выше, оставленный открытым
этим замером) установлен задачей #1049 (2026-09-13), кодом, а не догадкой:**
`EdgeSessionStore.appendHarnessEvents` (`dsh-edge/patches/0004-harness-ingest.patch`)
диспоузил Agent-хэндл после каждого батча — следующий вызов заново проходил
`openAgentForTurn` → `agents.resume()` → `persistence.prepare()` →
`backend.loadStored(id)`, а тот безусловно читает ВЕСЬ хранимый лог сессии
(`do-session-persistence.ts::eventRows`, `SELECT ... WHERE session_id = ?
ORDER BY seq`, без LIMIT) на каждый вызов — независимо от размера батча.
Собственный скан патча (`session.snapshotEvents()` для `baseTurn`) сюда не
добавлял ничего: `Session.snapshotEvents()` читает уже материализованный в
памяти массив, не SQL. Подробности и исходники —
`docs/research/12-dsh-edge-session-api.md`, раздел «Дописывание события в
чужую сессию без резюма Agent — недостижимо». Фикс (`dsh-edge/PATCHES.md`,
«Тёплый хэндл ingest») держит уже открытый хэндл тёплым между батчами одной
сессии вместо диспоуза на каждый вызов; ожидаемый эффект и точный рецепт
измерения post-deploy — там же, честная оценка без живого замера (задача не
включает деплой).

**Мера первой очереди (issue #1048), принятая ДО #1049 и всё ещё
действующая:** `DRAIN_INTERVAL_SECS` фонового дрена раннера поднят 1с → 30с
(единственное место правды — `DSH_EDGE_DRAIN_INTERVAL_DEFAULT_SECS` в
`scripts/lib/dsh-edge-session.sh`, пол 10с держит гвардия
`scripts/lib/test_drain_interval_guard.py`). Диагноз, ради которого мера была
принята («частота вызовов ingest = частота причины»), верен даже после
находки #1049 (сам скан `snapshotEvents()` бесплатен, но частота ВЫЗОВОВ
`appendHarnessEvents` по-прежнему определяет, сколько раз в сутки платится
холодный ресум) — мера снижает ЧАСТОТУ срабатывания причины (оценка ≈4,8×
меньше вызовов при K=30с против K=1с), не саму причину; сама причина —
предмет #1049 выше. `HARNESS_INGEST_IDLE_MS = 120_000` (#1049,
`dsh-edge/PATCHES.md`) взят с запасом именно над этим интервалом 30с, чтобы
нормально активный дрен не вытеснял тёплый хэндл между батчами.
