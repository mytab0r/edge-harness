...
| **Cron Triggers** | **не используем как отдельный механизм** — вместо него DO `alarm()` | В `wrangler.jsonc` нет секции `triggers.crons`. Cron Triggers на Free: 5 триггеров/аккаунт, 10 ms CPU на тик ([Workers Limits](https://developers.cloudflare.com/workers/platform/limits/), «Last updated Sep 5, 2026» — снимок, сверено живым запросом 2026-09-07: «Number of Cron Triggers per account 5», «CPU time per Cron Trigger 10 ms»); минимальный интервал — «At every minute» `* * * * *` ([Cron Triggers](https://developers.cloudflare.com/workers/configuration/cron-triggers/), «Last updated Sep 4, 2026» — снимок) | Cron Trigger будит **воркер**, без привязки к конкретному DO-инстансу и его состоянию — пришлось бы делать доп. `fetch` к DO изнутри cron-хендлера ради того же результата, что `alarm()` даёт напрямую. Осознанно не меняем: alarm ближе к данным, cron был бы лишним прыжком, не выигрышем |
...
Разбор кандидатов на замену (почему Cron Triggers не меняем на alarm, почему
git-ref-замок аренды задачи уже и есть нативный примитив) — в
