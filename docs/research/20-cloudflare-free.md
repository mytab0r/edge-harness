...
## API v4: что реально отдаёт токен репозитория (прогон 2026-09-05, issue #322)

Проверено живыми GET-запросами в CI ([запуск](https://github.com/mytab0r/edge-harness/actions/runs/33975422670)),
не по докам. Инструменты — `scripts/cf/`, справочник для агентов —
[docs/agents/INFRA-CF.md](../agents/INFRA-CF.md).

- **Аккаунт общий с другими проектами владельца.** `/workers/scripts` вернул
  9 воркеров, этому репозиторию принадлежат 2 (`edge-harness`, `dsh-edge`).
  То же для DO namespaces (3 в аккаунте, наших 2), KV (7), D1 (4), R2 (1) —
  все листинги account-wide, без фильтра по проекту в самом API.
- **Зон (доменов) в аккаунте 0.** Закрывает issue #289: своего домена у
  Cloudflare-аккаунта нет вообще, не только не подключён к этому проекту.
- **Историю логов воркера REST не отдаёт.** `/logpush/jobs` → 403 (токену не
  хватает права Logs Read); даже с этим правом Logpush — это архивация во
  внешнее хранилище, настраиваемая заранее, а не «покажи логи за последний
  час». Единственный способ увидеть логи в моменте — `wrangler tail`
  (WebSocket, интерактивно, не годится для CI).
...
- **`/workers/scripts/{name}/settings` — рабочий способ узнать имена bindings**
  без раскрытия значений: `type: "secret_text"` для секретов, `type:
  "plain_text"`/`"assets"`/`"durable_object_namespace"` для остального.
  `/workers/scripts/{name}/deployments` даёт историю деплоев (id, timestamp,
  triggered_by) — то самое «последний деплой», которое не находилось руками.
- **Инцидент при первом прогоне:** account-wide листинг, напечатанный сырьём,
  утёк в публичный Actions-лог (имена чужих воркеров того же аккаунта). Лог
  удалён (`gh api -X DELETE .../actions/runs/{id}/logs`), класс закрыт
  фильтром на уровне API-обёртки (allowlist `edge-harness`/`dsh-edge`), не
  разовой правкой — см. `scripts/cf/lib.sh`.
