...
### #678: цена одного прогона `worker.yml` в записях (`rows_written`) — живой срез 2026-09-07

Зачем: `.github/workflows/worker.yml:27-29` держит `concurrency: group: worker`
(статическая строка — один исполнитель на весь репозиторий), потолок на
Free — 100 000 `rows_written`/сутки. Оценка «хватает на ~17 прогонов в
сутки» (100 000 / 5 800) была выведена из инцидента 2026-09-03 (81 578
`rows_written` за 14 прогонов `worker.yml`) — то есть **в разгар дефекта
неограниченного скана `messages`** (#320/#575, см. выше), который с тех пор
починен и слит. Вопрос: цена прогона сейчас ниже?

**Срез снят** `quotas.yml` (`workflow_dispatch` на `main`,
[run 34163953822](https://github.com/mytab0r/edge-harness/actions/runs/34163953822),
2026-09-07T21:40:11Z, код с `main`, не с ветки #678 — `quotas.py` не зовёт
разбивку по `namespaceId` вообще, поэтому ветка тут ничего не меняет):

| Метрика | Значение | Лимит | % |
|---|---|---|---|
| `rows_written`/сутки | 29 625 | 100 000 | 29.6 % |
| `rows_read`/сутки | 3 819 259 | 5 000 000 | 76.4 % |

Снято в 21:40 UTC — 90.3 % суток (00:00→21:40 из 24 ч) уже прошло. Линейная
экстраполяция на полные сутки (грубо, не факт): `rows_written` ≈ 32 800
(32.8 %), `rows_read` ≈ 4 229 000 (84.6 %).

**Прогонов `worker.yml` за сегодня (UTC 2026-09-07) — 8**: 4 success, 3
failure, 1 ещё выполнялся на момент среза (`gh run list --workflow=worker.yml`).
За то же окно тот же харнес дёргали ещё **167 прогонов `orchestra.yml`** и
**65 прогонов `pr-review.yml`** (оба реально ходят в HTTP API воркера — грепом
по `HARNESS_URL`/`/api/tasks` в `.github/workflows/*.yml`); `ai-review.yml`
(73 прогона) в этот список не входит — в его workflow нет обращений к DO.

**Честно: цена одного прогона `worker.yml` этим срезом НЕ изолирована.**
`quotas.py` считает `rows_written`/`rows_read` суммарно по аккаунту, без
разбивки по вызвавшему workflow; разбивка по `namespaceId` (правка #678 в
`do_rows_read.py`) разделяет DO-неймспейсы одного CF-аккаунта (харнес vs.
что-то ещё на аккаунте), а НЕ прогоны `worker.yml` от прогонов
`orchestra.yml`/`pr-review.yml` — все они бьют в один и тот же
DO-неймспейс харнеса. Чтобы получить такую разбивку, нужно гонять
`do_rows_read.py` напрямую (сейчас это только `deploy-worker.yml`, запускать
который в рамках этой задачи запрещено) — открытый пробел, см. ниже.

Наивная (заведомо завышающая) оценка — весь сегодняшний расход приписан
только прогонам `worker.yml`:

- `rows_written`: 29 625 / 7 завершённых прогонов ≈ **4 232 записи/прогон**
  (было 81 578 / 14 ≈ 5 827 ≈ «5800» 2026-09-03 — **ниже примерно на 27 %**,
  направление совпадает с ожиданием от фикса #575, но метод контаминации тот
  же на обеих датах: у 2026-09-03 тоже не было изоляции от прочих workflow,
  так что это сравнение «того же грубого метода», не чистое до/после).
- `rows_read`: 3 819 259 / 7 ≈ **545 608 строк/прогон** тем же наивным
  методом.

**Бюджет `rows_written` (100 000/сутки) наивной ценой прогона:**
100 000 / 4 232 ≈ **23 прогона/сутки**, если бы `worker.yml` был
единственным потребителем. Но `worker.yml` — только 8 из ≈314 прогонов всех
workflow сегодня (2.5 %), реальная цена одного прогона `worker.yml`
почти наверняка ниже 4 232, а значит настоящий потолок выше 23 — но
насколько именно, эта проверка не отвечает без прямой разбивки по workflow.

**`rows_read`, а не `rows_written`, остаётся более тесным ограничением.**
Тем же наивным методом: 5 000 000 / 545 608 ≈ **9 прогонов/сутки** — вдвое
меньше, чем по `rows_written`. И это не гипотеза: `rows_read` сегодня уже на
76.4 % при 90.3 % прошедших суток — «#678» ставил вопрос про `rows_written`,
но `rows_read` (тот самый счётчик из #320/#575) фактически ближе к потолку
прямо сейчас.

**Параллельные исполнители не умножают суточный бюджет.** Оба счётчика —
аккаунт-wide за календарные сутки, общие на ЛЮБОЕ число одновременных
исполнителей: снятие `concurrency: group: worker` не добавляет прогонов в
день, а лишь распределяет те же ≈9–23 (в зависимости от того, какая метрика
рулит) на N параллельных раннеров вместо одного — календарные сутки
проходятся быстрее, но общий потолок прогонов в сутки не меняется.

**Что не хватило этому замеру:** прямая разбивка `rows_written`/`rows_read`
по workflow (не по namespaceId — правка #678 разделяет DO-неймспейсы одного
CF-аккаунта, не GH-workflow) сейчас доступна только через `deploy-worker.yml`
(job `measure-rows-read`, запуск которого в рамках этой задачи запрещён) или
локальный прогон `do_rows_read.py` с прод-токеном (секреты не извлекаются).
Следующий чистый шаг: снять `deploy-worker.yml` на полные сутки ПОСЛЕ
2026-09-08 (первые чистые сутки целиком на фиксе #575) и сравнить
разбивку по `namespaceId` с базой 2026-09-03/09-05 выше — это подтвердит
или опровергнет долю харнеса в общем `rows_written`, но не отделит
`worker.yml` от `orchestra.yml`/`pr-review.yml` — для этого нужен отдельный
инструмент (например, метка job/workflow, которую сам DO нигде не пишет
сегодня).
