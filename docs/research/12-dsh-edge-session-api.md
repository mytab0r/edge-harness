...
- Прокси статусов `/api/harness/*` на воркере морды (закрытие белого пятна
  ...
  но его источника нет в этом репо (patches/ на main — 0001-0004 без
  harness-GET). Неавторизованный зонд `GET /api/harness/events` отвечает
  тем же `401 {"ok":false,"error":"Owner authentication required."}`, что и
  заведомо несуществующий путь, — auth в морде стоит ДО роутинга, поэтому
  существование/форма маршрута зондом не доказывается; plugin-manager
  (#113, PR пост-мержа 0.1.3) опирается на прокси с проверкой формы ответа
  журнала и громкой ошибкой при несоответствии. Живая проверка прокси И
  заказа с кукой владельца — беклог-задача #233 (рамка расширена комментарием
  в задаче).
