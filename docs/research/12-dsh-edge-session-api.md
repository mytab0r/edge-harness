...
- Контракт `session.prompt`/`session.history` (#113, раздел выше) снят с
  ...
  несоответствии контракту, так что после первого зелёного деплоя с этим
  шагом формулировка «не подтверждено живым прогоном» для ЭТИХ ДВУХ методов
  снимается (за заказом плагинов из морды остаётся режим `queue`/UI —
  не та же поверхность).
