...
## Маршрут /api/harness/events (#86)

Читатели статусов (plugin-manager, integrations) ходят в `GET /api/harness/events`
— с #86 это ПАТЧ МОРДЫ (`dsh-edge/patches/0005-harness-pipeline-view.patch`,
источник в репо): проекция событий сессии-конвейера `harness-pipeline`
(инжест через патч 0004) в форму ответа прежнего журнала edge-harness. Раньше
маршрут существовал только в деплое владельца (закрытие белого пятна #105,
релиз-ноты plugins-manager-v0.1.2) как прокси на списанный журнал — #235 закрыт
по существу: источник в репо, цель — своя сессия, чужой воркер не нужен.
Форма запроса/ответа не изменилась, плагины не перевыпускались. Живой прогон
маршрута — интеграция `dsh-edge/pipeline-integration/check.mjs` на каждом
деплое морды и канарейка канала в `decommission-worker.yml`.
