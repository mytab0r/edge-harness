...
`dsh_edge_sessions`, то есть память в облаке есть (по-русски: есть место,
куда писать и откуда читать, агент может вернуться к незаконченной работе).
Блокер задачи #133 — не отсутствие памяти, а 403 GitHub API на эгрессе из
воркера морды (`PROTOCOL.md`, запись 2026-08-31; ADR 0009). Не путать с
`error code: 1010` выше в этом файле — тот фильтр стоит ПЕРЕД мордой, по
подписи клиента (направление клиент→морда, эксперимент #225), а блокер #133 —
направление морда→GitHub на эгрессе, другой факт и другая улика. Решение:
полностью отделить раннерский код от GitHub (только dispatch для запуска,
дальше работает в DSH).
