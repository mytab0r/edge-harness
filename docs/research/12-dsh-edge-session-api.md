...
незаконченной работе). Движок хранения зафиксирован живым замером
`/api/health` (2026-09-03; дословная цитата владельца — в комментариях #133,
`gh api repos/mytab0r/edge-harness/issues/133/comments`):
`"storage":"durable-object-sqlite-vfs"` — Durable Object с SQLite, не KV/R2
(перенесено сюда из design.md change'а `cloud-orchestrator-convergence`,
находка ревью PR #262, одиннадцатый раунд). **не подтверждено, что таблица
называется именно `dsh_edge_sessions`**: апстрим не вендорится в этот
репозиторий (только патчи
