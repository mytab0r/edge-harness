...
- Имя поля идентификатора сессии в ответе `session.list {}` (#940): раздел
  «RPC» выше подтверждает живым вызовом только `items[].projections.
  values.title`, не имя поля с самим `sessionId`. `scripts/orchestra/
  session_orphan_sweep.py::session_id_of` пробует `item["sessionId"]`, затем
  `item["id"]` (тот же именной приём, что несёт каждый другой session.*-метод
  этого API) и честно пропускает элемент с явным предупреждением, если не
  нашёл ни одного — до первого живого прогона на проде это остаётся
  предположением, не фактом.
- Пагинация `session.list {}` (#940): не подтверждено, что ответ вообще
  имеет форму «страница» (курсор/`hasMore`, как `beforeSeq` у
  `session.history` выше, — или полный список всегда). Единственный
  потребитель списка, `session_orphan_sweep.py`, читает ОДНУ страницу и
  называет это ограничение в каждой строке отчёта прогона (находка ревью
  PR #944: молчаливая одностраничность теряла бы хвост списка — а рост
  числа сессий и есть предмет #940). Проверить живым прогоном; если
  выяснится курсор — дописать контракт в раздел «RPC» здесь и обход в
  `fetch_session_list`.
