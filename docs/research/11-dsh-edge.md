...
- цена, на которую жаловался #1049 (холодный `agents.resume()` читает весь лог — `do-session-persistence.ts::eventRows(id, 0)` без LIMIT), на 0.14.0 платится один раз на сессию за активацию DO ещё до всяких наших кэшей: резидент живёт до выгрузки DO. Посмертный замер `rows_read`/событие, на который раньше ссылалась эта строка (секция «Тёплый хэндл ingest» [`dsh-edge/PATCHES.md`](../../dsh-edge/PATCHES.md)), в main НЕ существует: секция жила только на ветке PR #1057 (`agent/1049-harness-ingest-baseturn-cache`) и не была смержена; после переезда на 0.14.0 замер и неприменим — его база (холодный ресум на каждый ingest-вызов) исчезла, число в прежней постановке мерить не о чем (разбор и решение развилки — [`dsh-edge/PATCHES.md`](../../dsh-edge/PATCHES.md), «Закрытие #1149»);
...
**Уточнение 2026-09-14 (#1163): тот же слом уже жил в main, независимо от PR #1057.** Абзац выше описывает столкновение резидентов с ЕЩЁ НЕ СМЕРЖЕННОЙ машинерией #1049/PR #1057. Но и уже смерженная в main версия патча 0004 (без этой машинерии — она диспоузит `AgentHandle` в `finally` после КАЖДОГО ingest-батча, не только по idle-таймеру) ломается тем же способом: первый источник-сборка деплой на пине 0.14.0 (слияние #1088, run `34773180930`) прошёл дальше предыдущего красного и упал именно здесь — батч 1 в сессию `ingest-check` проходит, батч 2 в ТУ ЖЕ сессию сразу следующим вызовом (без idle-задержки) получает `session "ingest-check" is not live in this store`. Точный стек прогона (`gh run view 34773180930 --log-failed`) — факт: брошено из `Proxy.liveEntryFor`, вызванного из `Proxy.flush`, вызванного из `EdgeSessionStore.appendHarnessEvents` — то есть батч 2 падает внутри `sessions.flush(session)` (публикация живым подписчикам), а не при открытии хэндла; связь с резидентным кэшем выше (dispose не снимает сессию с публикации живым подписчикам корректно) — объясняющая модель, не сверенная байт-в-байт с исходником. Это уже живой прод-инцидент, не только риск доводки PR #1057 — #1163 чинит именно его (снимает `finally`-dispose из уже смерженной версии 0004), уже, чем #1149 (не трогает остальные патчи серии, не принимает решения по развилке выше).
...
Уточнение 2026-09-06 (issue #518, бамп пина 0.8.0 → 0.11.1, #505/PR #513): апстрим убрал `@deepseek-ai/dsh-client-runtime` в релизе 0.10.0 (его собственный `docs/releases/0.10.0.md`: «Upstream removed `dsh-host-apiproxy` and `dsh-client-runtime`»). На пине 0.11.1 `ui-edge`'s `dsh.client.inject` — `["@deepseek-ai/dsh-api-workspace-controller", "@deepseek-ai/dsh-client-ui-settings", "@deepseek-ai/dsh-client-locale"]` (`dsh-client-runtime` из списка исчез без замены). Сервис `slots`, который раньше нёс `dsh-client-runtime`, на 0.11.1 приносит `@deepseek-ai/dsh-client-ui-renderer` (`SlotRegistry extends Service; constructor(ctx) { super(ctx, 'slots') }`, снято с опубликованного `lib/client.js` пакета) — этот пакет `{"client":{"platform":"web","immediately":true}}`, входит в `cordis.patch.yml` `@deepseek-ai/dsh-base`/`@deepseek-ai/dsh-web-app` (reviewed-состав, всегда в ростере) и `ui-edge` его package.json НЕ декларирует вовсе — рабочий пример того, что «поставщика» always-on сервиса объявлять не нужно. Наши клиентские плагины (`plugins-src/plugin-manager`, `plugins-src/integrations`) раньше копировали старую декларацию `dsh-client-runtime` — сборка `assemble-standalone-web.mjs` красилась `injects missing Edge Web package @deepseek-ai/dsh-client-runtime`; фикс снял запись по образцу `ui-edge`, оставив только `dsh-client-locale`.
...
- **Два разных `inject`**: `dsh.client.inject` в package.json — модули, загружаемые ДО пакета (сервисы готовы к `apply`); `exports.inject` из бандла — имена СЕРВИСОВ ctx (кордис-сервисы). До пина 0.10.0 сервис `slots` ставил клиентский плагин `dsh-client-runtime` (удалён апстримом в 0.10.0), `locale` — `dsh-client-locale`. На пине 0.11.1 (issue #518) `slots` ставит `@deepseek-ai/dsh-client-ui-renderer` — always-on пакет reviewed-состава, package.json потребителя (как `ui-edge`, так и наших `plugin-manager`/`integrations`) его вообще не обязан объявлять в `dsh.client.inject`.
...
- **Серверная часть**: алиасы бандла `bundle-standalone.mjs` выводятся только из литералов импортов в `apps/dsh-edge/src` — плагин, который никто не импортирует, в бандл воркера не попадает. Композиция — статические `Context.install` в `session-store.ts`.
...
Закрытие в этом репозитории (не в апстриме; 2026-09-05, [белое пятно #43](https://github.com/mytab0r/edge-harness/issues/43)): выбран вариант «патч-серия поверх пинового клона» и построен в `deploy-dsh-edge.yml` — серию патчей и гвардии состава описывает [`dsh-edge/PATCHES.md`](../../dsh-edge/PATCHES.md) (композицию достраивает патч 0002 + кодогенерация, ростер клиентских плагинов — патч 0003, оба закрывают пункты 1–2 выше). Пункт 3 закрыт тем, что зависимости плагинов ставит `pnpm add <tgz>` (все семь патчей переживают add — подтверждено в design dsh-edge-plugin-system), за ним в деплое стоит проверка `patchedDependencies`; второй пакетный менеджер и маскирующий флаг `--legacy-peer-deps` запрещены гвардией `scripts/lib/test_dsh_edge_pnpm_guard.py` (шаг в repo-ci). У апстрима динамической загрузки по-прежнему нет — спайк `pawaca/dsh-edge#43` открыт (перепроверено 2026-09-05).
...
- **Кнопки добавления/редактирования провайдера в Settings → Models неактивны.** UI-компоненты `CustomProviderCard` и `ProviderEditor` из upstream пакета `@deepseek-ai/dsh-client-ui-settings-models` исправны. Дело **не** в запрете записи: морда объявляет себя пишущим провайдером настроек явно — `do-settings-provider.ts:21`, `override readonly writable = true`. Корневая причина раздвоена и различается по типам кнопок. (1) **"Add"** (добавить провайдера из directory) отключена условием `disabled: addable.length === 0 || !state.writable` (`@deepseek-ai/dsh-client-ui-settings-models@0.0.1-rc.3`, `lib/client.js:2015`), где `addable = rows.filter(r => !r.configured && r.entry.settingsNs !== "")` (строка 1844). Единственный провайдер морды (`deepseek-official`) уже отмечен `configured=true`, поэтому список `addable` всегда пуст. (2) **"Add custom provider"** (`CustomProviderCard`) отключена условием `disabled: protocols.length === 0 || !state.writable` (строка 2029), где `protocols = protocolChoices(state.namespaces.get("llm-pi-ai"))` (строка 1847). Namespace `llm-pi-ai` не смонтирован в морде → `protocolChoices(undefined)` возвращает `[]` (`lib/client.js:488`) → кнопка недоступна независимо от `writable`. Практический вывод: `writable = true` — необходимое, но не достаточное условие для ни одной из двух кнопок. Когда компоненты всё же активны, пути записи разделяются: правка существующего провайдера (`ProviderEditor`) берёт settings-namespace **динамически** из ответа сервера (`const ns = namespace.ns` → `api.settings.mutate({ ns, ops })`, строки 1424, 1442-1446), namespace приходит из поля `settingsNs` directory-записи. Создание нового провайдера (`CustomProviderCard`) пишет в **жёстко зашитый** литерал `"llm-pi-ai"` (строка 1054). Практическое следствие: собственный settings-namespace работает с UI для правки существующего провайдера; чтобы ожила кнопка создания, серверу нужно смонтировать namespace ровно с именем `llm-pi-ai`, который можно реализовать без пакета `dsh-llm-pi-ai`.

  **Реакция (#378, 2026-09-03): обе причины закрыты серверным плагином `provider-registry`** (change
  `openspec/changes/dsh-edge-provider-registry`, релиз `plugins-registry-v0.1.1`, запись в
  `dsh-edge/plugins.json`), без пакета `dsh-llm-pi-ai`: (1) плагин монтирует settings-namespace
  `llm-pi-ai` со своей схемой (форма апстримного `Config`: `providers`-словарь; `api` — union с
  единственным протоколом транспорта `openai-completions`) — `protocolChoices` оживают; (2) плагин
  публикует directory-записи готовых OpenAI-compat маршрутов (`zhipu`, `nvidia-nim`, `openrouter`,
  `deepseek`) — они видны кнопке «Add», пока не настроены. Каждая настроенная запись
  `providers.<route-id>` становится живым маршрутом пикера (один `DeepSeekAdapter` на маршрут,
  ключи — штатный credential-механизм морды). На пине 0.8.0 (клиент 0.1.1-rc.2) контракты
  перепроверены по бандлу: `NS$1 = "llm-pi-ai"` (строка 1077), гейты кнопок (2056/2070),
  `layoutOf` (1377-1379), `protocolChoices` читает union по пути `providers.<probe>.api` из
  `settings.describe`. Гвардия литерала в деплое (`dsh-edge/verify-provider-namespace.mjs`,
  мутационно проверена) роняет деплой, если апстрим переименует `NS$1`. Дым инсталла
  (`smoke-edge-plugins.mjs`) и интеграция на собранном артефакте
  (`dsh-edge/registry-integration/check.mjs`: namespace, directory, негатив при записи,
  добавление, пикер, рестарт DO, удаление) закрывают класс «зелёная сборка с мёртвыми кнопками».

  **Фундаментальная причина (до #378).** `session-store.ts:192-303` строит cordis-композицию с нуля и монтирует **только** адаптер `dshLlmDeepseek` (строка 243), без пакета `dsh-llm-pi-ai` (мультипровайдерный пул). Provider ID адаптера захардкожен как `deepseek-official`; settings namespace `llm-pi-ai` не существовал, поэтому directory `llm.providers` содержал **ровно один** route — не пустой список, а список без выбора. Это осознанное решение апстрима: согласно заметке `.agents/notes/implemented/architecture/2026-08-22-dsh-edge-v0.4-multi-provider-vision.md` (раздел Excluded), пакет `dsh-llm-pi-ai` зависит от Node.js-only SDK'ов и превышал бы gzip-бюджет Worker'а. До реестра смена провайдера была ЗАМЕНОЙ единственного слота через `vars` деплоя или Settings → baseURL, а не добавлением второго параллельного. Маршрутизация поверх нескольких провайдеров — задача #158 (реестр этого не делает); пин апстрима см. `dsh-edge/upstream.json` (бамп #134: 0.8.0 = `b9a8ddd`).
...
- **Кнопки добавления/редактирования провайдера** — гейт вскрыт по клиентскому бандлу и закрыт серверным плагином `provider-registry` (#378, см. «Что НЕ работает»); живой ход через добавленный в морде маршрут на проде не снят (нужна кука владельца и настоящий ключ нового провайдера). Раннерский замер прироста gzip-бандла при монтировании namespace `llm-pi-ai` своим кодом не делался: пакет `llm-pi-ai` не монтируется, цифра 900 KiB и вывод о превышении бюджета относятся к нему и взяты из апстримной заметки.
