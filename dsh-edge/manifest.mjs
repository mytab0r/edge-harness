...
/** Шаблон id (с якорями) — общий для плагинов (manifest.mjs) и интеграций (integrations.mjs): id интеграции становится псевдо-задачей журнала integration:<id>. */
export const ID_PATTERN = new RegExp('^(?:' + ID_BODY_SOURCE + ')$')
...
// имя релиза из манифеста. Тело id БЕЗ якорей берётся из ID_BODY_SOURCE
// (единственное место правды выше), а не копируется литералом: производный
// регэксп, как и ID_PATTERN, обязан менять тело вместе с источником
// (находка AI-ревью PR #660). Вольное имя релиза по образцу соседа —
// класс #659.
const RELEASE_TAG_PATTERN = new RegExp(
  '^plugins-(' + ID_BODY_SOURCE + ')-v(\\d+\\.\\d+\\.\\d+(?:[-+][0-9A-Za-z.-]+)?)$')
