...
//
// Заглушка журнала здесь — node:http, не воркер, поэтому Service binding
// (env.HARNESS_SERVICE, issue #503) к ней не подключить: этот прогон проверяет
// ФОЛБЭК-путь патча (глобальный fetch(HARNESS_URL)), который код использует,
// когда binding не сконфигурирован (ровно этот сценарий — конфиг ниже без
// HARNESS_SERVICE). Контракт прокси (валидация параметров, форма ответа,
// прозрачность чужих кодов) от выбора транспорта не зависит — оба пути после
// получения journalResponse делают одно и то же. Сам факт «binding вместо
// fetch чинит error 1042 на боевых *.workers.dev» — docs/research/
// 20-cloudflare-free.md, «Worker → Worker на *.workers.dev»; здесь его
// нечем воспроизвести (нет второго воркера, только HTTP-заглушка).
//
