 ...
 *
 * Порог — расхождение MAJOR-версии, НЕ любое несовпадение диапазона (эта
 * гвардия такие пары НЕ находит, см. тест MINOR_LAG_ONLY_LOCK): расследование
 * прогоном ПОЛНОЙ проверки диапазона (любое несовпадение, до сужения порога
 * до MAJOR) на пине #1041 нашло, ПОМИМО react-dom/react, восемь пар вида
 * `@deepseek-ai/cordis-plugin-loader@1.0.2` при требовании `^1.0.3` — то же
 * расхождение для КАЖДОГО прежнего (годами зелёного) прогона на этом пине:
 * не регрессия, а давно принятое отставание патч/минор в одном major, где
 * API стабилен по смыслу semver. Именно эта находка и аргументирует порог:
 * фейлить билд на них — шум без сигнала. React error #130 — про
 * несовместимость MAJOR (это и есть breaking change по контракту semver) —
 * только она роняет сборку.
 *
